...
// LightMap 용으로 용도 변환
void DxVMCommandFX::SetStringFX_Piece()
{
    //----------------------------------------------------------------------------- m_strDefindFX
    m_strDefindFX =
    "//-------------------------------------------------------------------------------------- \r\n"
    ...
    //"    float3 vLightMapAvg = tex2D( LightMapAvgTexSampler, In.TexCoordLM ); \r\n"
    " \r\n"
    "    // Specular값 계산      \r\n"
    "    float3 vSpecular = GetSpecular( In ); \r\n"
    " \r\n"
    ...
    /*" \r\n"
    "    // 기본 내적 계산 \r\n"
    "    float fDiffuseDot = saturate( dot( g_vLightDir, normalize( In.Normal ) ) ); \r\n"
    "    float3 vBaseDA = (g_vLightDiffuse * fDiffuseDot * GetDiffusePower()) + (g_vLightAmbient * GetAmbientPower()); \r\n"
    " \r\n"
    "    // NormalMap 내적 계산 \r\n"
    "    fDiffuseDot = saturate( dot( g_vLightDir, g_vWorldNormal ) ); \r\n"
    "    float3 vNormalDA = (g_vLightDiffuse * fDiffuseDot * GetDiffusePower()) + (g_vLightAmbient * GetAmbientPower()); \r\n"
    " \r\n"*/
    " \r\n"
    "    // 기본 내적 계산 \r\n"
    ...
    //"    float3 vColorTemp = vLightColor.xyz * 2 * min( vNormalDA/vBaseDA,vNormalDA ); // LightMap에 알맞게 색을 정함. \r\n"
...
void DxVMCommandFX::MakeStringFX_Piece( TSTRING& strFX, const COMMENT_FX& sCommentFX, size_t nTextureNUM, size_t nCubeTextureNUM, size_t nPixelScalarNUM, size_t nPixelVectorNUM )
{
    strFX.clear();
