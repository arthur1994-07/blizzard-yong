...
namespace sc {	
	...
	*/
	...
	public:
		...
		int			GetSize(void) const;			// Return size of list
		int			GetRefCount(NODE<TYPE>* pData) const;

		bool		IsEmpty(void) const;					// Check, is list empty?
		bool		IsHead(NODE<TYPE>* pData) const;
		bool		IsTail(NODE<TYPE>* pData) const;
		bool		IsEnd(NODE<TYPE>* pData) const;
	};
	...
	template<typename TYPE>
	int TCList<TYPE>::GetSize(void) const
	{ 
		return m_nSize; 
	}

	template<typename TYPE>
	int TCList<TYPE>::GetRefCount(NODE<TYPE>* pData) const
	{
		return pData->nRefCount;
	}

	template<typename TYPE>
	bool TCList<TYPE>::IsEmpty(void) const
	{
		if (m_nSize == 0)
            return true;
		else
	...
	template<typename TYPE>
	bool TCList<TYPE>::IsHead(NODE<TYPE>* pData) const
	{
		if (pData == m_pHead)
            return true;
		else
	...
	template<typename TYPE>
	bool TCList<TYPE>::IsTail(NODE<TYPE>* pData) const
	{
		if (pData == m_pTail)
            return true;
		else
	...
	template<typename TYPE>
	bool TCList<TYPE>::IsEnd(NODE<TYPE>* pData) const
	{
		return IsTail(pData);
	}
