...
namespace SQLite
{
    ...
    Table Database::QuerySQL(const std::string& Query)
    {
        return QuerySQL(Query.c_str());
    }
    ...
    TablePtr Database::QuerySQL2(const std::string& Query)
    {
        return QuerySQL2(Query.c_str());
    }
	...
	{
		// Allocate the string to the number of bytes in the UTF8 string,
		//   (counting until null), which could be too much bytes,
		//   but never too less...
    ...
    int Database::ExecuteSQL(const std::string& Query)
    {
        return ExecuteSQL(Query.c_str());
    }
	...
	{
		m_iLen=lpStr.m_iLen;
		m_strUTF8_MultiByte=new char[m_iLen];
		strncpy_s(m_strUTF8_MultiByte, m_iLen, lpStr.m_strUTF8_MultiByte, m_iLen);
	}
	...
	{
		TCHAR * strRet;
		ConvertUTF8ToString(m_strUTF8_MultiByte, m_iLen, strRet);
