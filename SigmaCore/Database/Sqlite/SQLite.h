...
namespace SQLite
{
	...
	public:
        ...
        TablePtr QuerySQL2(const std::string& Query);
        ...
        Table QuerySQL(const std::string& Query);
	...
	public:
		...
		//! Jgkim
		template< typename T >
		T GetValue(LPCTSTR lpColName)
		{
			LPCTSTR szValue = GetValue(lpColName);
			if ( !szValue )
			{
				return 0;
			}

			return boost::lexical_cast<T>( szValue );
		}
		...
		//! Jgkim
		template< typename T >
		T GetValue(int iColIndex)
		{
			try 
			{
				LPCTSTR szValue = GetValue(iColIndex);
				if ( szValue )
				{
					return boost::lexical_cast<T>( szValue );
				}
				...
				{
					sc::writeLogError( "T GetValue(int iColIndex) : NULL" );
					return 0;
				}
			...
			catch (boost::bad_lexical_cast &e) 
			{
				...
				if ( szValue )
				{
					sc::writeLogError( sc::string::format( "%1% : %2%", e.what(), szValue ) );
				}
				else
				{
					sc::writeLogError( sc::string::format( "%1%", e.what() ) );
				}
				return 0;
			} 
		}
		...
		bool GetValue(int iColIndex)
		{
			...
			int nValue = boost::lexical_cast<int>( szValue );
			if ( 0 == nValue )
				return false;
			else
				return true;
