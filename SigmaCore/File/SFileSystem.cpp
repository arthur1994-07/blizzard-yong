...
BOOL SFileSystem::OpenFileSystem(const char* strPath)
{
	...
	m_pFile = _fsopen(strPath,"r+b",_SH_DENYNO);
...
{
	FILE* fp = _fsopen(strPath,"w+b",_SH_DENYNO);
	if(!fp) return FALSE;
...
BOOL SFileSystem::AddFile(const char* strPath)
{
	...
	FILE* fp = _fsopen(strPath,"rb",_SH_DENYNO);
	if(!fp) return FALSE;
...
BOOL SFileSystem::ChangeDir(const char* strPath)
{
    ...
    char* pNextToken;
	SFileHandle FH;
	for(pTok=strtok_s( strTemp, "\\/\x0\n\r\t", &pNextToken ); pTok; pTok=strtok_s( NULL, "\\/\x0\n\r\t", &pNextToken ) )
	{
...
void SFileSystem::GetPathOnly(char* strDst,const char* strPath)
{
	...
	else
	{
        ...
        strncpy_s(strDst, _MAX_FNAME, strPath, iLen);
		strDst[iLen] = '\0';
	}
}