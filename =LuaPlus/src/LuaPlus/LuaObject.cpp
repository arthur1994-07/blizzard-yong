...
__int64 LuaObject::GetInteger64() const // 2008-03-21, Add Jgkim
{
	luaplus_assert(m_state  &&  IsInteger());
	return (__int64)(unsigned __int64)nvalue(&m_object);
}
...
#endif /* LUA_WIDESTRING */
	{
		return (int) tsvalue(&m_object)->len;
	}
	else if (IsUserData())
	{
		return (int) uvalue(&m_object)->len;
	}
	else
	{
		luaplus_assert(0);
...
{
  return (int) (state->GetCState()->top - state->GetCState()->base);
}
...
int LuaObject::GetCount()
{
	luaplus_assert(m_state);
	Push();
	int count = (int) lua_objlen(LuaState_to_lua_State(m_state), -1);
	m_state->Pop();
	return count;
}
