    ...
    public class Button extends UIComponent
    {
        ...
        override public function updateText() : void
        {
            ...
            if (this._label != null && this.textField != null)
            {
                ...
                if (this.preventAutoSize == true)
                {
                    TextFieldEx.setVerticalAlign(this.textField, TextFieldEx.VALIGN_CENTER);
                    _loc_1 = constraints.getElement("textField");
                    if (_loc_1 != null)
                    {
                        this.textField.height = this.height - _loc_1.top - _loc_1.bottom;
                    }
                }
                if (this.autoSize == TextFieldAutoSize.NONE)
                {
                    _loc_2 = this.textField.width * this.scaleX;
                    if (_loc_2 < this.textField.textWidth)
                    {
                        TooltipHtmlText = this._label;
                        _loc_3 = 0;
                        while (_loc_3 < this._label.length)
                        {
                            
                            this.textField.text = this._label.substr(0, this._label.length - _loc_3) + "...";
                            if (_loc_2 - 4 > this.textField.textWidth)
                            {
                                break;
                            }
                            _loc_3++;
