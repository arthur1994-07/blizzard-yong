...
CCursorTargetInfo::CCursorTargetInfo( GLGaeaClient* pGaeaClient, OldInterface* pInterface, EngineDeviceMan* pEngineDevice )
...
void CCursorTargetInfo::AddHp( DWORD nColor )
{
	...
	GLCrowClient* pCrow = dynamic_cast<GLCrowClient*>(m_pActor);
	if( pCrow->m_pCrowData->m_emNPCType == NPC_TYPE_AUTHENTICATOR )
	{
		dwNowHp = (pCrow->m_arrHP4School[ENUM_SCHOOL(m_pGaeaClient->GetCharacter()->m_wSchool)]);
		fPercent = static_cast< FLOAT >(pCrow->m_arrHP4School[ENUM_SCHOOL(m_pGaeaClient->GetCharacter()->m_wSchool)]);
		fPercent /= static_cast< FLOAT >( m_pActor->GetHp().dwMax );
		fPercent *= 100.0f;
	}
	else
	{
		dwNowHp = m_pActor->GetHp().dwNow;
		fPercent = static_cast< FLOAT >( m_pActor->GetHp().dwNow );
		fPercent /= static_cast< FLOAT >( m_pActor->GetHp().dwMax );
		fPercent *= 100.0f;
	}
