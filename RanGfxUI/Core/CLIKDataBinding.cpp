...
#include "../../RanThirdParty/GFx/Src/Kernel/SF_HeapNew.h"
#include "../../RanThirdParty/GFx/Src/Kernel/SF_Debug.h"
#include "../../RanThirdParty/GFx/Src/Kernel/SF_MsgFormat.h"
...
{
    HandlerDescriptors.push_back(desc);
...
{
	DescArray_iter iter = HandlerDescriptors.begin();
    for( ;  iter != HandlerDescriptors.end(); ++iter )
    {
		const CLIKDataBindingHandlerDesc& checkDesc = *iter;
        if( checkDesc == desc)
        {
            Data->OnRemoveHandler(binding, desc);
			HandlerDescriptors.erase( iter );
            break;
        }
    }
}
...
void    CLIKDataBindingAdapter::Binding::NotifyDataChange(GFx::Movie* pmovie)
{
    ...
    for (unsigned i = 0; i < HandlerDescriptors.size(); i++)
    {
...
void CLIKDataBindingAdapter::GetDataWrapperType( EMGFX_SLOT_TYPE emType, VEC_BIND_SLOT& vecBindSlot )
{
	Hash< String, Binding, String::HashFunctor >::Iterator iter = Bindings.Begin();
	for( ; iter != Bindings.End(); ++iter )
	{
		Binding& binding = iter->Second;

		GfxSlotBase* pSlot = (GfxSlotBase*) binding.GetDataWrapper();
		if( pSlot->GetType() != emType && pSlot->GetBaseType() != emType )
			continue;

		SBindingSlot bindSlot( pSlot );
		for( unsigned i = 0; i < binding.HandlerDescriptors.size(); ++i )
		{
			CLIKDataBindingHandlerDesc& desc = binding.HandlerDescriptors[ i ];
			bindSlot.vecBind.push_back( &desc.BoundObj );
		}

		vecBindSlot.push_back( bindSlot );
	}
}
