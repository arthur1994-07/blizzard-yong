...
{
	if(iDataLength < 1)
        throw std::runtime_error(std::string("FileCrypt ERROR: in CSHA::AddData(), Data Length should be > 0!"));
	unsigned int uiT;
	//Update bitcount
	uiT = m_auiBits[0];
...
void CSHA::FinalDigest(char* pcDigest)
{
	...
	if(false == m_bAddData)
        throw std::runtime_error(std::string("FileCrypt ERROR: in CSHA::FinalDigest(), No data Added before call!"));
