...
const char* CItemSuitTree::GetFormatName(SITEM* pItem)
{
	...
	const EMITEMLEVEL emItemLevel = pItem->GetItemLevel();
	std::string strItemLevel = sc::string::format(
		"%1%",
		ID2GAMEWORD( "ITEM_INFO_LEVEL", static_cast< int >( emItemLevel ) + 1 ) );

	if ( pItem->sBasicOp.dwReqCharClass==GLCC_NONE)
	{
		strItem.Format ( "[%04d/%02d]  %s - %s  [{%s}]",
...
BOOL CItemSuitTree::AddItemMain( HTREEITEM hMainItem )
{
	...
	GLStringTable::GetInstance().InsertString(sItemNode.m_sItem.sBasicOp.strName, sItemNode.m_sItem.sBasicOp.strName, GLStringTable::ITEM);
	GLStringTable::GetInstance().InsertString(sItemNode.m_sItem.sBasicOp.strComment, sItemNode.m_sItem.sBasicOp.strComment, GLStringTable::ITEM);
...
*/
	...
	int  itemType;
    std::string strItemName = szItemName;
