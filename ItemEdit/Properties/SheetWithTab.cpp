...
void CsheetWithTab::ExportExcelDetail( boost::function<void (int)> PercentCallbackFunc )
{
	...
	if ( m_ItemSuitTreePage.ExportExcelDetail_CheckItem(
		strSavedFileName,
		PercentCallbackFunc ) )
	{
		std::string Msg(
...
void CsheetWithTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	...
	if (nFlag != CDF_NONE)
	{
		...
		newClientRect.right = (LONG)(clientRect.right * fWidth);//tmNew.tmAveCharWidth / tmOld.tmAveCharWidth;
		newClientRect.bottom = (LONG)(clientRect.bottom * fHeigth); //newHeight / oldHeight;
	...
	while (pChildWnd)
	{
		...
		windowRect.left = (LONG)(windowRect.left * fWidth); //tmNew.tmAveCharWidth / tmOld.tmAveCharWidth;
		windowRect.right = (LONG)(windowRect.right * fWidth); //tmNew.tmAveCharWidth / tmOld.tmAveCharWidth;
		windowRect.top = (LONG)(windowRect.top * fHeigth); //newHeight / oldHeight;
		windowRect.bottom = (LONG)(windowRect.bottom *fHeigth); //newHeight / oldHeight;
