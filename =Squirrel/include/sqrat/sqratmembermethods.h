...
namespace Sqrat 
{
	...
	inline SQInteger sqVarGet(HSQUIRRELVM vm) {
		...
		if (SQ_FAILED( sq_get(vm,-2) ))
		{
			...
			const SQChar* funcName = sq_objtostring(&obj);
			return sq_throwerror(vm, (SQRAT_VAR_GET_CLASS_NAME() + string(_SC("::")) + string(funcName) + _SC(" is not Member")).c_str() );
		}
