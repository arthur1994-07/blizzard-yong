...
int TTabWnd::drawSelTabTop(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
	...
	// gray inside
	pDC->FillSolidRect(x+3, 
		               y+3,textSize.cx+TAB_SPACE-6, 
		               TABSEL_HEIGHT, 
	...
	for (iterator = m_TabList.begin(); iterator != m_TabList.end(); iterator++) 
	{
		...
		if (pItem->m_bVisible) 
		{
			...
			if (nIndex != m_nSelectedTab) 
			{
				...
				{
				case TP_TOP: 
					x += drawTabTop(&dc, x, client, pItem); 
					break;
				case TP_BOTTOM: x += drawTabBottom(&dc, x, client, pItem); 
					break;
				}
			} 
			else 
			{
				switch (m_nTabPos) 
				{
				case TP_TOP: x += drawSelTabTop(&dc, x, client, pItem); 
					break;
				case TP_BOTTOM: x += drawSelTabBottom(&dc, x, client, pItem); 
					break;
				}
			}
		}
                                 ...
                                 CCreateContext *pContext)
{
	...
	if (!pWnd->Create(NULL, NULL, dwStyle, rect, this, 13576+ static_cast<UINT> (m_TabList.size()), pContext))
	{
...
TTabItem *TTabWnd::CreatePane(LPCTSTR lpszLabel, 
							  int nRows, 
							  int nCols, 
                              CWnd *pWnd, 
							  UINT nID)
...
BOOL TVisualFramework::execCreateTabView(CWnd *pWnd, TVisualObject *pObject)
{
	...
	TTabWnd *pTab = (TTabWnd*) pWnd;
	TTabItem *pItem;
	pItem = pTab->CreatePane(pObject->m_strTitle, 
		                     pObject->m_pRuntimeClass, 
							 pObject->m_pContext);
...
BOOL TVisualFramework::execCreateSplitter(CWnd *pWnd, TVisualObject *pObject)
{
	...
	{
		TTabWnd *pTab = (TTabWnd*)pWnd;
		TTabItem *pItem = pTab->CreatePane(pObject->m_strTitle, 
			                               pObject->m_nRows, 
									       pObject->m_nCols, 
										   pObject->m_pWnd);
		pItem->SetIcon(pObject->GetIcon());
...
BOOL TVisualFramework::execCreateSplitterView(CWnd *pWnd, TVisualObject *pObject)
{
	...
	CSplitterWnd *pSplitter = (CSplitterWnd*)pWnd;
	pSplitter->CreateView(pObject->m_nRowIndex, 
		                  pObject->m_nColIndex, 
						  pObject->m_pRuntimeClass, 
						  pObject->m_Size, 
						  pObject->m_pContext);
...
BOOL TVisualFramework::execCreateSplitterSplitter(CWnd *pWnd, TVisualObject *pObject)
{
	...
	pSplitter->CreateStatic(pParent, 
		                    pObject->m_nRows, 
							pObject->m_nCols, 
