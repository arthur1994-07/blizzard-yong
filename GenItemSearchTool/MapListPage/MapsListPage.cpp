...
BOOL CMapsListPage::OnInitDialog()
{
	...
	const int nColumnCount = 31;	
	char* szColumnName1[nColumnCount] = { 
		"Mid,Sid", "MapName", "Used", "FileName", "FieldServer",
		"Bgm", "LoadingImg", "Piece", "CommissionFree", "PKEnable",
		"FreePK", "ItemDrop", "FriendCard", "Resurrection Forbid", "PatActivity",
		"ExpDrop", "VehicleActivity", "ClubBattleZone", "InstantMap", "QBoxDrop",
		"LunchBox Forbid", "CPRest", "PKMap", "UIMapSelect", "UIMapInfo",
        "PartyMap", "ExpeditionMap", "PrivateMarketOpenalbe", "Sparring", "Cull By Object Map",
		"Rebirth Block"
	};
...
void CMapsListPage::Init()
{
    ...
    m_sMapsList.LoadMapList(GLogicData::GetInstance().GetMapListFileName(), TRUE); // "mapslist.mst"
	UpdateItems();
...
bool CMapsListPage::LoadFile()
{
	...
	CFileDialog dlg(TRUE,"(*.lua;*.mst)|*.lua;*.mst|",NULL,OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
bool CMapsListPage::SaveFile()
{
	...
	//	Note : 파일 오픈 다이알로그를 만듬.
	CFileDialog dlg(FALSE,".mst|.lua",NULL,OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
bool CMapsListPage::LoadCsv()
{
	...
	//	Note : 파일 오픈 다이알로그를 만듬.
	CFileDialog dlg(TRUE,".csv",NULL,OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
