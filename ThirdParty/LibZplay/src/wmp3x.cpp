...
// NOW WAIT THREAD TO END
	...
	// [shhan][2016.08.24] windows10 의 특정 컴퓨터에서 프리징 걸린다.
	//WaitForSingleObject(w_hThread, INFINITE);

	// be sure that playing thread is ended
	DWORD dwReturn = WaitForSingleObject(w_hThread, 1000);
	if ( dwReturn == WAIT_TIMEOUT )
	{
		// [shhan][2016.08.24] 1초 기다려도 응답이 없다면.. 문제 있음.
...
// ==== CREATE PLAYING THREAD =========
	...
	DWORD dwReturn = WaitForSingleObject(c_hThreadStartEvent, 1000);
	if ( dwReturn == WAIT_TIMEOUT )
	{
		// [shhan][2016.08.24] 1초 기다려도 응답이 없다면.. 문제 있음.
		err(WMP3X_UNKNOWN_ERROR);
