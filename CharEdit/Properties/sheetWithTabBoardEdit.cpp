...
IMPLEMENT_DYNAMIC(CsheetWithTabBoardEdit, CPropertySheet)


CsheetWithTabBoardEdit::CsheetWithTabBoardEdit ( CWnd* pWndParent )
	...
	m_pSkinAniContrl(NULL)
{
	...
	m_BoardEditPage.SetSheetTab ( this );
	AddPage(&m_BoardEditPage);

}


CsheetWithTabBoardEdit::CsheetWithTabBoardEdit ( CRect& rect, CWnd* pWndParent )
			: CPropertySheet(IDS_PROPSHT_CAPTION, pWndParent),
			m_bNoTabs(FALSE),
			m_pWndParent(NULL),
			m_iDistanceFromTop(0)
...
CsheetWithTabBoardEdit::~CsheetWithTabBoardEdit()
{
}

static int module_piButtons[] = { IDOK, IDCANCEL, ID_APPLY_NOW, IDHELP };

BOOL CsheetWithTabBoardEdit::OnInitDialog() 
{
	HWND hTabWnd;
	CRect rectTabCtrl;
...
BEGIN_MESSAGE_MAP(CsheetWithTabBoardEdit, CPropertySheet)
	//{{AFX_MSG_MAP(CsheetWithTabBoardEdit)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()



void CsheetWithTabBoardEdit::SetPages( DxSkinAniControl* pSkinAniContrl )
{
	m_pSkinAniContrl = pSkinAniContrl;

	SetActivePage (0);
	m_BoardEditPage.ResetPage ();
	m_BoardEditPage.UpdatePassengerDataList();
}

void CsheetWithTabBoardEdit::ReSetPages ()
{
	m_pSkinAniContrl = NULL;
...
void CsheetWithTabBoardEdit::SetEditView( const bool bView )
{
	if( bView )
	{	
