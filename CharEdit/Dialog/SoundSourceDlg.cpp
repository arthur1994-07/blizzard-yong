...
BOOL CSoundSourceDlg::OnInitDialog()
{
	...
	{
		nIndex = m_ctrlCategory.AddString ( NTypeDesc::Desc[i] );
...
void CSoundSourceDlg::OnBnClickedOk()
{
	...
	pComboBox = (CComboBox*) GetDlgItem ( IDC_COMBO_PLAYTYPE );
	m_PlayType = (WORD)pComboBox->GetCurSel ();

	//	루프
	CButton* pButton = NULL;
	pButton = (CButton*) GetDlgItem ( IDC_CHECK_LOOP );
	m_bLoop = pButton->GetCheck ();

	OnOK();
}

void	CSoundSourceDlg::GetSelectSoundSource ( SSoundSource& rSoundSource, WORD& rPlayType, BOOL& rbLoop )
{
	rSoundSource = m_SoundSource;
	rPlayType = m_PlayType;
	rbLoop = m_bLoop;
}
...
void CSoundSourceDlg::OnNMDblclkListSoundsource(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	m_pSound = DxSoundMan::GetInstance().LoadWav( m_SoundSource.FileName.GetString(), false );
	if ( !m_pSound )
	{
		CString strTemp;
