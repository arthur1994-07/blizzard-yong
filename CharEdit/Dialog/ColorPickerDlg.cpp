...
BOOL CColorPickerDlg::Create(UINT nIDTemplate, CWnd* pParentWnd /* = NULL */ )
{
	m_bModeless = TRUE;
	g_iColorResult = -1;
	m_nIndex = -1;
...
void CColorPickerDlg::PostNcDestroy()
{
	if ( m_bModeless )
	{
		delete this;
...
void CColorPickerDlg::OnDeltaposSpinRed(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;

	g_iClickSpinColor = SPIN_RED;

	m_fColorHeight = (float)color.r;
}

void CColorPickerDlg::OnDeltaposSpinGreen(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;

	g_iClickSpinColor = SPIN_GREEN;

	m_fColorHeight = (float)color.g;
}

void CColorPickerDlg::OnDeltaposSpinBlue(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;

	g_iClickSpinColor = SPIN_BLUE;

	m_fColorHeight = (float)color.b;
}

void CColorPickerDlg::OnDeltaposSpinHue(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;

	g_iClickSpinColor = SPIN_HUE;

	m_fColorHeight = (float)color.toHSV().h;
}

void CColorPickerDlg::OnDeltaposSpinSat(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;

	g_iClickSpinColor = SPIN_SAT;

	m_fColorHeight = (float)color.toHSV().s;
}

void CColorPickerDlg::OnDeltaposSpinVal(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;

	g_iClickSpinColor = SPIN_VAL;

	m_fColorHeight = (float)color.toHSV().v;
}

void CColorPickerDlg::SetSpinControl( int nColorHeight )
{
	switch ( g_iClickSpinColor )
	{
	case SPIN_RED:
		{
			m_fColorHeight -= (float)nColorHeight;

			SetDlgItemInt( IDC_EDIT_RED, (int)m_fColorHeight );
		}
		break;
	...
	case SPIN_GREEN:
		{
			...
			SetDlgItemInt( IDC_EDIT_GREEN, (int)m_fColorHeight );
		}
		break;
	...
	case SPIN_BLUE:
		{
			...
			SetDlgItemInt( IDC_EDIT_BLUE, (int)m_fColorHeight );
		}
		break;
	...
	case SPIN_HUE:
		{
			...
			SetDlgItemInt( IDC_EDIT_HUE, (int)m_fColorHeight );
		}
		break;
	...
	case SPIN_SAT:
		{
			...
			SetDlgItemInt( IDC_EDIT_SAT, (int)m_fColorHeight );
		}
		break;
	...
	case SPIN_VAL:
		{
			...
			SetDlgItemInt( IDC_EDIT_VAL, (int)m_fColorHeight );
		}
		break;
	}
}