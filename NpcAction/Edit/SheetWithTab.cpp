...
void CsheetWithTab::ActiveActionPage(std::tr1::shared_ptr<CNpcDialogue> spNpcDialogue)
{
	CNpcActionDlg* pDlg = (CNpcActionDlg*)m_pWndParent;
	...
	m_NpcActionPage.SetDialogue ( m_NpcActionTable.GetDialogueSet(), spNpcDialogue );
	m_NpcActionPage.UpdateItems ();
}

void CsheetWithTab::ActiveActionCondition(int nCallPage, std::tr1::shared_ptr<SNpcTalkCondition> spConditon)
{
	SetActivePage ( NPCACTIONCONDITION );

	m_NpcActionCondition.SetCallPage(nCallPage);
	m_NpcActionCondition.SetCondition(spConditon);
	m_NpcActionCondition.UpdateItems();
}

void CsheetWithTab::ActiveActionAnswer(std::tr1::shared_ptr<CNpcDialogueSet> spNpcDialogueSet, std::tr1::shared_ptr<CNpcDialogueCase> spDialogueCase, const DWORD dwItemIndex )
{
	SetActivePage ( NPCACTIONANSWER );

	if ( spNpcDialogueSet || spDialogueCase )
		m_NpcActionAnswer.SetDialogueCase ( spNpcDialogueSet, spDialogueCase, dwItemIndex );
	m_NpcActionAnswer.UpdateItems ();
}

void CsheetWithTab::ActiveActionTalk(
    int nCallPage,
    ...
    std::tr1::shared_ptr<SNpcTalk> spTalk)
{
	...
	m_NpcActionTalk.SetModifyData ( bModify, spTalk );
	m_NpcActionTalk.UpdateItems();
}

spNpcDialogSet CsheetWithTab::GetDialogueSet()
{
