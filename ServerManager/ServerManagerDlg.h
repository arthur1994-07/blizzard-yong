...
public:	
	...
	int					m_nReconnectionCount;
    InspectionRewardPolicy*	m_pPolicyDlg;

	std::vector<STracingData> m_vecTracingData;

    CButton m_AutoEventButton;
    bool m_bAutoEventRun;
    EventMan* m_pEvent;
...
public:
    void SetAutoEventButton(bool Running);

    //! Lua command bind function
public:
    void Notice(int ServerGroup, const char* Notice);
    void ExpUp(int ServerGroup, float Scale);
    void ExpUpEnd(int ServerGroup);
...
public:	
	...
	void	SendAuthServerSMS(NET_MSG_GENERIC* nmg);
...
public:
    ...
    //afx_msg void OnBnClickedButtonCrashTest();
    
    afx_msg void OnBnClickedButtonAutoEvent();
protected:
    virtual LRESULT WindowProc(UINT message, WPARAM wParam, LPARAM lParam);
    virtual LRESULT OnUpdateLogConsole(WPARAM wParam, LPARAM lParam);
public:
	afx_msg void OnEnChangeTracingTimeEdit();
	CEdit m_EditTracingTime;
    afx_msg void OnBnClickedButtonOpenInspeDialog();
};
