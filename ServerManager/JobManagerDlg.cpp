...
void CJobManagerDlg::OnBnClickedSchExport()
{
	...
	if ( dlg.DoModal() == IDOK )
	{
		FILE *pFile = _fsopen( dlg.GetFileName().GetString(), "wb", _SH_DENYNO );
		if( pFile == NULL )
		{
		...
		for( i = 0; i < vecJob.size(); i++ )
		{
			...
			{
				fwrite( &vecGroupNum[j], sizeof(int), 1, pFile );
			}
		}
...
void CJobManagerDlg::OnBnClickedSchImport()
{
	...
	if ( dlg.DoModal() == IDOK )
	{
		...
		FILE *pFile = _fsopen( dlg.GetFileName().GetString(), "rb", _SH_DENYNO );
		if( pFile == NULL )
		{
