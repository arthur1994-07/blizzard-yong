...
int CServerManagerCfg::Load()
{
	...
	// Open for read 
	if ((oFile = _fsopen(strFileName.GetString(), "r",_SH_DENYNO)) == NULL)
	{
		return 0;
	}	
	...
	{	
		...
		else if (strcmp(token, "session_ip") == 0) 
		{
			...
			if (token)
			{
                ...
                sc::writeLogInfo(
                    sc::string::format(
                        "Session IP:%1%", m_strSessionIP.GetString()));
			}
			else
			{
				return 0;
		...
		{			
			while (token = strtok(NULL, seps))
			{
                ...
                /*
				CClientConsoleMessage::GetInstance()->WriteWithTime(
                    "SMS Phone %d:%s",
                    m_nSmsPhoneCount,
                ...
                */
                sc::writeLogInfo(
                    sc::string::format(
                        "SMS Phone %1%:%2%",
                        m_nSmsPhoneCount,
                        m_SmsPhoneList[m_nSmsPhoneCount].GetString()));
				m_nSmsPhoneCount++;
			}
			return 0;
		}
