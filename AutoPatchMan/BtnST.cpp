...
CButtonST::CButtonST()
{
	...
	SetPressedStyle(BTNST_NONPRESS, FALSE);
...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		...
		if (m_csIcons[CONTROL_IMAGE_IN].hIcon)	::DestroyIcon(m_csIcons[CONTROL_IMAGE_IN].hIcon);
		if (m_csIcons[CONTROL_IMAGE_OUT].hIcon)	::DestroyIcon(m_csIcons[CONTROL_IMAGE_OUT].hIcon);
		if (m_csIcons[CONTROL_IMAGE_PRESS].hIcon)	::DestroyIcon(m_csIcons[CONTROL_IMAGE_PRESS].hIcon);
		if (m_csIcons[CONTROL_IMAGE_DIS].hIcon)	::DestroyIcon(m_csIcons[CONTROL_IMAGE_DIS].hIcon);

		// Destroy bitmaps
		if (m_csBitmaps[CONTROL_IMAGE_IN].hBitmap)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_IN].hBitmap);
		if (m_csBitmaps[CONTROL_IMAGE_OUT].hBitmap)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_OUT].hBitmap);
		if (m_csBitmaps[CONTROL_IMAGE_PRESS].hBitmap)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_PRESS].hBitmap);
		if (m_csBitmaps[CONTROL_IMAGE_DIS].hBitmap)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_DIS].hBitmap);

		// Destroy mask bitmaps
		if (m_csBitmaps[CONTROL_IMAGE_IN].hMask)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_IN].hMask);
		if (m_csBitmaps[CONTROL_IMAGE_OUT].hMask)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_OUT].hMask);
		if (m_csBitmaps[CONTROL_IMAGE_PRESS].hMask)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_PRESS].hMask);
		if (m_csBitmaps[CONTROL_IMAGE_DIS].hMask)	::DeleteObject(m_csBitmaps[CONTROL_IMAGE_DIS].hMask);
	} // if
...
void CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ( m_bIsCheckBox )
	{
		if ( bIsDisabled )
		{
			byIndex = CONTROL_IMAGE_DIS;
		}
		else if ( bIsPressed ) 
		{
			byIndex = CONTROL_IMAGE_IN;
		}
		else 
		{
			byIndex = CONTROL_IMAGE_OUT; 
		}		
	}
	else
	{
	...
	if ( m_csIcons[byIndex].hIcon == NULL ) byIndex = CONTROL_IMAGE_IN;
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ( m_csBitmaps[byIndex].hBitmap == NULL ) byIndex = CONTROL_IMAGE_IN;
...
DWORD CButtonST::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut,
						 int nIconDown, int nCxDesiredDown, int nCyDesiredDown, int nIconDis, int nCxDesiredDis, int nCyDesiredDis )
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
	HICON		hIconDown		= NULL;
	HICON		hIconDis		= NULL;
	...
	if ( nIconDown ) 
	{
		hIconDown = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconDown), IMAGE_ICON, nCxDesiredDown, nCyDesiredDown, 0);
	}

	if ( nIconDis ) 
	{
		hIconDis = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconDis), IMAGE_ICON, nCxDesiredDis, nCyDesiredDis, 0);
	}

	return SetIcon(hIconIn, hIconOut, hIconDown, hIconDis);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(int nIconIn, int nIconOut, int nIconDown, int nIconDis)
{
	return SetIcon(nIconIn, 0, 0, nIconOut, 0, 0, nIconDown, 0, 0, nIconDis, 0, 0);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut, HICON hIconDown, HICON hIconDis)
{
	BOOL		bRetValue;
	ICONINFO	ii;
	...
	if (hIconIn)
	{
		...
		m_csIcons[CONTROL_IMAGE_IN].hIcon = hIconIn;
		// Get icon dimension
		...
		m_csIcons[CONTROL_IMAGE_IN].dwWidth	= (DWORD)(ii.xHotspot * 2);
		m_csIcons[CONTROL_IMAGE_IN].dwHeight	= (DWORD)(ii.yHotspot * 2);
		...
		if (hIconOut)
		{
			...
			m_csIcons[CONTROL_IMAGE_OUT].hIcon = hIconOut;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			...
			m_csIcons[CONTROL_IMAGE_OUT].dwWidth	= (DWORD)(ii.xHotspot * 2);
			m_csIcons[CONTROL_IMAGE_OUT].dwHeight	= (DWORD)(ii.yHotspot * 2);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		} // if
		...
		if ( hIconDown )
		{
			m_csIcons[CONTROL_IMAGE_PRESS].hIcon = hIconDown;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			bRetValue = ::GetIconInfo(hIconDown, &ii);
			if (bRetValue == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			m_csIcons[CONTROL_IMAGE_PRESS].dwWidth	= (DWORD)(ii.xHotspot * 2);
			m_csIcons[CONTROL_IMAGE_PRESS].dwHeight	= (DWORD)(ii.yHotspot * 2);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		}
		...
		if ( hIconDis )
		{
			m_csIcons[CONTROL_IMAGE_DIS].hIcon = hIconDis;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			bRetValue = ::GetIconInfo(hIconDis, &ii);
			if (bRetValue == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			m_csIcons[CONTROL_IMAGE_DIS].dwWidth	= (DWORD)(ii.xHotspot * 2);
			m_csIcons[CONTROL_IMAGE_DIS].dwHeight	= (DWORD)(ii.yHotspot * 2);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		}
	} // if
...
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut,
							int nBitmapDown, COLORREF crTransColorDown, int nBitmapDis, COLORREF crTransColorDis )
{
	...
	return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut, hBitmapDown, crTransColorDown, hBitmapDis, crTransColorDis);
} // End of SetBitmaps

DWORD CButtonST::SetBitmaps(int nBitmapIn, int nBitmapOut, int nBitmapDown, int nBitmapDis )
{
	return SetBitmaps( nBitmapIn, RGB( 255,0,255 ), nBitmapOut, RGB( 255,0,255 ), nBitmapDown, RGB( 255,0,255 ), nBitmapDis, RGB( 255,0,255 ) );
}
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut,
							HBITMAP hBitmapDown, COLORREF crTransColorDown, HBITMAP hBitmapDis, COLORREF crTransColorDis )
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		m_csBitmaps[CONTROL_IMAGE_IN].hBitmap = hBitmapIn;
		m_csBitmaps[CONTROL_IMAGE_IN].crTransparent = crTransColorIn;
		// Get bitmap size
		...
		m_csBitmaps[CONTROL_IMAGE_IN].dwWidth = (DWORD)csBitmapSize.bmWidth;
		m_csBitmaps[CONTROL_IMAGE_IN].dwHeight = (DWORD)csBitmapSize.bmHeight;

		// Create grayscale/darker bitmap BEFORE mask (of hBitmapIn)
		switch ((int)hBitmapOut)
		{
			case (int)BTNST_AUTO_GRAY:
				hBitmapOut = CreateGrayscaleBitmap(hBitmapIn, m_csBitmaps[CONTROL_IMAGE_IN].dwWidth, m_csBitmaps[CONTROL_IMAGE_IN].dwHeight, crTransColorIn);
				m_csBitmaps[CONTROL_IMAGE_OUT].hBitmap = hBitmapOut;
				crTransColorOut = crTransColorIn;
				break;
			case (int)BTNST_AUTO_DARKER:
				hBitmapOut = CreateDarkerBitmap(hBitmapIn, m_csBitmaps[CONTROL_IMAGE_IN].dwWidth, m_csBitmaps[CONTROL_IMAGE_IN].dwHeight, crTransColorIn);
				m_csBitmaps[CONTROL_IMAGE_OUT].hBitmap = hBitmapOut;
				crTransColorOut = crTransColorIn;
				break;
		} // switch
		...
		m_csBitmaps[CONTROL_IMAGE_IN].hMask = CreateBitmapMask(hBitmapIn, m_csBitmaps[CONTROL_IMAGE_IN].dwWidth, m_csBitmaps[CONTROL_IMAGE_IN].dwHeight, crTransColorIn);
		if (m_csBitmaps[CONTROL_IMAGE_IN].hMask == NULL)
		{
			FreeResources();
		...
		{
			m_csBitmaps[CONTROL_IMAGE_OUT].hBitmap = hBitmapOut;
			m_csBitmaps[CONTROL_IMAGE_OUT].crTransparent = crTransColorOut;
			// Get bitmap size
			...
			m_csBitmaps[CONTROL_IMAGE_OUT].dwWidth = (DWORD)csBitmapSize.bmWidth;
			m_csBitmaps[CONTROL_IMAGE_OUT].dwHeight = (DWORD)csBitmapSize.bmHeight;

			// Create mask for bitmap Out
			m_csBitmaps[CONTROL_IMAGE_OUT].hMask = CreateBitmapMask(hBitmapOut, m_csBitmaps[CONTROL_IMAGE_OUT].dwWidth, 
																	m_csBitmaps[CONTROL_IMAGE_OUT].dwHeight, crTransColorOut);
			if (m_csBitmaps[CONTROL_IMAGE_OUT].hMask == NULL)
			{
				FreeResources();
				return BTNST_FAILEDMASK;
			} // if
		...
		if ( hBitmapDown )
		{
			m_csBitmaps[CONTROL_IMAGE_PRESS].hBitmap = hBitmapDown;
			m_csBitmaps[CONTROL_IMAGE_PRESS].crTransparent = crTransColorDown;
			// Get bitmap size
			...
			m_csBitmaps[CONTROL_IMAGE_PRESS].dwWidth = (DWORD)csBitmapSize.bmWidth;
			m_csBitmaps[CONTROL_IMAGE_PRESS].dwHeight = (DWORD)csBitmapSize.bmHeight;

			// Create mask for bitmap Out
			m_csBitmaps[CONTROL_IMAGE_PRESS].hMask = CreateBitmapMask(hBitmapDown, m_csBitmaps[CONTROL_IMAGE_PRESS].dwWidth, 
																	 m_csBitmaps[CONTROL_IMAGE_PRESS].dwHeight, crTransColorDown);
			if (m_csBitmaps[CONTROL_IMAGE_PRESS].hMask == NULL)
			{
				FreeResources();
				return BTNST_FAILEDMASK;
			} // if
		...
		if ( hBitmapDis )
		{
			m_csBitmaps[CONTROL_IMAGE_DIS].hBitmap = hBitmapDis;
			m_csBitmaps[CONTROL_IMAGE_DIS].crTransparent = crTransColorDis;
			// Get bitmap size
			...
			m_csBitmaps[CONTROL_IMAGE_DIS].dwWidth = (DWORD)csBitmapSize.bmWidth;
			m_csBitmaps[CONTROL_IMAGE_DIS].dwHeight = (DWORD)csBitmapSize.bmHeight;

			// Create mask for bitmap Out
			m_csBitmaps[CONTROL_IMAGE_DIS].hMask = CreateBitmapMask(hBitmapDis, m_csBitmaps[CONTROL_IMAGE_DIS].dwWidth, 
																	m_csBitmaps[CONTROL_IMAGE_DIS].dwHeight, crTransColorDis);
			if (m_csBitmaps[CONTROL_IMAGE_DIS].hMask == NULL)
			{
				FreeResources();
				return BTNST_FAILEDMASK;
			} // if
...
//	m_crColors[BTNST_COLOR_FG_FOCUS]	= ::GetSysColor(COLOR_BTNTEXT);
	...
	m_crColors[BTNST_COLOR_BK_IN]		= RGB( 192,192,192 );
	m_crColors[BTNST_COLOR_FG_IN]		= RGB( 192,192,192 );
	m_crColors[BTNST_COLOR_BK_OUT]		= RGB( 192,192,192 );
	m_crColors[BTNST_COLOR_FG_OUT]		= RGB( 192,192,192 );
	m_crColors[BTNST_COLOR_BK_FOCUS]	= RGB( 192,192,192 );
	m_crColors[BTNST_COLOR_FG_FOCUS]	= RGB( 192,192,192 );
