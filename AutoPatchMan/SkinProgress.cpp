...
{
	// A..... Create the progress bar using user's parameters
	ProgressInStatusBar ( i_poStrMessage, i_nUpper, i_nProgress, i_nPane, i_nSize );
}
...
{
	// A..... Create the progress bar using user's parameters
	ProgressInDialog ( i_poWndProgress, i_nUpper, i_poBitmap, i_poWndMessage, i_poStrMessage, i_nProgress );
}
...
BOOL CSkinProgress::CreateCommon ( LPCTSTR i_poStrMessage, int i_nUpper, int i_nProgress, CBitmap* i_poBitmap ) 
{
	// A..... Set base values
	m_poBaseBmp       = NULL;
...
)
{
	...
	if(poStatusBar != NULL)
	{
		...
		// Use object's unique address as child window identifier
		bSuccess   = CStatic::CreateEx(nStyleEx, poStrClass, "", nStyle, oRectPane, poStatusBar, (UINT) this); 
		...
		else
		{
			...
			// D..... Create and initialize the progress bar
			bSuccess = CreateCommon ( i_poStrMessage, i_nUpper, i_nProgress, i_poBitmap );
...
)
{
	...
	// Attach it not the the CWnd anchor (thus getting in its client area) but to its parent to overlap it
	// Use object's unique address as child window identifier
	bSuccess   = CStatic::CreateEx ( nStyleEx, poStrClass, "", nStyle, oRectPane, m_poWndProgress->GetParent(), (UINT)this );
...
BOOL CSkinProgress::GetTargetRect ( CRect* o_poProgressRect )
{
  CStatusBar* poStatusBar;
...
BOOL CSkinProgress::SetBitmap ( CBitmap* i_poBitmap )
{ 
	BITMAP      sBmpSrc;
	CBitmap*    poOldBitmap;
	...
	{ // If the target rectangle is valid
		...
		{ // If no bitmap provided, takes the default bitmap
			m_poBaseBmp->LoadBitmap(IDB_BITMAP_PROGRESS); // Defaut bitmap, might have to be changed !
		}
		else
		...
		m_poStretchBmp->CreateCompatibleBitmap(&oDC, sBmpSrc.bmWidth, oRectBar.Height()); // Minus 2, with don't count the contour of the box
		...
		// Copy the picture
		oDcStretch.SetStretchBltMode(COLORONCOLOR);
		oDcStretch.StretchBlt(0, 0, sBmpSrc.bmWidth, oRectBar.Height(), &oDcBitmap, 0, 0, sBmpSrc.bmWidth, sBmpSrc.bmHeight, SRCCOPY);
		...
		{
			// DE.... Add the stretched bitmap in the image list
			m_oBarImgLst.Add(m_poStretchBmp, RGB(0, 255, 0));
...
// Copy a bitmap
BOOL CSkinProgress::CopyBitmap ( CBitmap* o_poBitmap, CBitmap* i_poBitmap )
{ 
	BITMAP         sBmpSrc;
	...
	if( (o_poBitmap == NULL) || (i_poBitmap == NULL) )
	{
		return false;
	}
	else
...
BOOL CSkinProgress::ConvBitmap ( CBitmap* o_poBitmap, CBitmap* i_poBitmap, CDC* i_poDC )
{ 
	BITMAP         sBmpSrc;
	...
	if( (o_poBitmap != NULL) && (i_poBitmap != NULL) )
	{
		...
		else
		{
			...
			if(panDataBmp != NULL)
			{
				...
				// F..... Convert the data
				DIB = CreateDIBitmap ( i_poDC->m_hDC, &sBmpInfoSrc.bmiHeader, CBM_INIT, panDataBmp, &sBmpInfoSrc, DIB_RGB_COLORS );
...
)
{
	...
	if(IsWindow(GetSafeHwnd()))
	{
		...
		{ // Resample positions to readapt them to the new range
			// Recalculate end
			m_nRight = (int) ((((LONGLONG)(m_nRight - m_nLower) * (i_nUpper - i_nLower)) / (m_nUpper - m_nLower)) + i_nLower );
...
// Set <m_nRight> value
int CSkinProgress::SetPos ( int  i_nPos )
{
	if(IsWindow(GetSafeHwnd()))
	{
		ASSERT((-0x7FFFFFFF <= i_nPos) && (i_nPos <= 0x7FFFFFFF));

		if(i_nPos > m_nUpper)	i_nPos = m_nUpper;
		if(i_nPos < m_nLower)	i_nPos = m_nLower;
...
int CSkinProgress::OffsetPos ( int  i_nOffset )
{ 
	if(IsWindow(GetSafeHwnd()))
	{
		m_nPrevRight  = m_nRight;
		...
		if(m_nRight > m_nUpper)	m_nRight = m_nUpper;
		if(m_nRight < m_nLower)	m_nRight = m_nLower;
...
// Set <m_nStep> value
int CSkinProgress::SetStep ( int i_nStep )             
{ 
	int PrevStep;
...
BOOL CSkinProgress::SetSize ( int  i_nSize )
{
	// A..... Set the new size of the progress bar
	if(i_nSize > 0)
...
// Get <m_nRight> value
int CSkinProgress::GetPos ( BOOL i_bPercent )
{
	if(i_bPercent == false)
	{
		// A..... Return the current <m_nRight> position
	...
	else
	{
		...
		return (int)(((LONGLONG)(m_nRight - m_nLower) * 25600 ) / (m_nUpper - m_nLower));
	}
}
...
void CSkinProgress::Reset ()
{
	...
	m_nStartTimeLongLong = (((LONGLONG) sStartTimeFile.dwHighDateTime << 32 ) | sStartTimeFile.dwLowDateTime );
...
// Set the new text
BOOL CSkinProgress::SetText ( LPCTSTR i_poStrMessage )
{ 
	// A..... Set the new text
	m_oStrMessage = i_poStrMessage;
...
BOOL CSkinProgress::SetProgress ( int i_nProgress )
{
	// A..... Set the new progress text
	m_nProgressText = i_nProgress;
...
BOOL CSkinProgress::RefreshPanes () 
{
	...
	if(IsWindow(GetSafeHwnd()))
	{
		...
		if(GetTargetRect(&oRectPane))
		{
			...
			if(poTextCWnd != NULL)
			{
				...
				{ // Redraw the text window
					...
					{ // Calculate progress text
						// CA.... Get the length of the cSPT_PERCENT message
						oStrPercent.Format ( "%d%%", (int)(((LONGLONG)(m_nRight - m_nLower) * 100) / (m_nUpper - m_nLower)));
					...
					case cSPT_AUTOSIZE :
						...
						nLenCompact = CompactText(&oStrMessage, &oDC, nLenText, nLenMargin + nLenTimed);
						if( (nLenCompact != 0) && (nLenCompact != oStrMessage.GetLength()))
					...
					if(m_poWndProgress == NULL)
					{
						...
						if( (nLenCompact < 0) || ( (oStrMessage.GetLength() <= 4) && (oStrMessage.Right(3) == "...")))
						{
							// DF.... Indicate the text is completely compacted, 
					...
					if(m_poWndProgress == NULL)
					{
						...
						{ // If the text pane is shared with the progress bar, calculate how much space the text takes up to
							...
							if((((nLenMessage + nLenMargin) > (nLenPane)) && ((m_nSize <= 0) || ((nLenMessage > 0) &&
								((nLenBlock & cRTP_NOTEXT) != 0)))) || ((nLenBlock & cRTP_BLOCK) != 0))
							{ // *IF* the packed text is larger than the space left by the resized progress bar *AND* the resizing of the progress bar is allowed *OR* the wrapping is blocked
								...
								if(nLenMessage > 0)	nLenPane += nLenMargin;
							}
			...
			{
				// G..... Refresh the progress bar
				if( (m_oRectPane != oRectPane) && ((oRectPane.Width() > 2) && (oRectPane.Height() > 2)))
				{
					// GA.... Move the progress bar
...
int CSkinProgress::CompactText ( CString* io_poString, CDC* i_poDC, int i_nMaxWidth, int i_nAddWidth )
{
	int     nLenString;
	int     nLenDots;
	int     nCountChar;
	...
	if(io_poString != NULL)
	{
		...
		// C..... Verify there is something to do
		if((nLenString == 0) || ((i_poDC->GetTextExtent(*io_poString, nLenString).cx + i_nAddWidth) <= i_nMaxWidth ))
		{
			return 0;
		}
		else
		{
			...
			if( i_poDC->GetTextExtent(*io_poString, nLenString).cx <= (i_poDC->GetTextExtent(&nFirstChar, sizeof(nFirstChar)).cx + nLenDots))
			{
				return (0 - nLenString);
			}
			else
			{
				...
				// Leading character must remain at least
				while((nLenString > 1) && ((i_poDC->GetTextExtent(*io_poString, nLenString).cx + (nLenDots >> 1)+ 
					  i_nAddWidth) > i_nMaxWidth ))
				{
...
BOOL CSkinProgress::GetTimed ( CString* o_poTimed )
{
	CString       oStrTempo;
	...
	*o_poTimed = " ";
	...
	if( (sTempoTimeSystem.wDay   != m_sStartTimeSystem.wDay)   // Check what change first, days
      || (sTempoTimeSystem.wMonth != m_sStartTimeSystem.wMonth)
      || (sTempoTimeSystem.wYear  != m_sStartTimeSystem.wYear) )
	{ // Date of Start
		oStrTempo.Format ( "%04d/%02d/%02d/%02d ", m_sStartTimeSystem.wYear, m_sStartTimeSystem.wMonth,
						  m_sStartTimeSystem.wDay, m_sStartTimeSystem.wDayOfWeek + 1 );
	...
	*o_poTimed += oStrTempo;
	...
	// Day from start
	if( (sTempoTimeSystem.wYear  != 1) || (sTempoTimeSystem.wMonth != 1) || (sTempoTimeSystem.wDay   != 1))
	...
	*o_poTimed += oStrTempo;
	...
	// E..... Add the current progress completion status
	// Elapsed percent
	nTempo = (int)(((LONGLONG)(m_nRight - m_nLower) * 25600 ) / (m_nUpper - m_nLower));
	...
	*o_poTimed += oStrTempo;
	...
	// Day to end
	if( (sTempoTimeSystem.wDay   != 1) || (sTempoTimeSystem.wMonth != 1) || (sTempoTimeSystem.wYear  != 1))
	{ // If not the same day, calculate the number of days elapsed from start
	...
	*o_poTimed += oStrTempo;
	...
	if((sTempoTimeSystem.wYear  != sCurrentTimeSystem.wYear) && (sTempoTimeSystem.wMonth != sCurrentTimeSystem.wMonth)
		&& (sTempoTimeSystem.wDay   != sCurrentTimeSystem.wDay))
	{
		oStrTempo.Format( "%04d/%02d/%02d/%02d ", sTempoTimeSystem.wYear, sTempoTimeSystem.wMonth, 
						   sTempoTimeSystem.wDay, sTempoTimeSystem.wDayOfWeek + 1 );
		*o_poTimed += oStrTempo;
	}	
...
BOOL CSkinProgress::UpdateProgress ( CRect* o_poRectPaint ) 
{
	CRect    oRectPaint;
	int      nLenPos;
	int      nStepPos;
	...
	m_nRightAff = (int) (((LONGLONG)(m_nRight - m_nLower) * (nLenPos  - (nStepPos << 1))) / (m_nUpper - m_nLower) ) + nStepPos;
	...
	// E..... On progress bar change, return the need to refresh it
	if((m_nPrevEndAff   != m_nEndAff) || (m_nPrevRightAff != m_nRightAff))
	{
		return true;
	}
	else
...
BOOL CSkinProgress::OnEraseBkgnd ( CDC* i_poDC )
{
	BOOL bResult;
...
void CSkinProgress::OnPaint () 
{
	...
	CDC      oDcProgress2; // CompatibleDC
	CBitmap* poOldBitmap; // oDC's previous bitmap
	CBitmap* poOldBitmap2; // oDC's previous bitmap
	...
	if( (oRectPaint.Width()  != m_oRectPaint.Width()) || (oRectPaint.Height() != m_oRectPaint.Height()))
	{ // If the progress bar size has changed
		...
		m_poBackProgressBmp = new CBitmap;
		m_poBackProgressBmp->CreateCompatibleBitmap(&oDC, oRectPaint.Width(), oRectPaint.Height()); // *ALWAYS* use '&oDC', *NEVER* '&oDcProgress' otherwise you'll get a monochrom image
	...
	oDcProgress2.CreateCompatibleDC(&oDC);
	poOldBitmap2 = oDcProgress2.SelectObject(m_poBackProgressBmp);
	...
	{ // If the ProgressBar has changed, redraw it completly
		...
		m_oBarImgLst.Draw(&oDcProgress2, cSPB_START, CPoint(0, 0), ILD_NORMAL);

		nCurrentPos = m_nEndAff - nStepPos;
		for(; nCurrentPos >= nStepPos; nCurrentPos -= nStepPos)
		{
			// CCAB.. Draw the background element until the right end of the bar
			m_oBarImgLst.Draw(&oDcProgress2, cSPB_BACKGROUND, CPoint(nCurrentPos, 0), ILD_NORMAL);
		}

		m_oBarImgLst.Draw(&oDcProgress2, cSPB_BACKGROUND, CPoint(nStepPos, 0), ILD_NORMAL);

		m_oBarImgLst.Draw(&oDcProgress2, cSPB_END, CPoint(m_nEndAff, 0), ILD_NORMAL);
	...
	{
		oDcProgress.FillSolidRect( oRectPaint, RGB(255,255,0) );

		nCurrentPos = m_nRightAff;
		nCurrentPos -= (nCurrentPos % nStepPos);
		for(; nCurrentPos >= nStepPos;   nCurrentPos -= nStepPos)
		{
			// CCCB.. Draw the bar element until the left end of the bar
			m_oBarImgLst.Draw(&oDcProgress, cSPB_CENTER, CPoint(nCurrentPos, 0), ILD_TRANSPARENT);
		}

		m_oBarImgLst.Draw(&oDcProgress, cSPB_LEFT, CPoint(0, 0), ILD_TRANSPARENT); // For m_nLeft-less progress bar routine
			
		// CCJ// Car... Draw the right element of the bar
		// Right
		m_oBarImgLst.Draw(&oDcProgress, cSPB_RIGHT, CPoint(m_nRightAff, 0), ILD_TRANSPARENT);
	...
	oDC.TransparentBlt( 0, 0 , oRectPaint.Width(), oRectPaint.Height(),&oDcProgress2, 0, 0, oRectPaint.Width(), oRectPaint.Height(), RGB(255,255,0) );
	oDC.TransparentBlt( 0, 0 , oRectPaint.Width(), oRectPaint.Height(),&oDcProgress, 0, 0, oRectPaint.Width(), oRectPaint.Height(), RGB(255,255,0) );

	// Release the DC
	oDcProgress.SelectObject(poOldBitmap);
	oDcProgress.DeleteDC();
	oDcProgress2.SelectObject(poOldBitmap2);
	oDcProgress2.DeleteDC();
...
void CSkinProgress::OnSizing ( UINT fwSide, LPRECT pRect )
{
	// A..... Resize the object
...
void CSkinProgress::OnSize ( UINT nType, int cx, int cy )
{
	// A..... Transmit the new dimensions
...
// On a Timer event
void CSkinProgress::OnTimer ( UINT nIDEvent )
{
