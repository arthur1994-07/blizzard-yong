	...
	m_pFont ( NULL )
{
	...
	m_CrowTreePage.SetSheetTab ( this );	//	0
	AddPage ( &m_CrowTreePage );	
	
	m_CrowAttackPage.SetSheetTab ( this );	//	1
	AddPage ( &m_CrowAttackPage );

	m_CrowDataPage.SetSheetTab ( this );	//	2
	AddPage ( &m_CrowDataPage );

	m_CrowAttackTreePage.SetSheetTab ( this );
	AddPage ( &m_CrowAttackTreePage );
...
{
	CMobNpcEditDlg* pDlg = (CMobNpcEditDlg*)m_pWndParent;
...
void CsheetWithTab::ActiveCrowDataPage ( int CurPage, SCROWDATA* pCrowData )
{
	CMobNpcEditDlg* pDlg = (CMobNpcEditDlg*)m_pWndParent;
	...
	m_CrowDataPage.SetItem ( pCrowData );		
	SetActivePage ( CROWDATAPAGE );
}

void CsheetWithTab::ActiveCrowAttackTreePage ( int CurPage, SCROWDATA* pCrowData )
{
	CMobNpcEditDlg* pDlg = (CMobNpcEditDlg*)m_pWndParent;
	pDlg->EnableMenu ( ID_MENUITEM_NEW,  FALSE );
	pDlg->EnableMenu ( ID_MENUITEM_LOAD, FALSE );
	...
	m_CrowAttackTreePage.SetCallPage ( CurPage );
	m_CrowAttackTreePage.SetCrowData ( pCrowData );		
	SetActivePage ( CROWATTACKTREEPAGE );
}

void CsheetWithTab::ActiveCrowAttackPage ( int CurPage, SCROWDATA* pCrowData, DWORD dwID )
{
	CMobNpcEditDlg* pDlg = (CMobNpcEditDlg*)m_pWndParent;
	pDlg->EnableMenu ( ID_MENUITEM_NEW,  FALSE );
	pDlg->EnableMenu ( ID_MENUITEM_LOAD, FALSE );
	...
	m_CrowAttackPage.SetItem ( pCrowData, dwID );		
	...
	case WM_KEYDOWN:
		{
			...
			{
				if ( GetActivePage() != &m_CrowTreePage )
				{
					m_CrowDataPage.InitAllItem();
					m_CrowAttackPage.InitAllItem();
					m_CrowAttackTreePage.InitAllItem();
