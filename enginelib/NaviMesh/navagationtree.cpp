...
HRESULT NavigationMesh::GetSizeNode(
	D3DXMATRIX& matComb,
	DWORD* pCellIndex,
	DWORD nCellIndex,
	D3DXVECTOR3& vMax,
...
HRESULT NavigationMesh::GetCenterDistNode(
	D3DXMATRIX& matComb,
	DWORD* pCellIndex,
	DWORD nCellIndex,
	D3DXVECTOR3& vMax,
...
BOOL NavigationMesh::IsWithInTriangle(
	D3DXMATRIX& matComb,
	LPD3DXVECTOR3 pvT1,
	LPD3DXVECTOR3 pvT2,
	LPD3DXVECTOR3 pvT3,
	...
	DWORD dwAxis )
{
	...
	{
	case 0:
		if( vCenter.x <= fDivision )
			return TRUE;
		break;
	
	case 1:
		if( vCenter.y <= fDivision )
			return TRUE;
		break;
	
	case 2:
		if( vCenter.z <= fDivision )
			return TRUE;
		break;
	};
...
HRESULT NavigationMesh::MakeAABBNode(
	DxAABBNode* pNode,
	D3DXMATRIX& matComb,
	DWORD* pCellIndex,
	DWORD nCellIndex,
	...
	D3DXVECTOR3& vMin )
{
	...
	if( nCellIndex == 1 )
	{
		...
		const NavigationCell *pCell = m_pDACell[ pCellIndex[ 0 ] ];
		pNode->Import( pCell->CellID(), m_pNaviVertex[ pCell->Vertex( 0 ) ], m_pNaviVertex[ pCell->Vertex( 1 ) ], m_pNaviVertex[ pCell->Vertex( 2 ) ] );
...
// CollisionID 로 충돌여부를 확인한다.
void NavigationMesh::IsCollision(
	DxAABBNode* pAABBCur,
	const D3DXVECTOR3& vP1,
	const D3DXVECTOR3& vP2,
	D3DXVECTOR3& vCollision,
	DWORD& CollisionID ) const
{
	...
	if( pAABBCur->IsCollision( vNewP1, vNewP2 ) )
	{
		...
		{	
			...
			if( COLLISION::IsLineTriangleCollision( pvT0, pvT1, pvT2, &vP1, &vP2, &vNewCollision, NULL, EMCC_CULL_CCW, TRUE ) )
			{
				D3DXVECTOR3 vDxVect;
				...
				if( fNewLength < fOldLength )
				{
					...
					CollisionID = pAABBCur->GetFaceID();
				}
			}
		}
		else
		{
			if( pAABBCur->pLeftChild )
				IsCollision( pAABBCur->pLeftChild, vP1, vP2, vCollision, CollisionID );

			if( pAABBCur->pRightChild )
				IsCollision( pAABBCur->pRightChild, vP1, vP2, vCollision, CollisionID );
		}
	}
}
    ...
    BOOL& bCollision) const
{
	...
	// 말이 안되는 위치에 검사가 시작된다면 하지 않도록 한다.
	if ( vPoint1.x > 1000000.f || vPoint1.y > 1000000.f || vPoint1.z > 1000000.f ||
		vPoint2.x > 1000000.f || vPoint2.y > 1000000.f || vPoint2.z > 1000000.f ||
		vPoint1.x < -1000000.f || vPoint1.y < -1000000.f || vPoint1.z < -1000000.f ||
		vPoint2.x < -1000000.f || vPoint2.y < -1000000.f || vPoint2.z < -1000000.f)
	{
		return;
	}
...
HRESULT	NavigationMesh::MakeAABBTree( )
{
	...
	HRESULT hr = GetSizeNode( matComb, pCellIndex, TotalCells(), vMax, vMin );
...
void NavigationMesh::GetAllCollisionCell(
	DWORD* pCollisionCellID,
	DWORD& CollisionIDCount,
	D3DXVECTOR3& vAMax,
	D3DXVECTOR3& vAMin,
	DxAABBNode* pAABBCur )
{
	...
	if( COLLISION::IsCollisionAABBToAABB( vAMax, vAMin, vBMax, vBMin ) )
	{
		if( pAABBCur->IsEnableFace() )
		{
			pCollisionCellID[ CollisionIDCount ] = pAABBCur->GetFaceID();
			CollisionIDCount++;
		}
		else
		{
