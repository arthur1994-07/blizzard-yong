...
public:
	...
	void  SetCellID ( const DWORD CellID) ;
...
inline bool NavigationCell::IsPointInCellCollumn(const D3DXVECTOR2& TestPoint) const
{
	...
	for (int i=0;i<3;i++)
	{
		Line2D::POINT_CLASSIFICATION SideResult = m_Side[i].ClassifyPoint(TestPoint,0.001f);
...
inline void NavigationCell::SetCellID(const DWORD CellID)
{
	m_CellID = CellID;
}
