...
VOID CTextUtil::SetStateBlock( BOOL bPrev )
{
	...
	if( bPrev )
	{
		...
		{
			sc::writeLogError(std::string("ERROR : CCTextUtil::SetStateBlock, bPrev && m_bStateBlocked"));
		}
		m_pTextureSB->Apply();
	...
	else
	{
		...
		{
			sc::writeLogError(std::string("ERROR : CTextUtil::SetStateBlock, !bPrev && !m_bStateBlocked"));
		}
