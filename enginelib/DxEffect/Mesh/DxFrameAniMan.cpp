...
void DxFrameAniMan::Save( sc::SerialFile& SFile )
{
	if( m_pAniManHead )
	{
		SFile << (BOOL)TRUE;
...
void DxFrameAniMan::Load( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	// Note : 
	BOOL bExist = FALSE;
	...
	if ( bExist )
	{
		...
		m_pAniManHead->LoadFile_OLD( SFile, pd3dDevice );
	}
...
namespace NSOPTIMIZEANI
{
								  ...
								  LPDIRECT3DDEVICEQ pd3dDevice, char *pParentName )
	{
		...
		while ( pMesh )
		{
			...
			pNewMesh->m_cMaterials = pMesh->m_cMaterials;
			pNewMesh->m_rgMaterials = new D3DMATERIALQ[pNewMesh->m_cMaterials];
			pNewMesh->m_exMaterials = new D3DEXMATERIAL[pNewMesh->m_cMaterials];
			//pNewMesh->pTexEff = new TEXEFF_PROPERTY[pNewMesh->cMaterials];
			pNewMesh->m_pTextureRes = new TextureResource[pNewMesh->m_cMaterials];
			pNewMesh->m_pMaskTexRes = new TextureResource[pNewMesh->m_cMaterials];
			pNewMesh->m_strTextureFiles = new CString[pNewMesh->m_cMaterials];

			for( DWORD i=0; i<pNewMesh->m_cMaterials; ++i )
			{
				pNewMesh->m_rgMaterials[i] = pMesh->m_rgMaterials[i];
				pNewMesh->m_exMaterials[i] = pMesh->m_exMaterials[i];
				
				strName = pMesh->m_strTextureFiles[i].GetString();
				std::transform ( strName.begin(), strName.end(), strName.begin(), tolower );
				pNewMesh->m_strTextureFiles[i] = strName.c_str();

                pNewMesh->m_pTextureRes[i] = TextureManager::GetInstance().LoadTexture(
                    pNewMesh->m_strTextureFiles[i].GetString(),
                    false,
			...
			*ppTailMesh = pNewMesh;			
			ppTailMesh = &pNewMesh->m_pMeshNext;
