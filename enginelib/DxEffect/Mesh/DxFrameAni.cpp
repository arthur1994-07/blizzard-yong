...
#include "../TexEff/DxTexEffMan.h"
#include "../TexEff/DxTexEffDiffuse.h"
#include "../TexEff/DxTexEffSpecular.h"
...
void DxFrameAttribute::Save( sc::SerialFile& SFile )
{
...
void DxFrameAttribute::Load( sc::SerialFile& SFile, DxAnimationMan* pAniManHead )
{
	CleanUp();
...
void DxFrameAniVBTEX::ANIMESH::Save( sc::SerialFile& SFile )
{
	SFile << m_dwVertices;
	SFile << m_dwFaces;
...
void DxFrameAniVBTEX::ANIMESH::Load( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, DxAnimationMan* pAniManHead )
{
	// Note : 깔끔하게 지워준다.
	CleanUp();
...
void DxFrameAniVBTEX::Render( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
        m_emType = m_textureRes.GetAlphaType();
	}
	else
	{
		return;
...
void DxFrameAniVBTEX::Save( sc::SerialFile& SFile )
{
	SFile << VERSION;
...
void DxFrameAniVBTEX::Load( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, DxAnimationMan* pAniManHead, const char* pName )
{
	// Note : 데이터 정리
	CleanUp();
	...
	if( pMeshs )
	{
		...
		if( pMesh )
		{
			...
			for( UINT i=0; i<dwAttrib; ++ i )
			{
				...
				MAPTEXFRAMEANI_ITER iter = m_mapFrameAni.find( pMeshs->m_strTextureFiles[ nAttribIdCUR ].GetString() );
				...
				else
				{
					...
					m_mapFrameAni.insert( std::make_pair( pMeshs->m_strTextureFiles[ nAttribIdCUR ].GetString(), pData ) );
				}
			}
		}

		pMeshs = pMeshs->m_pMeshNext;
	}
}
...
//				nAttribIdCUR = pAttrib[i].AttribId;
//
//				MAPTEXFRAMEANI_ITER iter = m_mapFrameAni.find( pMeshs->m_strTextureFiles[ nAttribIdCUR ].GetString(MAX_PATH) );
//				if( iter!=m_mapFrameAni.end() )
//				{
//					// Note : 동일한 Texture를 쓰는 것이 있다.
//					//		데이터 추가를 해야함.
...
//										pAttrib[nAttribIdCUR].FaceStart, pAttrib[nAttribIdCUR].FaceCount);
//
//					m_mapFrameAni.insert( std::make_pair( pMeshs->m_strTextureFiles[ nAttribIdCUR ].GetString(MAX_PATH), pData ) );
//				}
//			}
//
//			SAFE_DELETE( pAttrib );
...
void DxFrameAni::Save( sc::SerialFile& SFile )
{
	SFile << VERSION;
...
void DxFrameAni::Load( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, DxAnimationMan* pAniManHead )
{
	// Note : 초기화
	std::for_each( m_mapFrameAni.begin(), m_mapFrameAni.end(), std_afunc::DeleteMapObject() );
	m_mapFrameAni.clear();
...
DxFrameAniVBTEX* DxFrameAni::GetFrameAniVBTEX( size_t nIndex )
{
    if (m_mapFrameAni.size() > nIndex)
    {
        MAPTEXFRAMEANI_ITER it = m_mapFrameAni.begin();
        std::advance(it, nIndex);

        if (it != m_mapFrameAni.end())
        {
            return (*it).second;
        }
    }
