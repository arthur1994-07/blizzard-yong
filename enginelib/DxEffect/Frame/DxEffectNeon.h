...
struct DXMATERIAL_NEON
{
	...
	D3DXVECTOR2			vScaleUV;		// 스케일 용
	D3DCOLOR			vColor;
	TextureResource m_textureResSrc;
    TextureResource m_textureResNeon;
	char				szTexture[MAX_PATH];
	char				szNeonTex[MAX_PATH];
    ...
    DXMATERIAL_NEON& operator= ( const DXMATERIAL_NEON& rvalue )
    {
        bUse = rvalue.bUse;
        vMoveTex = rvalue.vMoveTex;
        vScaleUV = rvalue.vScaleUV;
        vColor = rvalue.vColor;
        m_textureResSrc = rvalue.m_textureResSrc;
        m_textureResNeon = rvalue.m_textureResNeon;
        memcpy( szTexture, rvalue.szTexture, sizeof( char ) * MAX_PATH );
        memcpy( szNeonTex, rvalue.szNeonTex, sizeof( char ) * MAX_PATH );
