...
{
	pd3dDevice->SetTexture( 0, m_textureResFlash.GetTexture() );
	pd3dDevice->SetTexture( 1, m_textureResFlash.GetTexture() );
...
HRESULT DxEffectRiver::UpdateWaveTex ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	pd3dDevice->SetTexture ( 0, m_textureResDark.GetTexture() );
...
HRESULT DxEffectRiver::Render_VS_PS_2 ( LPDIRECT3DDEVICEQ pd3dDevice, DxLandMan* pLandMan )
{
	...
	D3DXVECTOR4 vFog;
	pd3dDevice->GetRenderState ( D3DRS_FOGSTART,	(DWORD*)&vFog.x );
	pd3dDevice->GetRenderState ( D3DRS_FOGEND,	(DWORD*)&vFog.y );
	vFog.z = vFog.y - vFog.x;
	vFog.z = 1.f/vFog.z;
	pd3dDevice->SetVertexShaderConstantF ( 20, (float*)&vFog, 1 );
	...
	pd3dDevice->SetTexture( 0, m_textureResNormal.GetTexture() );
	pd3dDevice->SetTexture( 1, m_textureResNormal.GetTexture() );
