...
void DxWaterTree::Save ( sc::SerialFile& SFile )
{
	SFile << m_vMin;
	SFile << m_vMax;
...
void DxWaterTree::Load ( sc::SerialFile& SFile )
{
	SFile >> m_vMin;
	SFile >> m_vMax;
...
void DxWaterTree::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	if ( IsLoad() )		return;
...
void DxWaterAABB::Save ( sc::SerialFile& SFile )
{
	Save ( SFile, m_pTree );
}

void DxWaterAABB::Save ( sc::SerialFile& SFile, DxWaterTree* pTree )
{
	if ( pTree )
	{
		SFile << (BOOL)TRUE;
...
void DxWaterAABB::Load ( sc::SerialFile& SFile )
{
	Load ( SFile, m_pTree );
}

void DxWaterAABB::Load ( sc::SerialFile& SFile, PDXWATERTREE& pTree )
{
	BOOL bUse = TRUE;
	SFile >> bUse;
...
void DxWaterAABB::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, const CLIPVOLUME &sCV )
{
	DynamicLoad ( pd3dDevice, SFile, sCV, m_pTree );
}

void DxWaterAABB::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, const CLIPVOLUME &sCV, DxWaterTree* pTree )
{
	if ( !pTree )	return;
