...
HRESULT	DxEffectRainPoint::SEPERATEOBJ::CreateSeperateVB ( LPDIRECT3DDEVICEQ pd3dDevice )
{
...

	{
		...
		for( DWORD i=0; i<dwFaces; i++ )
		{
			...
			pRainArray[i].vDirect	= D3DXVECTOR3(sc::Random::RANDOM_POS()-0.5f, sc::Random::RANDOM_POS(), sc::Random::RANDOM_POS()-0.5f );
			pRainArray[i].bRainDrop	= FALSE;
			pRainArray[i].fHeight	= 0.f;
			pRainArray[i].fSize		= 1.f;
			pRainArray[i].fAlpha	= 1.f;
			pRainArray[i].fRandom	= (sc::Random::RANDOM_POS() * 2.5f);	//3~5사이 값
			pRainArray[i].fLoopSum	= (sc::Random::RANDOM_POS() * RAINTIME);	// Loop point
		}
	}

	{
		...
		for( DWORD i=0; i<dwFaces; i++ )
		{
            ...
            pSnowArray[i].vDirection	= D3DXVECTOR3(sc::Random::RANDOM_POS() - 0.5f, -((sc::Random::RANDOM_POS() * 0.5f) + 0.5f), sc::Random::RANDOM_POS()-0.5f );
			pSnowArray[i].fRandom		= (sc::Random::RANDOM_POS() * 2.f) + 1.f;		// 1~3 사이 값
			pSnowArray[i].fSize			= 1.f;
			pSnowArray[i].fLoopSum		= SNOWTIMEUP + (sc::Random::RANDOM_POS() * SNOWTIME);		//
		}
	}

	{
		...
		for( DWORD i=0; i<dwFaces; i++ )
		{
			...
			pLeavesArray[i].vDirection	= D3DXVECTOR3((sc::Random::RANDOM_POS()-0.5f), -(sc::Random::RANDOM_POS()+2.f), (sc::Random::RANDOM_POS()-0.5f)+2.f );
			pLeavesArray[i].vWindPos	= D3DXVECTOR2 ( 0.f, 0.f );
			pLeavesArray[i].fRandom		= 4.f + sc::Random::RANDOM_POS();		// 4~5 사이 값
			pLeavesArray[i].fSize		= 1.f;
			pLeavesArray[i].fLoopSum	= LEAVESTIMEUP + (sc::Random::RANDOM_POS()*LEAVESTIME);
...

	{
		...
		for (DWORD i=0; i<dwFaces; i++)
		{
			...
			pFireFlyArray[i].fEndTime	= 5.f+(sc::Random::RANDOM_POS()*25.f);	// 5~30 사이 값
		}
	}
...
HRESULT DxEffectRainPoint::SEPERATEOBJ::RenderRain ( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV )
{
	HRESULT	hr = S_OK;
	...
	// 튀기는 빗방울
	{
		...
		else
		{
			...
			for ( DWORD i=0; i<dwRenderFaces; i++ )		// 계산도 조금 시킨다.
			{
				...
				DxBillboardLookAt ( matLocal, &pRainArray[i].vPos, DxViewPort::GetInstance().GetMatView() );
...
HRESULT DxEffectRainPoint::SEPERATEOBJ::RenderSnow ( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV )
{
	HRESULT	hr = S_OK;
	...
	//	Note : Draw
	{
		...
		for ( DWORD i=0; i<dwRenderFaces; i++ )		// 계산도 조금 시킨다.
		{
			...
			DxBillboardLookAt ( matLocal, &pSnowArray[i].vPos, DxViewPort::GetInstance().GetMatView() );
...
HRESULT DxEffectRainPoint::SEPERATEOBJ::RenderLeaves ( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV )
{
	HRESULT hr = S_OK;
...
HRESULT DxEffectRainPoint::SEPERATEOBJ::RenderGround ( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV )
{
	HRESULT hr = S_OK;
/*
...
void DxEffectRainPoint::SEPERATEOBJ::Save( sc::SerialFile &SFile )
{
	SFile << dwFaces;
	SFile << dwGroundFaces;
	SFile << vMax;
...
void DxEffectRainPoint::SEPERATEOBJ::Load( sc::SerialFile &SFile, DWORD version )
{
	SFile >> dwFaces;
	SFile >> dwGroundFaces;
	SFile >> vMax;
...
void SParticleArea::FirstCreateParticle( const SParticleData& snowData, const SParticleData& rainData, const SParticleData& leavesData )
{
	...
	// Create New Snow Effect 
	{
		for( i = 0; i < snowData.Count; i++ )
		{
			...
			vPos = D3DXVECTOR3( sc::Random::RandomNumber( vMin.x, vMax.x ), sc::Random::RandomNumber( 0.0f, 300.0f), sc::Random::RandomNumber( vMin.z, vMax.z ) );
			vDir = D3DXVECTOR3( sc::Random::RandomNumber( -0.5f, 0.5f ), 0.0f, sc::Random::RandomNumber( -0.5f, 0.5f ) );

			fDisappearYPos = sc::Random::RandomNumber( snowData.fMinDisappear, snowData.fMaxDisappear );
			fFallSpeed = sc::Random::RandomNumber( -snowData.fMaxFallSpeed, -snowData.fMinFallSpeed );
	...
	// Create New Rain Effect 
	{
		for( i = 0; i < rainData.Count; i++ )
		{
			...
			fDisappearYPos = sc::Random::RandomNumber( rainData.fMinDisappear, rainData.fMaxDisappear );
			fFallSpeed = sc::Random::RandomNumber( -rainData.fMaxFallSpeed, -rainData.fMinFallSpeed );
			fWaveRate = 0.0f;

			vDir = D3DXVECTOR3( sc::Random::RandomNumber( 1.5f, 2.0f ), fFallSpeed, sc::Random::RandomNumber( 1.5f, 2.0f ) );
	...
	// Create New Leaves Effect 
	{
		for( i = 0; i < leavesData.Count; i++ )
		{
			...
			float fDisappearYPos, fFallSpeed, fWaveRate;
			vPos = D3DXVECTOR3( sc::Random::RandomNumber( vMin.x, vMax.x ), sc::Random::RandomNumber( 0.0f, 300.0f), sc::Random::RandomNumber( vMin.z, vMax.z ) );
			vDir = D3DXVECTOR3( sc::Random::RandomNumber( -1.0f, 1.0f ), 0.0f, sc::Random::RandomNumber( -1.0f, 1.0f ) );

			fDisappearYPos = sc::Random::RandomNumber( leavesData.fMinDisappear, leavesData.fMaxDisappear );
			fFallSpeed = sc::Random::RandomNumber( -leavesData.fMaxFallSpeed, -leavesData.fMinFallSpeed );
			fWaveRate = sc::Random::RandomNumber( 15.0f, 30.0f );
