...
void DxReflectTree::Save ( sc::SerialFile& SFile )
{
	SFile << m_vMin;
	SFile << m_vMax;
...
void DxReflectTree::Load ( sc::SerialFile& SFile )
{
	SFile >> m_vMin;
	SFile >> m_vMax;
...
void DxReflectTree::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	if ( IsLoad() )		return;
...
void DxReflectAABB::Save ( sc::SerialFile& SFile )
{
	Save ( SFile, m_pTree );
}

void DxReflectAABB::Save ( sc::SerialFile& SFile, DxReflectTree* pTree )
{
	if ( pTree )
	{
		SFile << (BOOL)TRUE;
...
void DxReflectAABB::Load ( sc::SerialFile& SFile )
{
	Load ( SFile, m_pTree );
}

void DxReflectAABB::Load ( sc::SerialFile& SFile, PDXREFLECTTREE& pTree )
{
	BOOL bUse = TRUE;
	SFile >> bUse;
...
void DxReflectAABB::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, const CLIPVOLUME &sCV )
{
	DynamicLoad ( pd3dDevice, SFile, sCV, m_pTree );
}

void DxReflectAABB::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile, const CLIPVOLUME &sCV, DxReflectTree* pTree )
{
	if ( !pTree )	return;
...
namespace NSREFLECT
{
	...
	void MakeOctree ( PDXREFLECTTREE& pTree )
	{
		...
		else if (pTree->m_fDisY>pTree->m_fDisZ)									// Y 길다.
		{
			vecDWORD.push_back(1);
