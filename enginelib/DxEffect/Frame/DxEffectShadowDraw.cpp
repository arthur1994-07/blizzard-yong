...
{	
	...
	D3DXVECTOR2	pD(
		matDirect._11*sPos.x + matDirect._21*sPos.y + matDirect._31*sPos.z + matDirect._41,
		matDirect._12*sPos.x + matDirect._22*sPos.y + matDirect._32*sPos.z + matDirect._42);

	fDis = matDirect._14*sPos.x + matDirect._24*sPos.y + matDirect._34*sPos.z + matDirect._44;
...
void DxShadowTree::CharShadowDraw ( LPDIRECT3DDEVICEQ pd3dDevice, LPDIRECT3DTEXTUREQ pTex )
{
	...
	if ( GLPeriod::GetInstance().IsOffLightForShadow() )	// 낮일 경우.
	{
		...
		DxSetTextureMatrix ( pd3dDevice, 0, matLastWVP_D );
...
void DxShadowTree::Save ( sc::SerialFile& SFile )
{
	SFile << m_vMin;
	SFile << m_vMax;
...
void DxShadowTree::Load ( sc::SerialFile& SFile )
{
	SFile >> m_vMin;
	SFile >> m_vMax;
...
void DxShadowTree::DynamicLoad ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	if ( m_bLoad )		return;
