...
protected:
	...
	DWORD				m_dwUseSwShader;	// 그래픽 카드가 Shader 를 지원 하는가? - TRUE 면 불가능
	...
	BOOL				m_bPixelShader_3;	// Pixel Shader 3.0 이상 버젼 사용 가능한가 ?
	

public:
	DWORD				GetDetailFlag ()		{ return m_dwDetailFlag; }
	BOOL				IsDeviceStaticShadow()	{ return m_emSSDetail; }		// 정적그림자가 Device 상 되는지 확인 코드.
	...
	BOOL				IsPixelShader_3_0 ()	{ return m_bPixelShader_3; }
	BOOL				IsEnableCharPixelShader()	{ return m_bPixelShader_2; }	// Device 적으로 2.0 이 가능한가 ?

	BOOL			IsRealReflect () const	{ return m_dwDetailFlag&REALREFLECT; }
	...
	void SetSkinDetail ( EMSKINDETAIL emSkinDetail );
...
public:
	...
	D3DXMATRIX GetCrowTransMatrix(const STARGETID &sTargetID)
	{
		D3DXMATRIX matTEMP;
		if ( m_pCrowTracer )	return m_pCrowTracer->GetTargetTransMatrix( sTargetID );
		else					return *D3DXMatrixIdentity(&matTEMP);
	}
