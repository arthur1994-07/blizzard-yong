...
TSTRING				g_strJIGGLE_BONE		= _T("");
TSTRING				g_strJIGGLE_BONE_PREV	= _T("");
float				g_fJIGGLE_BONE_SWING_Y	= 0.f;
float				g_fJIGGLE_BONE_SWING_Z	= 0.f;
D3DXVECTOR3			g_vJIGGLE_BONE_ROTATE	= D3DXVECTOR3( 0.f, 0.f, 0.f);
INT					g_nSELECT_COLL_BONE		 = -1;
...
BOOL				g_bEFFECT_TOOL			 = FALSE;
BOOL				g_bCHAR_EDIT_RUN	   	 = FALSE;
BOOL				g_bCHAR_EDIT_PIECE_RENDER_FIXED = FALSE;	// Fixed 이면서 DxSkinPiece 렌더링 때만 TRUE 이다. 나머지는 전부 FALSE
BOOL                g_bCHAR_CHF_RENDER       = FALSE;
BOOL				g_bEFFECT_SPHERE_VISIBLE = FALSE;
...
HRESULT DxEffectMan::InitDeviceObjects ( LPDIRECT3DQ pD3D, LPDIRECT3DDEVICEQ pd3dDevice, D3DSURFACE_DESC &d3dsdBackBuffer )
{
	...
	if ( m_bUseMIXED &&	d3dCaps.VertexShaderVersion >= D3DVS_VERSION(1, 1) )
	{
		...
		if ( d3dCaps.PixelShaderVersion >= D3DPS_VERSION(2, 0) )	m_bPixelShader_2 = TRUE;
		if ( d3dCaps.PixelShaderVersion >= D3DPS_VERSION(3, 0) )	m_bPixelShader_3 = TRUE;
	}
    ...
    DxEffectRainPoint::OnCreateDevice_STATIC ( pd3dDevice );
    DxEffectSpore::OnCreateDevice_STATIC ( pd3dDevice );
...
//	DxEffectToon::CreateDevice ( pd3dDevice );
    ...
    DxEffectRainPoint::OnResetDevice_STATIC ( pd3dDevice );
    DxEffectSpore::OnResetDevice_STATIC ( pd3dDevice );
...
//	DxEffectToon::ReleaseDevice ( pd3dDevice );
    ...
    DxEffectRainPoint::OnLostDevice_STATIC ( pd3dDevice );
    DxEffectSpore::OnLostDevice_STATIC ( pd3dDevice );
...
HRESULT DxEffectMan::DeleteDeviceObjects()
{
    ...
    DxEffectRainPoint::OnDestroyDevice_STATIC ();
    DxEffectSpore::OnDestroyDevice_STATIC ();
...
HRESULT DxEffectMan::CreateBumpMap( LPDIRECT3DDEVICEQ pd3dDevice )
{
    ...
    HRESULT hr = m_pBumpTexture->LockRect( 0, &lrDst, 0, 0 );
	if ( FAILED(hr) )
...
void DxEffectMan::SetSkinDetail ( EMSKINDETAIL emSkinDetail )	
{ 
	m_emSkinDetail = emSkinDetail;
