...
HRESULT DxEffectShadowLow::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	...
	{
		RenderObjTex  ( pframeCur, pd3dDevice, m_textureResSSDay.GetTexture(), SHADOW_TEX1::FVF, m_pSavedTex1SSB, m_pDrawTex1SSB );
	}
	...
	{
		RenderShadow  ( pframeCur, pd3dDevice, m_textureResSSDay.GetTexture(), SHADOW_TEX2::FVF, m_pSavedTex2SB, m_pDrawTex2SB );
	}
	else if ( emVBSET == EM_VBSET_2TEX_COLOR )	
	{
		RenderShadow  ( pframeCur, pd3dDevice, m_textureResSSDay.GetTexture(), SHADOW_TEX2_COLOR::FVF, m_pSavedTex2ColorSB, m_pDrawTex2ColorSB );
...
HRESULT	DxEffectShadowLow::SetShadowUV ( D3DXMATRIX matDirect, VBSET emVBSET )
{
	...
	if ( GLPeriod::GetInstance().IsOffLightForShadow() )	// 낮일 경우
	{
...
{
	if( DxShadowMap::GetInstance().GetShadowDetail() < LAND_QT_MIDDLE )							// 캐릭터 그림자를 사용 안 할 경우
	{
		if ( (m_dwFlag&USESTATICSHADOW) && 
			DxEffectMan::GetInstance().IsDeviceStaticShadow() && 
			DxShadowMap::GetInstance().GetShadowDetail() >= LAND_QT_LOW )				// 지형그림자를 사용 할 경우
		{
...
void DxEffectShadowLow::CreateVB ( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes* pmsMeshs, VBSET emVBSET )
{
	...
	{	
		dwVertices		= pmsMeshs->m_pLocalMesh->GetNumVertices ();
		dwFVFSize		= D3DXGetFVFVertexSize( pmsMeshs->m_pLocalMesh->GetFVF() );
		pmsMeshs->m_pLocalMesh->LockVertexBuffer ( 0L, (VOID**)&pVertices );
	}
	else
	{
		dwVertices		= pmsMeshs->m_pDxOctreeMesh->m_dwNumVertices;
		dwFVFSize		= D3DXGetFVFVertexSize( pmsMeshs->m_pDxOctreeMesh->GetFVF() );
		pmsMeshs->m_pDxOctreeMesh->LockVertexBuffer( (VOID**)&pVertices );
	}
	...
	if ( emVBSET == EM_VBSET_1TEX_S )
	{
		...
		for ( DWORD i=0; i<dwVertices; i++ )
		{
			pPosition		= (D3DXVECTOR3*)(pVertices + i*dwFVFSize);
			pVert[i].vPos	= *pPosition;
...
void DxEffectShadowLow::SaveBuffer ( sc::SerialFile &SFile )
{
	//	읽지 않고 스킵용으로 사용됨.
...
void DxEffectShadowLow::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
void DxEffectShadowLow::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
HRESULT DxEffectShadowLow::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
	assert(pSrcEffect);
