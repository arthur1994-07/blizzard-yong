...
DxEffSingle* WAVE_PROPERTY::NEWOBJ ( boost::shared_ptr<DxEffSingleGroup>& spThisGroup )
{
...
HRESULT WAVE_PROPERTY::SaveFile ( sc::SerialFile &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
...
HRESULT WAVE_PROPERTY::LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache, BOOL bOldDecalUSE )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT WAVE_PROPERTY::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice , BOOL bBackUpCache)
{
	//	Note : 형제, 자식.
	//
...
HRESULT DxEffectWave::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX &matComb, float fAlpha )
{
	HRESULT hr = S_OK;
	...
	//	Note : 빛에 영향을 받을 경우 ^^;
	//
	if ( m_dwFlag&USELIGHTING && (GLPeriod::GetInstance().IsOffLight()||DxLightMan::GetInstance()->IsNightAndDay()==FALSE) )
		goto _RETURN;
