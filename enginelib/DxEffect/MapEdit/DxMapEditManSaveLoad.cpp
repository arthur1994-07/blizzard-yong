...
void DxMapEditMan::LoadSet(char* pFileName, LPDIRECT3DDEVICEQ pd3dDevice, const SUBPATH* pPath)
{
	...
	if ( dwVer == VERSION )
	{
		...
		// Piece 관리하는 파일 Load
		std::string strStaticPisName;
		strStaticPisName = strSrcName;
		strStaticPisName += ".apis";
		std::string strDirName;
		strDirName = "..\\..";
		strDirName += pPath->ObjFileStaticMesh(); // SUBPATH::OBJ_FILE_STATICMESH;
		m_sPieceMan.Load(pd3dDevice, strStaticPisName.c_str(), strDirName.c_str());
	}
	else
	{
...
//													Q	u	a	d		B	a	s	e
// ----------------------------------------------------------------------------------------------------------------------------------
void DxMapEditMan::QuadBase::SaveFile ( sc::SerialFile &SFile )
{
	SFile << nID_X;
	SFile << nID_Z;
	SFile << vMax;
...
void DxMapEditMan::QuadBase::LoadFile ( sc::SerialFile &SFile )
{
	SFile >> nID_X;
	SFile >> nID_Z;
	SFile >> vMax;
...
//															B	A	S	E
// ----------------------------------------------------------------------------------------------------------------------------------
void DxMapEditMan::BASE::SaveFile ( sc::SerialFile &SFile, BOOL bThread )
{
	if ( bThread )
	{
		VERTEX*	pVertices;
...
void DxMapEditMan::BASE::LoadFile ( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile &SFile, BOOL bThread )
{
	if ( bThread )
	{
		VERTEX*	pVertices;
