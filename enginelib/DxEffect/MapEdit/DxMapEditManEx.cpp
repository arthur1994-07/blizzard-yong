...
HRESULT DxMapEditMan::QuadBase::FrameMove ( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV, BOOL bGame )
{
	//if ( !IsRenderEnalble() )
	//{
	//	if ( bGame )
...
HRESULT DxMapEditMan::QuadBase::Render ( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV )
{
...
BOOL DxMapEditMan::QuadBase::IsCollisionLine ( const D3DXVECTOR3 &vPickRayOrig, const D3DXVECTOR3 &vPickRayDir, float& fLength ) const
{
...
void DxMapEditMan::QuadBase::CreateBaseData ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	for ( DWORD z=0; z<EM_GRID; ++z )
	{
		...
		{
			float fRandX = sc::Random::RANDOM_POS(); // TEST
			float fRandY = sc::Random::RANDOM_POS(); // TEST
