...
const DWORD	DxEffCharCloneBlur::VERSION			= 0x0101;
char		DxEffCharCloneBlur::NAME[MAX_PATH]	= "4.캐릭터 잔상 2";
...
DxEffChar* DxEffCharCloneBlur::CloneInstance ( LPDIRECT3DDEVICEQ pd3dDevice, DxCharPart* pCharPart, DxSkinPiece* pSkinPiece, DWORD dwMaterialNum )
{
	HRESULT hr;
...
void DxEffCharCloneBlur::Render ( LPDIRECT3DDEVICEQ pd3dDevice, const D3DXMATRIX* arrayBoneMatrices, BOOL bPieceRender )
{
	if ( !m_pVB )	return;
...
HRESULT DxEffCharCloneBlur::LoadFile_0100 ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//Note : 이팩트의 Property 를 저장.
	//

	EFFCHAR_PROPERTY_CLONEBLUR_100 pPorperty;

	SFile.ReadBuffer ( &pPorperty, sizeof(EFFCHAR_PROPERTY_CLONEBLUR_100) );

	m_Property.Assign( pPorperty );

	return S_OK;
}

HRESULT DxEffCharCloneBlur::LoadFile_0101 ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//	Note : 이팩트의 Property 를 저장.
	//
...
HRESULT	DxEffCharCloneBlur::LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT	DxEffCharCloneBlur::SaveFile ( sc::BaseStream &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
