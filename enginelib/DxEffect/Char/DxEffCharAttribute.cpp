...
const DWORD	DxEffCharAttribute::VERSION			= 0x0101;
...
DxEffChar* DxEffCharAttribute::CloneInstance ( LPDIRECT3DDEVICEQ pd3dDevice, DxCharPart* pCharPart, DxSkinPiece* pSkinPiece, DWORD dwMaterialNum )
{
	HRESULT hr;
...
void DxEffCharAttribute::Render ( LPDIRECT3DDEVICEQ pd3dDevice, const D3DXMATRIX* arrayBoneMatrices, BOOL bPieceRender )
{
	if ( !m_pLeftVB )	return;
...
HRESULT DxEffCharAttribute::LoadFile_0100 ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFCHAR_PROPERTY_ATTRIBUTE_100	Property;

	//	Note : 이팩트의 Property 를 저장.
	//
	SFile.ReadBuffer ( &Property, sizeof(EFFCHAR_PROPERTY_ATTRIBUTE_100) );
...
HRESULT DxEffCharAttribute::LoadFile_0101 ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//	Note : 이팩트의 Property 를 저장.
	//
...
HRESULT	DxEffCharAttribute::LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT	DxEffCharAttribute::SaveFile ( sc::BaseStream &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
