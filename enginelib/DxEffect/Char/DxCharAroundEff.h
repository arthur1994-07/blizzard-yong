...
private:
	...
	VERTEXCOLORTEX m_Vertices[MAX_EFFECT*4];
...
public:
	DxEffChar* CloneInstance ( LPDIRECT3DDEVICEQ pd3dDevice, DxCharPart* pCharPart, DxSkinPiece* pSkinPiece, DWORD dwMaterialNum );

	virtual HRESULT LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice );
	virtual HRESULT SaveFile ( sc::BaseStream &SFile );

	HRESULT LoadFile_0001 ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice );
	HRESULT LoadFile_0002 ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice );		//	데이터 포맷 변경 진행
