...
BOOL	DxOctree::SaveFile ( sc::SerialFile &SFile )
{
	SFile << m_bSubDivided;
...
BOOL	DxOctree::LoadFile ( sc::SerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice, PDXEFFECTBASE &pEffGlobalHead, BOOL bUseDynamicLoad )
{
	BOOL	bExist = FALSE;
	...
	else
	{
		...
		m_pDxFrameHead->LoadFile_OLD ( SFile, pd3dDevice, pEffGlobalHead, m_pDxAlphaMapFrameHead );
		}
		m_bLoaded = TRUE;
	}
...
BOOL DxOctree::DynamicLoad ( sc::SerialFile& SFile, LPDIRECT3DDEVICEQ pd3dDevice, PDXEFFECTBASE &pEffGlobalHead )
{	
	if ( !SFile.IsOpen () )
	{
		return FALSE;
...
BOOL DxOctree::LoadBasicPos ( sc::SerialFile& SFile, LPDIRECT3DDEVICEQ pd3dDevice, PDXEFFECTBASE &pEffGlobalHead, const D3DXVECTOR3 &vMax, const D3DXVECTOR3 &vMin )
{
