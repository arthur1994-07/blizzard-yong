									...
									D3DXVECTOR3 &vMax, D3DXVECTOR3 &vMin )
{
	...
	if ( nCellIndex == 1 )
	{
		...
		CCollisionMapCell *pCell = &m_pMapCells[pCellIndex[0]];
		...
		pNode->Import( pCell->GetCellID(), *pvT1, *pvT2, *pvT3 );
...
void CCollisionMap::IsCollision_NoneConst( DxAABBNode *pAABBCur, const D3DXVECTOR3 &vP1, const D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor, BOOL bEditor, EMCOLLMODE emCullMode )
{
	...
	if ( pAABBCur->IsCollision(vNewP1,vNewP2) )
	{
		...
		{
			if ( !DxFrameMesh::m_bNaviDraw && (m_ppDxFrame[ pAABBCur->GetFaceID() ]->bFlag&DXFRM_NAVIFRAME) )
			{
				// 아무런 동작도 하지 않게 한다.
			}
			else
			{
				CCollisionMapCell	*pCell = &m_pMapCells[ pAABBCur->GetFaceID() ];
				...
				if ( COLLISION::IsLineTriangleCollision( pvT0, pvT1, pvT2, &vP1, &vP2, 
					&vNewCollision, pNor, emCullMode, TRUE ) )
				{
					//	충돌 정보 수집
					SCollisionCell CollisionCell;
					CollisionCell.dwID = pAABBCur->GetFaceID();

					if ( bEditor )
					{
						//	Note : 내비게이션 메시 그리지 않음 검사
						CollisionCell.pFrame = m_ppDxFrame[pAABBCur->GetFaceID()];
					}
		...
		else
		{
			if ( pAABBCur->pLeftChild )
				IsCollision_NoneConst ( pAABBCur->pLeftChild, vP1, vP2, pNor, bEditor, emCullMode );

			if ( pAABBCur->pRightChild )
				IsCollision_NoneConst ( pAABBCur->pRightChild, vP1, vP2, pNor, bEditor, emCullMode );
		}
	}
}

BOOL CCollisionMap::IsCollision_NoneConst ( const D3DXVECTOR3 &vP1, D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor, BOOL bEditor, const DxFrame* pFrame, char* pName, EMCOLLMODE emCullMode )
{
	if ( !m_pAABBTreeRoot )	
	{
		if ( m_pFrameMesh )
		{
			return m_pFrameMesh->IsCollision_NoneConst( vP1, vP2, pNor, pName, emCullMode );
		}
	}
	else
	{
		...
		IsCollision_NoneConst( m_pAABBTreeRoot, vP1, vP2, pNor, bEditor, emCullMode );
		...
		if ( m_pCollisionCell )
		{
			vP2 = m_pCollisionCell->vCollision;
		}
