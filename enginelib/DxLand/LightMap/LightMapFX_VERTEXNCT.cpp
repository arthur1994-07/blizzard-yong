...
LightMapFX_VERTEXNCT::LightMapFX_VERTEXNCT()
	: m_pDCRT(NULL)
{
}
...
HRESULT LightMapFX_VERTEXNCT::OnResetDevice( LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr(S_OK);
...
HRESULT LightMapFX_VERTEXNCT::OnLostDevice()
{
	HRESULT hr(S_OK);
...
void LightMapFX_VERTEXNCT::SetFX( TSTRING& strFX )
{
	strFX = 
		"//-------------------------------------------------------------------------------------- \r\n"
...
BOOL LightMapFX_VERTEXNCT::RenderInexact( LPDIRECT3DDEVICEQ pd3dDevice, LPD3DXMESH pMesh, DWORD dwMaterials, UINT nIndex )
{
	if ( !m_pFX )
		return FALSE;
...
BOOL LightMapFX_VERTEXNCT::Render( LPDIRECT3DDEVICEQ pd3dDevice, 
...
void LightMapFX_VERTEXNCT::SetWorld( const D3DXMATRIX& matWorld, float fAlpha )
{
	if ( !m_pFX )
		return;
