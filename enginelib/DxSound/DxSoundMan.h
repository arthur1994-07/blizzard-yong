...
public:
    ...
    typedef WAVFILE_MAP::value_type     WAVFILE_MAP_VALUE;
...
public:	
	...
	//	<--	웨이브파일 리소스 관리 메쏘드	-->	//
public:
	CSound*	LoadWav(const std::string& strFileName, bool bStereo);
	HRESULT	ReleaseWav(const std::string& strFileName, bool bStereo);
	HRESULT	DeleateWav(const std::string& strFileName, bool bStereo);
	bool	IsLoadedWav(const std::string& strFileName, bool bStereo);
