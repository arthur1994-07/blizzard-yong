...
bool SSound::Load(const std::string& strFileName, bool bLoop, bool bStereo)
{
	if (!strFileName.size())
        return false;
...
BOOL SSound::SaveSet(sc::SerialFile& SFile)
{
	int nFileName = (int) m_strFileName.size();	
	SFile.WriteBuffer(&nFileName, sizeof(int));
	if (nFileName)
        SFile.WriteBuffer(m_strFileName.c_str(), sizeof(char)*nFileName);    
    SFile << m_bLoop;
	return TRUE;
}

BOOL SSound::LoadSet(sc::BaseStream& SFile, WORD SoundVer)
{
	if (SoundVer == 100)
        return LoadSet100(SFile, SoundVer);
...
BOOL SSound::LoadSet100(sc::BaseStream& SFile, WORD SoundVer)
{
	char szFileName[256] = "";
	WORD wBufferCount;
	BOOL bLoop;
