...
CSound* DxSoundMan::LoadWav(const std::string& strFileName, bool bStereo)
{
	if (!m_pSoundManager)
        return NULL;
	if (!strFileName.size())
	...
	//	실제 WAV 파일 로드하기
	{
		...
		HRESULT hr = m_pSoundManager->Create(
            &(pWavFileData->pSound),
            strPathName.GetBuffer(),
			dwCreationFlags,
            guid3DAlgorithm,
            m_nBufferSize);
	...
	m_mapWavFile.insert(WAVFILE_MAP_VALUE(WavFileKey, pWavFileData));

	return pWavFileData->ADD_REF ();
}

HRESULT DxSoundMan::ReleaseWav(const std::string& strFileName, bool bStereo )
{
...
HRESULT DxSoundMan::DeleateWav(const std::string& strFileName, bool bStereo)
{
	if (!strFileName.size())
        return S_FALSE;
...
bool DxSoundMan::IsLoadedWav(const std::string& strFileName, bool bStereo)
{
	if (!strFileName.size())
        return false;
	if (!m_mapWavFile.size())
        return false;
...
BOOL	DxSoundMan::SaveSet ( sc::SerialFile &SFile )
{
	SFile << m_SoundManVER;
...
BOOL	DxSoundMan::LoadSet ( sc::SerialFile &SFile )
{	
	WORD	SoundManVER;
	SFile >> SoundManVER;
	...
	else if ( SoundManVER == 100 ) //	데이터 포맷 변경 진행
	{
		SFile >> m_fRolloffFactor;
		SFile >> m_fDistanceFactor;
		SFile >> m_fDopplerFactor;
...
{	
	m_nBufferSize = new_min(nBufferSize,dwMAX_BUFFER_SIZE);
}
...
const float	DxSoundMan::CALC_FADE( const D3DXVECTOR3& vSoundPos, const float& fMAX_RANGE )
{
	...
	else
	{
		...
		fFadeRatio = new_max ( fFadeRatio, 0.4f );
	}

	return fFadeRatio;
}

void DxSoundMan::Archive( boost::function<void (const TCHAR*)> fun )
{
    BOOST_FOREACH(WAVFILE_MAP_VALUE& it, m_mapWavFile)
    {
        CString strFullPath = m_strPath + it.first.strFileName.c_str();
        fun(strFullPath.GetBuffer());
    }
}
