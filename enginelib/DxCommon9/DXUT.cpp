...
namespace Custom
{
    const UINT RECT_COUNT = 4;
	const UINT RGN_SIZE = sizeof( RGNDATAHEADER ) + sizeof( RECT ) * RECT_COUNT;

    BOOL g_bPresent = TRUE;
    BOOL g_bRgnEnable = FALSE;
    BOOL g_bVisible = FALSE;
    UINT g_uiWidth = 1024;
    UINT g_uiHeight = 768;
    BYTE g_btRgn[ RGN_SIZE ] = { 0 };
	LPRGNDATA g_lpRgn = (LPRGNDATA)g_btRgn;

    VOID InitRgn()
    {
        // V512 A call of the 'memset' function will lead to overflow of the buffer 'g_lpRgn'.
        memset( g_lpRgn, 0, RGN_SIZE );
		g_lpRgn->rdh.dwSize = sizeof( RGNDATAHEADER );
		g_lpRgn->rdh.iType = RDH_RECTANGLES;
		g_lpRgn->rdh.nCount = RECT_COUNT;
		g_lpRgn->rdh.nRgnSize = sizeof( RECT ) * RECT_COUNT;
    }

    VOID SetRgn( UINT uiWidth, UINT uiHeight )
    {
        g_uiWidth = uiWidth;
		g_uiHeight = uiHeight;

        g_lpRgn->rdh.rcBound.left = 0;
		g_lpRgn->rdh.rcBound.top = 0;
		g_lpRgn->rdh.rcBound.right = uiWidth;
		g_lpRgn->rdh.rcBound.bottom = uiHeight;
    }

    VOID CalcRgn( RECT& rtBound )
    {
		const RECT RGN_RECT[ RECT_COUNT ] =
		{
			{ 0, 0, g_uiWidth, rtBound.top },
			{ 0, rtBound.bottom, g_uiWidth, g_uiHeight },
			{ 0, rtBound.top, rtBound.left, rtBound.bottom },
			{ rtBound.right, rtBound.top, g_uiWidth, rtBound.bottom }
		};

		memcpy( g_lpRgn->Buffer, RGN_RECT, sizeof( RECT ) * RECT_COUNT );
    }
    ...
    VOID ModifyWindowStyle( HWND hWnd )
	{
		...
		DWORD dwWindowStyle = GetWindowLong( hWnd, GWL_STYLE );
		dwWindowStyle |= WS_CLIPCHILDREN;
		SetWindowLong( hWnd, GWL_STYLE, dwWindowStyle );
	}

    VOID ModifyPresentParams( D3DPRESENT_PARAMETERS& pp )
	{
		if( g_bRgnEnable )
		{
    ...
    HRESULT Present( IDirect3DDevice9* pd3dDevice )
	{
        if( !g_bPresent )
            return S_OK;

		if( g_bRgnEnable && g_bVisible )
			return pd3dDevice->Present( NULL, NULL, NULL, g_lpRgn );
...
HRESULT DXUTSetWindow( HWND hWndFocus, HWND hWndDeviceFullScreen, HWND hWndDeviceWindowed, bool bHandleMessages )
{
    ...
    Custom::ModifyWindowStyle( hWndFocus );
                          ...
                          void* pUserContext )
{
    ...
    Custom::InitRgn();
    Custom::SetRgn( nSuggestedWidth, nSuggestedHeight );
                                   ...
                                   DXUTMatchOptions* pMatchOptions )
{
    ...
    // 다이렉트 위에 윈도우 컨트롤을 올리기 위해서 셋팅
	Custom::ModifyPresentParams( pValidDeviceSettings->pp );
}
...
HRESULT DXUTCreate3DEnvironment( IDirect3DDevice9* pd3dDeviceFromApp )
{
    ...
    {
        // 다이렉트 위에 윈도우 컨트롤을 올리기 위해서 셋팅
		Custom::ModifyPresentParams( pNewDeviceSettings->pp );
    ...
    // Set Rgn
	{
		...
		DXUTGetD3DDevice()->GetBackBuffer( 0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer );

		D3DSURFACE_DESC d3dsdBackBuffer;
		pBackBuffer->GetDesc( &d3dsdBackBuffer );

        Custom::SetRgn( d3dsdBackBuffer.Width, d3dsdBackBuffer.Height );
...
void DXUTRender3DEnvironment()
{
    ...
    if( !GetDXUTState().GetRenderingPaused() )
    {
        ...
        if( rcClient.Width() <= 0 || rcClient.Height() <= 0 )
            return;
        assert( DXUTGetBackBufferSurfaceDesc()->Width == (UINT)rcClient.Width() );
        assert( DXUTGetBackBufferSurfaceDesc()->Height == (UINT)rcClient.Height() );
#endif

        // Show the frame on the primary surface.
        hr = Custom::Present( pd3dDevice );
        if( FAILED(hr) )
        {
            if( D3DERR_DEVICELOST == hr )
            {
...
void DXUTSetVSyncSetting( bool bVSync )
{
	GetDXUTState().SetOverrideForceVsync( bVSync ? TRUE : FALSE );
}

void DXUTSetPresentEnable( bool bOn )
{
    Custom::g_bPresent = ( bOn ) ? TRUE : FALSE;
}
