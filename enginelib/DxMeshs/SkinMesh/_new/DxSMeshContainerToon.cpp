...
void CreateMeshNORMAL( IDirect3DDevice9 *pd3dDevice, const LPD3DXSKININFO pSkinInfo, const DWORD *pAdjacency, const LPD3DXMESH pMesh, LPD3DXMESH& pMeshOUT )
{
    ...
    for (iAttributeSW = 0; iAttributeSW < dwNumAttributeGroups; iAttributeSW++)
    {
        DWORD cInfl   = 0;
    ...
    if (iAttributeSW < dwNumAttributeGroups)
    {
        LPD3DXMESH pMeshTmp;
...
void CreateMeshHLSL( IDirect3DDevice9 *pd3dDevice, const LPD3DXSKININFO pSkinInfo, const DWORD *pAdjacency, const LPD3DXMESH pMesh, LPD3DXMESH& pMeshOUT )
{
    ...
    UINT MaxMatrices = 60;
    NumPaletteEntries = new_min(MaxMatrices, pSkinInfo->GetNumBones());
...
BOOL IsPointInOder( LPDIRECT3DDEVICEQ pd3dDevice, const LPD3DXMESH pMesh, float fThickness, float fSizeADD, LIST_NEWVERTEXMAN& sNewVertexMan )
{
	...
	{
		(*iter)->CalDirection();
		(*iter)->m_fThickness = fThickness + (fSizeADD * sc::Random::RANDOM_POS());	// 사이즈. ~!
	}
...
void SMeshContainer::CreateNormalMapMESH( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	//m_pNormalMesh->CloneNormalMESH( pd3dDevice,  MeshData.pMesh );
	//pNewMesh = NULL;
...
//
//void SMeshContainer::Save( sc::SerialFile& SFile )
//{
//	/*		D3DXMESHCONTAINER
//	LPSTR                   Name;
//
...
//	//SFile << (BOOL)FALSE;
//
//	////LPD3DXATTRIBUTERANGE	pAttributeTable;		-- m_dwNumAttributeGroups
//	//SFile << (BOOL)FALSE;
//
//	////DWORD					m_dwNumAttributeGroups; 
//	//SFile << m_dwNumAttributeGroups;
//
//	////DWORD					NumInfl;
//	//SFile << NumInfl;
//
...
void SMeshContainer::SaveNORMAL( sc::SerialFile& SFile )
{
	//SFile << VERSION;
...
void SMeshContainer::SaveHLSL( sc::SerialFile& SFile )
{
	//SFile << VERSION;
...
void SMeshContainer::SaveNormalMesh( sc::SerialFile& SFile )
{
	SFile << VERSION;
...
void SMeshContainer::LoadNORMAL( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	//SAFE_DELETE( m_pSlimMesh );
	//SAFE_DELETE( m_pEdgeMeshHLSL );
...
void SMeshContainer::LoadHLSL( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	//SAFE_DELETE( m_pSlimMesh );
	//SAFE_DELETE( m_pEdgeMeshHLSL );
...
void SMeshContainer::LoadNormalMesh( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
	//SAFE_DELETE( m_pNormalMesh );
...
void SMeshContainer::SaveLOD_SW( sc::SerialFile& SFile )
{
	//m_sMeshLOD[0].Save( SFile );
	//m_sMeshLOD[1].Save( SFile );
}

void SMeshContainer::LoadLOD_SW( LPDIRECT3DDEVICEQ pd3dDevice, sc::SerialFile& SFile )
{
