...
#include "../DxLand/DxPiece/OBJOCTree.h"
...
{
#ifdef _DEBUG
	float fOldCount = sc::DXUtil_Timer( sc::TIMER_GETAPPTIME );
#endif
	...
	// Note : 메쉬의 타입별로 나눈다.
	if ( pLandMan->IsStaticMesh() || pLandMan->IsStaticMaterialMesh() )
	{
		pLandMan->ClipStaticMesh( this );
	}
	else
	{
		ClipOctree ( pLandMan->GetOctree() );
	}
...
#ifdef _DEBUG
	float fDxCount = sc::DXUtil_Timer( sc::TIMER_GETAPPTIME ) - fOldCount;
...
void DxDecal::ClipStaticMesh ( DxStaticMesh *pStaticMesh )
{
	if ( !pStaticMesh )
	{
		return;
	...
	MAPSINGLETEXMESH& sTexMapMesh = pStaticMesh->m_mapMesh;
	MAPSINGLETEXMESH_ITER iter = sTexMapMesh.begin();
	for( ; iter!=sTexMapMesh.end(); ++iter )
	{
		ClipSingleTexMesh( (*iter).second );
	}

	MAPSINGLETEXMESH& sTexmapMeshALPHA = pStaticMesh->m_mapMeshALPHA;
	iter = sTexmapMeshALPHA.begin();
	for( ; iter!=sTexmapMeshALPHA.end(); ++iter )
	{
		ClipSingleTexMesh( (*iter).second );
	}

	MAPSINGLETEXMESH& sTexmapMeshSOFTALPHA = pStaticMesh->m_mapMeshSOFTALPHA;
	iter = sTexmapMeshSOFTALPHA.begin();
	for( ; iter!=sTexmapMeshSOFTALPHA.end(); ++iter )
	{
		ClipSingleTexMesh( (*iter).second );
	}

	MAPSINGLETEXMESH& sTexmapMeshSOFTALPHA01 = pStaticMesh->m_mapMeshSOFTALPHA01;
	iter = sTexmapMeshSOFTALPHA01.begin();
	for( ; iter!=sTexmapMeshSOFTALPHA01.end(); ++iter )
	{
		ClipSingleTexMesh( (*iter).second );
	}

	MAPSINGLETEXMESH& sTexmapMeshSOFTALPHA02 = pStaticMesh->m_mapMeshSOFTALPHA02;
	iter = sTexmapMeshSOFTALPHA02.begin();
	for( ; iter!=sTexmapMeshSOFTALPHA02.end(); ++iter )
	{
		ClipSingleTexMesh( (*iter).second );
	}
}

void DxDecal::ClipSingleTexMesh ( DxSingleTexMesh* pSingleTexMesh )
{
	if ( !pSingleTexMesh )
	{
		return;
	...
	AABBRenderStrategy* pAABBRenderStrategy = pSingleTexMesh->GetAABBRenderStrategy();
	if ( pAABBRenderStrategy )
	{
		pAABBRenderStrategy->ClipMesh( this );
	}
}

//void DxDecal::ClipOBJOCTree ( OBJOCTree* pOBJTree )
//{
//	if ( !pOBJTree )	return;
//
//	if ( !COLLISION::IsCollisionVolume(m_ClipVolume, pOBJTree->GetMax(), pOBJTree->GetMin() ) )	return;
//
//	const DxAABBNode* pAABBNode = pOBJTree->GetCollisionTree();
//
//	if ( pAABBNode )
//	{
//		if ( pOBJTree->m_pVertSrc && pOBJTree->m_pIB )
//		{
//			PWORD pwIndexB=NULL;
//			if( FAILED( pOBJTree->m_pIB->Lock( 0, 0, (VOID**)&pwIndexB, D3DLOCK_READONLY ) ) )
//			{
//				return;
//			}
//
//			s_bLockMeshB = TRUE;
//			s_dwFVF      = pOBJTree->m_dwFVF;
//			s_pbDataVB   = (PBYTE)pOBJTree->m_pVertSrc;
//			s_pwIndexB   = pwIndexB;
//
//			ClipMeshAABB ( pAABBNode );
//
//			pOBJTree->m_pIB->Unlock();
//		}
//	}
//
//	if ( pOBJTree->m_pLeftChild  ) ClipOBJOCTree( pOBJTree->GetLeftChild_OBJOCTree()  );
//	if ( pOBJTree->m_pRightChild ) ClipOBJOCTree( pOBJTree->GetRightChild_OBJOCTree() );
//}

void DxDecal::ClipOBJOCTree ( const DxAABBNode* pAABBNode, DWORD dwFVF, PBYTE pVert, WORD* pIndex )
{
	s_bLockMeshB = TRUE;
	s_dwFVF      = dwFVF;
	s_pbDataVB   = pVert;
...
void DxDecal::ClipMeshAABB ( const DxAABBNode* pAABBNode )
{
	...
	{
		if ( pAABBNode->IsEnableFace() )
		{
			DWORD fvfsize = D3DXGetFVFVertexSize ( s_dwFVF );

			PWORD pwIndex = s_pwIndexB + pAABBNode->GetFaceID()*3;
...
LPDIRECT3DVERTEXBUFFERQ DxDecal::CreateVertexBuffer( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if( FAILED(hr) )
	{
        CDebugSet::ToLogFile(std::string("DxDecal::CreateVertexBuffer() -- CreateVertexBuffer -- Failed"));
		return NULL;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxDecal::CreateVertexBuffer() -- Lock -- Failed"));
		return NULL;
	}
...
LPDIRECT3DINDEXBUFFERQ DxDecal::CreateIndexBuffer ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxDecal::CreateIndexBuffer() -- CreateIndexBuffer -- Failed"));
		return NULL;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxDecal::CreateIndexBuffer() -- Lock -- Failed"));
		return NULL;
	}
...
void DxDecal::CloneVertexBuffer ( LPDIRECT3DDEVICEQ pd3dDevice, LPDIRECT3DVERTEXBUFFERQ& pVB )
{
	...
	if ( FAILED(hr) )
	{
        CDebugSet::ToLogFile(std::string("DxDecal::CloneVertexBuffer() -- Lock -- Failed"));
		return;
	}
...
void DxDecal::CloneIndexBuffer ( LPDIRECT3DDEVICEQ pd3dDevice, LPDIRECT3DINDEXBUFFERQ& pIB )
{
	...
	if( FAILED(hr) )
	{
        CDebugSet::ToLogFile(std::string("DxDecal::CloneIndexBuffer() -- Lock -- Failed"));
		return;
	}
