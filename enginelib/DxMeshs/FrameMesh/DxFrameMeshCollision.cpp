...
//
void DxMeshes::IsCollision_DxMeshes( const D3DXMATRIX &matCom, const D3DXVECTOR3 &vP1, D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor, D3DXVECTOR3 &vCollision, 
						   BOOL &bCollision, char* pName, EMCOLLMODE emCullMode )
{
	...
	for ( DWORD dwFaceCur=0; dwFaceCur<dwNumFaces; dwFaceCur++ )
	{
		...
		if ( COLLISION::IsLineTriangleCollision( &vT0, &vT1, &vT2, (D3DXVECTOR3*)&vP1, (D3DXVECTOR3*)&vP2, &vNewCollision, t, u, v, NULL, emCullMode, TRUE ) )
		{
			D3DXVECTOR3 vDxVect;
			vDxVect = vP1 - vNewCollision;

			float fNewLength = D3DXVec3LengthSq ( &vDxVect );
			vDxVect = vP1 - vCollision;
			...
			if ( fNewLength < fOldLength )
			{
				...
				for( DWORD i=0; i<dwAttrib; ++i )
				{
					...
					{
						if ( pName )
							StringCchCopy( pName, MAX_PATH, m_strTextureFiles[dwAttribID].GetString() );
					}
				}

				SAFE_DELETE_ARRAY( pAttrib );

				// Normal 값을 구하길 원할 때만 동작된다.
				if ( pNor )
				{
					D3DXVECTOR3 vNor0 = * (D3DXVECTOR3*) (pbDataVB+fvfsize*dwIndex0+sizeof(D3DXVECTOR3));
					D3DXVECTOR3 vNor1 = * (D3DXVECTOR3*) (pbDataVB+fvfsize*dwIndex1+sizeof(D3DXVECTOR3));
					D3DXVECTOR3 vNor2 = * (D3DXVECTOR3*) (pbDataVB+fvfsize*dwIndex2+sizeof(D3DXVECTOR3));

					D3DXVec3TransformNormal( &vNor0, &vNor0, &matCom );
					D3DXVec3TransformNormal( &vNor1, &vNor1, &matCom );
					D3DXVec3TransformNormal( &vNor2, &vNor2, &matCom );

					*pNor = vNor0 + (vNor1 - vNor0) * u + (vNor2 - vNor0) * v;
...
BOOL DxFrame::IsCollision ( const D3DXVECTOR3 &vPoint1, const D3DXVECTOR3 &vPoint2, D3DXMATRIX* pMat ) const
{
...
BOOL DxFrame::CollisionDetect_NoneConst( const D3DXVECTOR3 &vP1, D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor, D3DXVECTOR3 &vCollision, char* pName, EMCOLLMODE emCullMode )
{
	...
	//COLLISION::vColTestStart = vP1;
	//COLLISION::vColTestEnd = vP2;
	CollisionDetect_NoneConst( this, vP1, vP2, pNor, bCollision, vCollision, pName, emCullMode );

	return bCollision;
}

void DxFrame::CollisionDetect_NoneConst( DxFrame *pframeCur, const D3DXVECTOR3 &vP1, D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor,
							BOOL &bCollision, D3DXVECTOR3 &vCollision, char* pName, EMCOLLMODE emCullMode )
{
	...
	while ( pmsMeshs != NULL )
	{
		...
		{
			pmsMeshs->IsCollision_DxMeshes( pframeCur->matCombined, vP1, vP2, pNor, vCollision, bCollisionCur, pName, emCullMode );
		}
	...
	{
		CollisionDetect_NoneConst( pframeChild, vP1, vP2, pNor, bCollision, vCollision, pName, emCullMode );

		pframeChild = pframeChild->pframeSibling;
	}
}

BOOL DxFrameMesh::IsCollision_NoneConst ( const D3DXVECTOR3 &vP1, D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor, char* pName, EMCOLLMODE emCullMode )
{
	BOOL bCollision = FALSE;
	...
	//COLLISION::vColTestStart = vP1;
	//COLLISION::vColTestEnd = vP2;
	CollisionDetect_NoneConst( GetFrameRoot(), vP1, vP2, pNor, bCollision, pName, emCullMode );

	return bCollision;
}

void DxFrameMesh::CollisionDetect_NoneConst( DxFrame *pframeCur, const D3DXVECTOR3 &vP1, D3DXVECTOR3 &vP2, D3DXVECTOR3* pNor,
								   BOOL &bCollision, char* pName, EMCOLLMODE emCullMode )
{
	if ( !pframeCur )	return;
	...
	//if ( pframeCur->IsCollision ( vP1, vP2, &m_pDxFrame->matRot ) )  -- 문제점 노출.
	{
		...
		while ( pmsMeshs != NULL )
		{
			if ( pmsMeshs->m_pLocalMesh )
			{
				pmsMeshs->IsCollision_DxMeshes( pframeCur->matCombined, vP1, vP2, pNor, m_vCollionPos, bCollisionCur, pName, emCullMode );
			}
	...
	while ( pframeChild != NULL )
	{
		CollisionDetect_NoneConst( pframeChild, vP1, vP2, pNor, bCollision, pName, emCullMode );

		pframeChild = pframeChild->pframeSibling;
	}
}

BOOL DxFrame::BoxIsInTheFrustum ( const D3DXMATRIX* pMat, const CLIPVOLUME &cv, LPDIRECT3DDEVICEQ pd3dDevice )
{
	D3DXVECTOR3 vMin = vTreeMin;
	D3DXVECTOR3 vMax = vTreeMax;
