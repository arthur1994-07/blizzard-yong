		...
		while ( pMesh )
		{
			...
			if ( pVertices )
			{
				...
				{			
					...
					pVertices[*pPrevVertexPos + i].vPos.x = TransformVertex.x;
					pVertices[*pPrevVertexPos + i].vPos.y = TransformVertex.y;
					pVertices[*pPrevVertexPos + i].vPos.z = TransformVertex.z;
				}
							...
							LPDIRECT3DDEVICEQ pd3dDevice, char *pParentName )
{
	...
	while ( pframeSrc )
	{
		...
		{
			int nStrLen = static_cast<int>( strlen(pSrc->szName)+1 );
