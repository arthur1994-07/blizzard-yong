...
void DxAnimation::CalculateEndTime()
{
	m_fEndTime = GetAnimTimeLine ();
}
...
LPD3DXMATRIX DxAnimation::SetTime ( float fGlobalTime, DWORD dwFlag )
{
	...
	else
	{
		...
		float fTime = (float)fmod(fGlobalTime, static_cast<float>(dwFullTime) );
		...
		if (m_pScaleKeys)
		{
			...
			//float fTime = fGlobalTime;
			//if ( dwFlag&DXANI_MOD )
			//	fTime = (float)fmod ( fGlobalTime, static_cast<float>(m_fEndTime) );
...
UINT DxAnimation::IsAnimationMaxKey()
{
	UINT nSize(0);

	if ( nSize < m_cPositionKeys )
		nSize = m_cPositionKeys;

	if ( nSize < m_cRotateKeys )
		nSize = m_cRotateKeys;

	if ( nSize < m_cScaleKeys )
		nSize = m_cScaleKeys;

	if ( nSize < m_cMatrixKeys )
		nSize = m_cMatrixKeys;
...
HRESULT DxAnimationMan::Render( LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME *pCV )
{
	HRESULT hr;
...
float DxAnimationMan::GetEndTime ()
{
	float fEndTime(0.f);
	DxAnimation* pAnimCur = m_pDxAnimationHead;
	while ( pAnimCur != NULL )
	{
		if ( pAnimCur->GetEndTime() > fEndTime )
		{
			fEndTime = pAnimCur->GetEndTime();
		}
		pAnimCur = pAnimCur->pAnimNext;
	}
