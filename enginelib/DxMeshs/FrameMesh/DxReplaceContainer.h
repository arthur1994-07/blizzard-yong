...
public:
    ...
    __forceinline void AddRef() { ++nRefCount; }
    __forceinline void ReleaseRef() { --nRefCount; }
    __forceinline int  GetRef() const { return nRefCount; }

    HRESULT Load(const std::string& FileName, LPDIRECT3DDEVICEQ pd3dDevice, boost::function<void (const TCHAR*)> pArchive = 0);
...
public:
	HRESULT Render( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX* pMatWorld, const CLIPVOLUME* pCV, float fAlpha );
	void	Render_ALPHA( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX* pMatWorld, const CLIPVOLUME* pCV, float fAlpha );

public:

	void	GetCollisionLineInTriListExceptY      ( D3DXMATRIX* pMatWorld, const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, std::vector<COLLISION::TRIANGLE> &TriList ); // 주병진 추가 메소드
	void	GetCollisionLineInTriListExceptYFrame ( DxFrame* pframeCur, const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, std::vector<COLLISION::TRIANGLE> &TriList );              // 주병진 추가 메소드
...
{
private:
    typedef std::tr1::unordered_map<std::string, DXREPLACEPIECE*> REPLACEMAP;
	typedef REPLACEMAP::iterator					              REPLACEMAP_ITER;
    typedef REPLACEMAP::value_type					              REPLACEMAP_VALUE;
...
public:
    DXREPLACEPIECE* FindPice(const std::string& FileName);

public:
    DXREPLACEPIECE* LoadPiece(const std::string& FileName);
    void ReleasePiece(const std::string& FileName);
...
public:
	...
	// Note : 알파를 빼려고 한다.
	static void DrawRePlacePiece( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJNODE *pObjTree, const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect, float fDisPOW2 );
	static void DrawRePlacePiece_ALPHA( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJNODE *pObjTree, const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect );
	...
	static void DrawRePlacePieceNode_ALPHA( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJNODE *pNode, const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect );

	static void GetCollisionLineInTriListExceptY ( DXPIECEOBJNODE *pNode, const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, std::vector<COLLISION::TRIANGLE> &TriList ); //주병진 추가 메소드
