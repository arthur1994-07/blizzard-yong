...
void DxReplaceContainer::MakeAABBTree(
    LPDXPIECEOBJ* ppArray,
    DWORD dwArraySize,
    LPDXPIECEOBJNODE& pTreeNode,
...
HRESULT DXREPLACEPIECE::Load(const std::string& FileName, LPDIRECT3DDEVICEQ pd3dDevice, boost::function<void (const TCHAR*)> pArchive)
{
	HRESULT hr=S_OK;
	...
	StringCchCopy(DXREPLACEPIECE::szFileName, nStrLen, FileName.c_str());

	char szFullName[MAX_PATH] = "";
	StringCchCopy(szFullName, MAX_PATH, DxReplaceContainer::GetInstance().GetPath() );
	StringCchCat (szFullName, MAX_PATH, FileName.c_str());
	...
	if ( SFile.OpenFile ( FOT_READ, szFullName ) )
	{
		...
		if ( bExist )
		{
			...
			DXREPLACEPIECE::pFrameRoot->LoadFile_OLD( SFile, pd3dDevice, DXREPLACEPIECE::pEffectHead, DXREPLACEPIECE::pAphaMapFrameHead );
		}
		...
		if ( bExist )
		{
			...
			DXREPLACEPIECE::pAniManHead->LoadFile_OLD ( SFile, pd3dDevice );
		}
		...
		if ( bExist )
		{
			...
			if( DXREPLACEPIECE::vMin.z > pPos->z )	DXREPLACEPIECE::vMin.z = pPos->z;
		}
	}
	else
	{
...
HRESULT DXREPLACEPIECE::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX* pMatWorld, const CLIPVOLUME* pCV, float fAlpha )
{
	GASSERT(pd3dDevice);
	GASSERT(pMatWorld);
	...
	else if ( m_pStaticMesh )
	{
		...
		//{
			pd3dDevice->SetTransform ( D3DTS_WORLD, pMatWorld );
			m_pStaticMesh->Render_PIECE_( pd3dDevice, NULL, (*pMatWorld), D3DXVECTOR2(0.f,0.f), 1.f, NSMaterialManager::EMRM_TOOL, NULL, NULL, FALSE );

			DxRenderStates::GetInstance().SetOnAlphaMap ( pd3dDevice );
			m_pStaticMesh->Render_PIECE_Alpha( pd3dDevice, NULL, (*pMatWorld), D3DXVECTOR2(0.f,0.f), 1.f, NSMaterialManager::EMRM_TOOL, NULL, NULL, FALSE );
...
void DXREPLACEPIECE::Render_ALPHA ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX* pMatWorld, const CLIPVOLUME* pCV, float fAlpha )
{
//	GASSERT(pd3dDevice);
//	GASSERT(pMatWorld);
	...
	else if ( m_pStaticMesh )
	{
		...
		//{
			...
			m_pStaticMesh->Render_PIECE_SoftAlpha( pd3dDevice, NULL , (*pMatWorld), D3DXVECTOR2(0.f,0.f), 1.f, NSMaterialManager::EMRM_TOOL, NULL, NULL, FALSE );
...
void DXREPLACEPIECE::GetCollisionLineInTriListExceptY ( D3DXMATRIX* pMatWorld, const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, std::vector<COLLISION::TRIANGLE> &TriList ) // 주병진 추가 메소드
{
	GASSERT(pMatWorld);
	...
	if ( pFrameRoot )
	{
		GetCollisionLineInTriListExceptYFrame( pFrameRoot, vStart, vEnd, TriList );
	}
	else if ( m_pStaticMesh )
	{	
		UINT32 nPreTriSize = static_cast<UINT32> (TriList.size());

		///////////////////////////////
		// Note : 라인의 좌표를 로컬로 변환한다
		...
		m_pStaticMesh->GetCollisionLineInTriListExceptY( vNewStart, vNewEnd, TriList );

		///////////////////////////////
		// Note : TriList를 다시 Transform 한다

		UINT32 nNowTriSize = static_cast<UINT32> (TriList.size());

		if ( nPreTriSize < nNowTriSize )
		{
			std::vector<COLLISION::TRIANGLE>::iterator iterList = TriList.begin() + nPreTriSize;
			//for ( ; iterList!=TriList.end(); ++iterList )
			//{
			//	(*iterList).Functor_TriangleTransform( *pMatWorld );
			//}

			//for_each(iterList, TriList.end(), COLLISION::TRIANGLEFunctor_TriangleTransform()((*pMatWorld)));
			for_each( iterList, TriList.end(), boost::bind(COLLISION::TRIANGLE::Functor_TriangleTransform, _1, *pMatWorld) );
		}
	}
}

void DXREPLACEPIECE::GetCollisionLineInTriListExceptYFrame ( DxFrame* pframeCur, const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, std::vector<COLLISION::TRIANGLE> &TriList )
{
	if ( !pframeCur || !pframeCur->pEffect )
	{
		return;
	}
	else
	{
		...
		D3DXMATRIX matInverse;
		D3DXMatrixInverse( &matInverse, NULL, &pframeCur->matCombined );
		...
		while ( pmsMeshs != NULL )
		{
			if ( pmsMeshs->m_pDxOctreeMesh )
			{
				pmsMeshs->m_pDxOctreeMesh->GetCollisionLineInTriListExceptY( vNewStart, vNewEnd, TriList );
			}
		...
		if ( nPreTriSize < nNowTriSize )
		{
			...
			//for ( ; iterList!=TriList.end(); ++iterList )
			//{
			//	(*iterList).Functor_TriangleTransform( pframeCur->matCombined );
			//}

			//for_each(iterList, TriList.end(), COLLISION::Functor_TriangleTransform(pframeCur->matCombined));
			for_each( iterList, TriList.end(), boost::bind(COLLISION::TRIANGLE::Functor_TriangleTransform, _1, pframeCur->matCombined) );
		}
		
		if ( pframeCur->pframeSibling )			GetCollisionLineInTriListExceptYFrame ( pframeCur->pframeSibling,    vStart, vEnd, TriList );
		if ( pframeCur->pframeFirstChild )		GetCollisionLineInTriListExceptYFrame ( pframeCur->pframeFirstChild, vStart, vEnd, TriList );
	}
}
...
void DXPIECEOBJ::CheckPickAlpha( const D3DXVECTOR3& vMax, const D3DXVECTOR3& vMin, float fLength, float fElapsedTime )
{
	...
	if( fLength > (fTempLength - m_fLengthPOW2) )
	{
		...
		{
			if( m_fAlpha > 0.24f )	m_fAlpha -= fElapsedTime*2.7f;
			else					m_fAlpha = 0.24f;
...
void DxPieceQuickSort2::RenderPickAlpha( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	for( int i=(int)(m_dwPieceCount-1); i>=0; --i )
	{
		...
		DWORD dwColor = static_cast<DWORD>(pPiece->m_fAlpha * 255.f);
		dwColor = dwColor << 24;
		dwColor += 0xffffff;
...
void DxPieceQuickSort2::InsertData( DXPIECEOBJ* pPiece, const D3DXVECTOR3& vFromPt )
{
	...
	while( pCur )
	{
		// Array 사이즈보다 많아져서 10개씩 Array를 늘인다.
		if ( m_dwArraySize <= m_dwPieceCount )
		{
			PIECEDISTANCE* pArrayPiece = new PIECEDISTANCE[m_dwArraySize * 2];
			memcpy( pArrayPiece, m_pArrayPiece, sizeof( PIECEDISTANCE ) * m_dwArraySize );
			
			SAFE_DELETE_ARRAY( m_pArrayPiece ); 
			m_pArrayPiece = pArrayPiece;
			m_dwArraySize = m_dwArraySize * 2;
		}
...
DXREPLACEPIECE* DxReplaceContainer::FindPice(const std::string& FileName)
{
	REPLACEMAP_ITER iter = m_mapReplace.find(FileName);
	if (iter != m_mapReplace.end())
        return iter->second;
    else
	    return NULL;
}

DXREPLACEPIECE* DxReplaceContainer::LoadPiece(const std::string& FileName)
{
	HRESULT hr=S_OK;
	GASSERT(m_pd3dDevice);

	DXREPLACEPIECE* pPiece = FindPice(FileName);
	if (pPiece)
	{
		pPiece->AddRef();
		return pPiece;
	...
	hr = pNewPiece->Load(FileName, m_pd3dDevice);
	if (FAILED(hr))
	{
	...
	m_mapReplace.insert(REPLACEMAP_VALUE(std::string(pNewPiece->szFileName), pNewPiece));

	return pNewPiece;
}

void DxReplaceContainer::ReleasePiece(const std::string& FileName)
{
	REPLACEMAP_ITER iter = m_mapReplace.find(FileName);
	if (iter == m_mapReplace.end())
        return;
...
HRESULT DxReplaceContainer::FrameMove ( float fTime, float fETime )
{
	...
	// Note : 
	m_sQuickSort.SetArrayMaxSize( static_cast<DWORD>( m_mapReplace.size() ) );
...
HRESULT DxReplaceContainer::DrawRePlacePieceNode(
    LPDIRECT3DDEVICEQ pd3dDevice,
    DXPIECEOBJNODE* pNode, 
    const CLIPVOLUME* pCV,
    EMREFLECTOPTION emReflect,
    float fDisPOW2)
{
	...
	if (m_bRendAABBB)
        EDITMESHS::RENDERAABB ( pd3dDevice, pNode->vMax, pNode->vMin );	// Test AABB Render
...
void DxReplaceContainer::DrawRePlacePieceNode_ALPHA( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJNODE *pNode, const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect )
{
...
HRESULT DxReplaceContainer::DrawRePlacePiece ( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJ *pObjList, DXPIECEOBJNODE *pObjTree, 
												const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect, float fDisPOW2 )
{
	m_dwRendPiece = 0;
...
void DxReplaceContainer::DrawRePlacePiece( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJNODE *pObjTree, const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect, float fDisPOW2 )
{
...
void DxReplaceContainer::DrawRePlacePiece_ALPHA( LPDIRECT3DDEVICEQ pd3dDevice, DXPIECEOBJNODE *pObjTree, const CLIPVOLUME *pCV, EMREFLECTOPTION emReflect )
{
    if( !GLOBAL_PARAM::IsValue( GP_PIECE ) )
		return;
...
void DxReplaceContainer::GetCollisionLineInTriListExceptY ( DXPIECEOBJNODE *pNode, const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, std::vector<COLLISION::TRIANGLE> &TriList ) //주병진 추가 메소드
{
	if ( !pNode ) return;
	...
	if ( pPieceObjCur )
	{
		if ( pPieceObjCur->pReplacePiece )
		{
			pPieceObjCur->pReplacePiece->GetCollisionLineInTriListExceptY ( &pPieceObjCur->matComb, vStart, vEnd, TriList ); 
		}

		return;
	}

	GetCollisionLineInTriListExceptY( pNode->pLeftChild,  vStart, vEnd, TriList );
	GetCollisionLineInTriListExceptY( pNode->pRightChild, vStart, vEnd, TriList );
}
