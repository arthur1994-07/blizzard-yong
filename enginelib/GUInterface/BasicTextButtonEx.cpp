...
void CBasicTextButton::CreateBaseButton(
    const char* szButtonControl, 
    int nType, 
    WORD wFlipMode, 
    const char* szText,
    DWORD dwFlags )
{
	...
	CD3DFontPar* pFont = m_pEngineDevice->LoadFont( _DEFAULT_FONT, 9, dwFlags );
...
void CBasicTextButton::CreateButtonImage ( int nType, CD3DFontPar* pFont, BOOL bReversal )
{
	...
	const int nSIZENUMBER[SIZE_NSIZE] = { 14, 16, 161, 18, 19, 191, 22, 40 };
	...
	{	//	라디오버튼과 아닌것에 따른 플립
		...
		m_pButton = new CBasicTextButtonImage(m_pEngineDevice);
	...
	m_pButtonFlip = new CBasicTextButtonImage(m_pEngineDevice);
	...
	m_pButtonText = new CBasicTextBox(m_pEngineDevice);
	...
	m_pButtonTextBack = new CUIControl(m_pEngineDevice);
