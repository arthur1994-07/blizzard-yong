...
CBasicAnimationBox::CBasicAnimationBox(EngineDeviceMan* pEngineDevice)
...
void CBasicAnimationBox::CreateAni( const char *keyvalue, DWORD AniType )
{
	...
	CUIControl* pAniControl = new CUIControl(m_pEngineDevice);
...
void CBasicAnimationBox::SetAniDiffuse(const D3DXCOLOR& d3dColor)
{
	...
	if ( m_TotElapsedTime < m_fEndTime)
	{
		...
		if ( m_TotElapsedTime - m_fLastRenderTime > m_fAniTime )
		{
				...
				{	
					...
					if( m_dwRenderPic >= m_pAniControl.size() )
					{
						if( m_bLoop )
						{
							...
							m_dwRenderPic = static_cast<DWORD> ( m_pAniControl.size()-1 );
						}
					}
				}
				break;
			...
			case BACK_ANI:
				{
					m_dwRenderPic--;
					//if (m_dwRenderPic < 0) // always false
                    //    m_dwRenderPic = (DWORD) m_pAniControl.size() - 1;
				}
				break;
			}
