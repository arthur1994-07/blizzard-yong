...
BOOL CUIMan::UpdateList( CUIFocusContainer* pList, BOOL bFocusList, float fElapsedTime, ITranslateUIMsg* pInterface )
{
    GASSERT( pList );

	if( NSGUI::IsCharMoveAllBlock() && !bFocusList )
		return TRUE;
    ...
    for ( ; riter != riter_end; ++riter )
    {
        ...
        if ( bFirstControl )
        {
            ...
            // MEMO : 이 함수를 호출한 Interface에서 호출한다.
            pInterface->TranslateUIMessage( cID, dwMsg );
...
HRESULT CUIMan::FrameMove( float fElapsedTime, ITranslateUIMsg* pInterface )
{
    ...
    if( !UpdateList( &m_FocusList, TRUE, fElapsedTime, pInterface ) )	return E_FAIL;
    if( !UpdateList( &m_BottomList, FALSE, fElapsedTime, pInterface ) )	return E_FAIL;
    if( !UpdateList( &m_TopList, FALSE, fElapsedTime, pInterface ) )	return E_FAIL;
	if( !UpdateList( &m_UpdateList, TRUE, fElapsedTime, pInterface ) )	return E_FAIL;
