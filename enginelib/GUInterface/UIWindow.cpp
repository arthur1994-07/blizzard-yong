...
CUIWindow::CUIWindow (EngineDeviceMan* pEngineDevice) 
...
void CUIWindow::CreateTitle ( char* szFrame, char* szLeft, char* szMid, char* szRight, char* szTextBox, const char* szText )
{
	...
	CD3DFontPar* pFont = m_pEngineDevice->LoadFont( _DEFAULT_FONT, FONTSIZE, _DEFAULT_FONT_SHADOW_EX_FLAG );

	WORD wAlignFlag = UI_FLAG_XSIZE;

	m_pTitle = new CUIWindowTitle(m_pEngineDevice);
...
void CUIWindow::CreateTitleFocus ( char* szFrame, char* szLeft, char* szMid, char* szRight, char* szTextBox, const char* szText )
{
	...
	m_pTitleFocus = new CUIWindowTitle(m_pEngineDevice);
...
{	
	...
	m_pBody = new CUIWindowBody(m_pEngineDevice);
