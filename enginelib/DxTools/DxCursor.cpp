...
void DxCursor::Render( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	//if( DxDynamicVB::g_sVB_PDT.nOffsetToLock + dwVertexSizeFULL > DxDynamicVB::g_sVB_PDT.nFullByte )
	//{
	//	dwFlag = D3DLOCK_DISCARD; 
	//	DxDynamicVB::g_sVB_PDT.nVertexCount = 0;
	//	DxDynamicVB::g_sVB_PDT.nOffsetToLock = 0; 
	//}

	//VERTEXRHW *pVertices;
	//hr = DxDynamicVB::g_sVB_PDT.pVB->Lock ( DxDynamicVB::g_sVB_PDT.nOffsetToLock, dwVertexSizeFULL, (VOID**)&pVertices, dwFlag );
	//if ( FAILED(hr) )	return;
	...
	//DxDynamicVB::g_sVB_PDT.pVB->Unlock ();
 ...
 //
	...
	//pd3dDevice->SetStreamSource ( 0, DxDynamicVB::g_sVB_PDT.pVB, 0, sizeof(VERTEXRHW) );
	//pd3dDevice->SetTexture( 0, m_pTexture );

	//pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, DxDynamicVB::g_sVB_PDT.nVertexCount, 2 );

	//m_pSavedSB->Apply();

	//// Note : 값을 더해줌. 
	//DxDynamicVB::g_sVB_PDT.nVertexCount += 4;
	//DxDynamicVB::g_sVB_PDT.nOffsetToLock += dwVertexSizeFULL;
}
