...
void DxSkyCloud::InitData( float fRotate, float fRotateSUM, float fWidthRate, float fHeightRate, const char* pName )
{
	...
	{
		m_fRotateSUM = D3DX_PI*2.f*sc::Random::RANDOM_POS();
	}
	else
	{
...
void DxSkyCloud::RenderReflectMaterial( LPDIRECT3DDEVICEQ pd3dDevice, const D3DXMATRIX& matBase )
{
	pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, DxFogMan::GetInstance().GetFogWhiteColor() );
...
void DxSkyCloudMan::RenderReflectMaterial( LPDIRECT3DDEVICEQ pd3dDevice, const D3DXMATRIX& matBase )
{
	if( !m_pSkyCloud )	return;
	...
	for( DWORD i=0; i<CLOUD_NUM; ++i )
	{
		m_pSkyCloud[i].RenderReflectMaterial( pd3dDevice, matBase );
	}
