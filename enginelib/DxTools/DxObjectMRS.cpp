...
void DxObjectMRS::Render( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if( !( dwLButton&(DXKEY_PRESSED|DXKEY_DRAG) ) )
	{
		// 누르고 있을 경우는 업데이트를 하지 않는다.
		m_matBackUp = *m_pmatWorld;
		m_vSnapMove = D3DXVECTOR3( 0.f, 0.f, 0.f );
...
BOOL DxObjectMRS::IsCollisionLine( D3DXVECTOR2 vPoint, D3DXVECTOR3 vP1_T, D3DXVECTOR3 vP2_T, float fLength ) const
{
	D3DXMATRIX	matIdentity;
	D3DXMatrixIdentity( &matIdentity );
