...
HRESULT DxRenderStates::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	for( which=0; which<2; which++ )
	{
		...
		// [shhan][2013.07.02] intel 칩에서 정상작동을 하지 않는다.
		//						Z 버퍼랑 연결이 잘 되지 않는거 같다.
		//						2Pass 로 돌리면, 렌더링 되지 않는 부분이 생겨서 colorclear 색이 나온다.
		//if( dwMinFilter == D3DTEXF_ANISOTROPIC )
		//{
		//	pd3dDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, dwMinFilter );
		//	pd3dDevice->SetSamplerState( 0, D3DSAMP_MIPFILTER, D3DTEXF_NONE );
...
{
	// [shhan][2014.07.17]
	// 여기서 에러가 뜬다는건 다른곳에서 Device 관련 Release 나 혹은 뭔가를 잘 못 사용하고 있는 것이다.
	// 이 부분을 의심할 필요는 없다.
	// 실제로 다른곳에서 Release 를 안했더니 여기서 에러가 떴다.
	SAFE_RELEASE( pBLOCK );
}
