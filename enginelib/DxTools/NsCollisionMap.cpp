...
void DxFaceTree::Save ( sc::SerialFile& SFile )
{
	SFile << vMax;
	SFile << vMin;
...
void DxFaceTree::Load ( sc::SerialFile& SFile )
{
	CleanUp();
...
BOOL DxAABBCollision::IsCollisionLine ( const D3DXVECTOR3& vP1, const D3DXVECTOR3& vP2, D3DXVECTOR3 &vColl, D3DXVECTOR3 &vNor, WORD& wTexIndex ) const
{
	BOOL bColl = FALSE;
...
void DxAABBCollision::Save ( sc::SerialFile& SFile )
{
	SFile << m_vMax;
	SFile << m_vMin;
...
void DxAABBCollision::Load ( sc::SerialFile& SFile )
{
	CleanUp();
...
_RETURN:
	...
	void MakeStaticMesh1 ( DxMeshes* pMesh, D3DXMATRIX& matWorld, LPD3DXVECTOR3& pVertex, LPWORD& pIndex, DWORD& dwFace, MAPTEXINDEX& mapTexIndex )
	{
		...
		for ( DWORD i=0; i<dwAttibTableSize; ++i )
		{
			...
			StringCchCopy( szName, MAX_PATH, pMesh->m_strTextureFiles[dwAttribID].GetString() );
	...
	void SaveTree ( sc::SerialFile& SFile, DxFaceTree* pTree )
	{
		if ( pTree )
		{
			SFile << (BOOL)TRUE;
	...
	void LoadTree ( sc::SerialFile& SFile, PDXFACETREE& pTree )
	{
		BOOL bUse = TRUE;
		SFile >> bUse;
