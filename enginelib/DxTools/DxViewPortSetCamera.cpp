...
void DxViewPort::FrameMoveSpecCam( float fElapsedTime )
{
	...
	if ( bHandling && m_dwFlag&DXVP_USER )
	{
		...
		DWORD dwKEY_W(DXKEY_IDLE);	// 앞
		DWORD dwKEY_S(DXKEY_IDLE);	// 뒤
		DWORD dwKEY_A(DXKEY_IDLE);	// 좌
		DWORD dwKEY_D(DXKEY_IDLE);	// 우
		DWORD dwKEY_E(DXKEY_IDLE);	// 상
		DWORD dwKEY_Q(DXKEY_IDLE);	// 하
