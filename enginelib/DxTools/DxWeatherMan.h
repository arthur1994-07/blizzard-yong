...
struct	CLOUD_PROPERTY_100	//	데이터 포맷 변경 진행
{
	bool			m_bUse;
	float			m_fWidth;
	char			m_szTexture[MAX_PATH];
...
struct WEATHER_PROPERTY_102	//	데이터 포맷 변경 진행
{
	DWORD				m_dwFlag;
...
public:
	static DWORD LoadSetProp ( sc::SerialFile &SFile, WEATHER_PROPERTY &sProperty );

public:
	void SaveSet ( sc::SerialFile &SFile );
	void LoadSet ( sc::SerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice );

	void LoadSet_101 ( sc::SerialFile &SFile );
	void LoadSet_102 ( sc::SerialFile &SFile );
	void LoadSet_103 ( sc::SerialFile &SFile );	//	데이터 포맷 변경 진행
