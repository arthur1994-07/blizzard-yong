...
{
	if( !pCameraAni->m_szName )
		return;

	DWORD dwSize;

	dwSize = static_cast< DWORD >( sizeof(char) * (strlen(pCameraAni->m_szName)+1) );
	...
	dwSize = static_cast< DWORD >( sizeof(char) * (strlen(pCameraAni->m_szFromCamera)+1) );
	...
	if ( pCameraAni->m_emCamera==CAMERA_TTARGET )
	{
		...
		dwSize = static_cast< DWORD >( sizeof(char) * (strlen(pCameraAni->m_szTargetCamera)+1) );
...
HRESULT DxCameraAni::SaveSet ( sc::SerialFile &SFile )
{
	int nNameSize;
	int nFromCaSize;
	int nTargetCaSize;

	if ( m_szName )			nNameSize = static_cast< int >( sizeof(char) * (strlen(m_szName)+1) );
	else					nNameSize = 0;

	if ( m_szFromCamera )	nFromCaSize = static_cast< int >( sizeof(char) * (strlen(m_szFromCamera)+1) );
	else					nFromCaSize = 0;

	if ( m_szTargetCamera )	nTargetCaSize = static_cast< int >( sizeof(char) * (strlen(m_szTargetCamera)+1) );
...
HRESULT DxCameraAni::LoadSet_101 ( sc::SerialFile &SFile, DxFrameMesh *pFrameMesh )
{
	DWORD dwLength;
...
HRESULT DxCameraAni::LoadSet_102 ( sc::SerialFile &SFile, DxFrameMesh *pFrameMesh )
{
	DWORD dwLength;
...
HRESULT DxCameraAni::LoadSet ( sc::SerialFile &SFile, DxFrameMesh *pFrameMesh )
{
	DWORD dwVer, dwSize;
	...
	switch ( dwVer )
	{
	case 0x0100:	LoadSet_101( SFile, pFrameMesh );	break;
	case 0x0101:	LoadSet_101( SFile, pFrameMesh );	break;
	case VERSION:	LoadSet_102( SFile, pFrameMesh );	break;
	default:
		{
...
HRESULT DxCameraAni::Save ( sc::SerialFile &SFile )
{
	int nTotalSize = 0;
	int nStringSize;
	...
	if ( m_szName )			nStringSize = static_cast< int >( strlen(m_szName)+1 );
	else					nStringSize = 0;
	...
	if ( m_szFromCamera )	nStringSize = static_cast< int >( strlen(m_szFromCamera)+1 );
	else					nStringSize = 0;
	...
	if ( m_szTargetCamera )	nStringSize = static_cast< int >( strlen(m_szTargetCamera)+1 );
	else					nStringSize = 0;
...
HRESULT DxCameraAni::Load_101 ( sc::SerialFile &SFile )
{
	DWORD dwLength;
	SFile >> dwLength;
	if ( dwLength )
...
HRESULT DxCameraAni::Load_102 ( sc::SerialFile &SFile )
{
	DWORD dwLength;
	SFile >> dwLength;
	if ( dwLength )
...
HRESULT DxCameraAni::Load ( sc::SerialFile &SFile )
{
	DWORD dwVer, dwSize;
	...
	switch( dwVer )
	{
	case 0x0100:	Load_101( SFile );	break;
	case 0x0101:	Load_101( SFile );	break;
	case VERSION:	Load_102( SFile );	break;
	default:
		{
			SFile.SetOffSet ( SFile.GetfTell() + dwSize );
			return S_FALSE;	
...
HRESULT DxCamAniMan::SaveSet ( sc::SerialFile &SFile )
{
	DxCameraAni* pCur;
...
HRESULT DxCamAniMan::LoadSet ( sc::SerialFile &SFile, DxFrameMesh *pFrameMesh )
{
	SAFE_DELETE(m_pAniList);
...
HRESULT DxCamAniMan::Save ( sc::SerialFile &SFile )
{
	DxCameraAni* pCur;
	DWORD dwNumCameraAni = 0;
...
HRESULT DxCamAniMan::Load ( sc::SerialFile &SFile )
{
	SAFE_DELETE(m_pAniList);
	...
	case CAMERA_TFREE:
		{
			...
			D3DXVec3TransformNormal ( &vUpVec, &vUpVec, pmatCur );
