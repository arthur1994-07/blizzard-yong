...
HRESULT DxGlowMan::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_sVert_Color_256[0].dwColor = m_sVert_Color_256[1].dwColor = m_sVert_Color_256[2].dwColor = m_sVert_Color_256[3].dwColor = 0xfffffff;
	...
	m_sVert_512[2].vPos = D3DXVECTOR4 ( 0.f,	512.f,	1.f, 1.f );
	m_sVert_512[3].vPos = D3DXVECTOR4 ( 512.f,	512.f,	1.f, 1.f );
...
void DxGlowMan::RenderTex ( const LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !DxSurfaceTex::GetInstance().m_pGlowSuf_TEMP )
		return;

	if ( !DxSurfaceTex::GetInstance().m_pGlowSuf )
		return;
...
void DxGlowMan::TexRender2( const LPDIRECT3DDEVICEQ pd3dDevice, const D3DXVECTOR2& vOffSet1, const D3DXVECTOR2& vOffSet2, const float fRate )
{
	...
	pd3dDevice->SetFVF( VERTEXRHW::FVF );
	pd3dDevice->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, m_sVert_512, sizeof(VERTEXRHW) );
...
void DxGlowMan::GlowRender_Burn( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !DxSurfaceTex::GetInstance().m_pTempSuf256 )
		return;

	if ( !DxSurfaceTex::GetInstance().m_pBurnResultSuf )
		return;
...
//	DWORD	dwValueUp	= (DWORD)(255.f*(0.9f+(sc::Random::RANDOM_POS()*0.1f))*m_fReminedTime);
	DWORD	dwValueUp;
	if( m_fElapsedTime>0.1f )		dwValueUp = (DWORD)(255.f*(0.70f+(sc::Random::RANDOM_POS()*0.30f)));
	else if( m_fElapsedTime>0.05f )	dwValueUp = (DWORD)(255.f*(0.81f+(sc::Random::RANDOM_POS()*0.19f)));
	else							dwValueUp = (DWORD)(255.f*(0.89f+(sc::Random::RANDOM_POS()*0.11f)));
...
void DxGlowMan::TexOneRender( LPDIRECT3DDEVICEQ pd3dDevice, VERTEXCOLORRHW* sVertices, float fMicro, DWORD dwColor, int nPixelW, int nPixelH )
{
	...
	sVertices[0].dwColor = sVertices[1].dwColor = sVertices[2].dwColor = sVertices[3].dwColor = dwColor;
	...
	pd3dDevice->DrawPrimitiveUP ( D3DPT_TRIANGLESTRIP, 2, sVertices, sizeof(VERTEXCOLORRHW) );

	m_pSavedBurnSB->Apply();
}

void DxGlowMan::TexOneRender( LPDIRECT3DDEVICEQ pd3dDevice, VERTEXRHW* sVertices )
{
	...
	pd3dDevice->DrawPrimitiveUP ( D3DPT_TRIANGLESTRIP, 2, sVertices, sizeof(VERTEXRHW) );
