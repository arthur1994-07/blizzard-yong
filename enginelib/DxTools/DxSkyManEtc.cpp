...
void DxSkyMan::LightingCheckReset ()
{
	...
	m_vLightingPos	= D3DXVECTOR3 ( (sc::Random::RANDOM_NUM()+(vDirect.x*3.f))*2000.f, vLookatPt.y+1400.f, (sc::Random::RANDOM_NUM()+(vDirect.z*3.f))*2000.f );
	m_cLightingColor = (((DWORD)(sc::Random::RANDOM_POS()*255.f))<<16) + (((DWORD)(sc::Random::RANDOM_POS()*150.f))<<8) + (DWORD)(sc::Random::RANDOM_POS()*255.f);
...
void DxSkyMan::LightingRender ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	DxBillboardLookAtHeight ( matLocal, &m_vLightingPos, DxViewPort::GetInstance().GetMatView() );
	...
	DWORD dwAlpha = (DWORD)( ((sc::Random::RANDOM_POS()*0.5f)+0.5f)*255.f );
	...
	pd3dDevice->SetTexture ( 0, m_textureResLighting.GetTexture() );
...
void DxSkyMan::ThunderRender ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXVECTOR3 vPos, float fScale, float fBase, float fFact )
{
	...
	float fAlpha = fBase + (sc::Random::RANDOM_POS()*fFact);
	...
	pd3dDevice->SetTexture ( 0, m_textureResThunderC.GetTexture() );
	pd3dDevice->SetTexture ( 1, m_textureResThunder.GetTexture() );
...
void DxSkyMan::ThundersRender ( LPDIRECT3DDEVICEQ pd3dDevice, LIGHTING_OPTION vLighting )
{
	...
	float		fRainRate	= DxWeatherMan::GetInstance()->GetRain()->GetApplyRate();
	D3DXMATRIX	matLocal;
	DxBillboardLookAt ( matLocal, &vLighting.vPos, DxViewPort::GetInstance().GetMatView() );
	...
	pd3dDevice->SetTexture ( 0, m_textureResThunders.GetTexture() );
...
{
    m_textureResCloud = TextureManager::GetInstance().LoadTexture(
        m_szCloudTex,
        false,
    ...
    m_textureResCloudShadow = TextureManager::GetInstance().LoadTexture(
        m_szCloudShadowTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
 ...
 //   TextureManager::GetInstance().BackUpCache( m_textureResCloud );
	//TextureManager::GetInstance().BackUpCache( m_textureResCloudShadow );
...
HRESULT CCloud::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::RestoreDeviceObjects() -- CreateVertexBuffer() -- Failed"));
		return E_FAIL;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::RestoreDeviceObjects() -- m_pCloudVB->Lock() -- Failed"));
		return E_FAIL;
	}
	...
	float fHana = (sc::Random::RANDOM_POS()*2.f)+1.f;		// 1.f~3.f;	구름을 길게 늘이는데 사용하는 값이다.
	fHana = 1.f;
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::RestoreDeviceObjects() -- CreateIndexBuffer() -- Failed"));
		return E_FAIL;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::RestoreDeviceObjects() -- m_pCloudIB->Lock() -- Failed"));
		return E_FAIL;
	}
...
{
	m_textureResCloud.Clear();
	m_textureResCloudShadow.Clear();
...
void CCloud::CloudRender_Reflect ( LPDIRECT3DDEVICEQ pd3dDevice, CLOUD_OPTION& sCloud )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::CloudRender_Reflect() -- m_pCloudVB->Lock() -- Failed"));
		return;
	}
	...
	pd3dDevice->SetTexture ( 0, m_textureResCloudShadow.GetTexture() );
...
void CCloud::CloudRender_Base ( LPDIRECT3DDEVICEQ pd3dDevice, CLOUD_OPTION& sCloud )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::CloudRender_Base() -- m_pCloudVB->Lock() -- Failed"));
		return;
	}
...
void CCloud::CloudRender_Part ( LPDIRECT3DDEVICEQ pd3dDevice, CLOUD_OPTION& sCloud, D3DXVECTOR2* pTex )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::CloudRender_Part() -- m_pCloudVB->Lock() -- Failed"));
		return;
	}
...
//	dwColor += ((DWORD)(255.f*(1.f-m_fWeatherRate))) <<24;				// 중앙만 특별하다.
	...
	pd3dDevice->SetTexture ( 0, m_textureResCloud.GetTexture() );
...
void CCloud::CloudShadowRender_Part ( LPDIRECT3DDEVICEQ pd3dDevice, CLOUD_OPTION& sCloud, D3DXVECTOR2* pTex, DWORD dwColor )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::CloudShadowRender_Part() -- m_pCloudVB->Lock() -- Failed"));
		return;
	}
	...
	pd3dDevice->SetTexture ( 0, m_textureResCloudShadow.GetTexture() );		// 텍스쳐 설정
...
//	dwColor = GetSkyColor ();
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("CCloud::CloudyRender_Part() -- m_pCloudVB->Lock() -- Failed"));
		return;
	}
