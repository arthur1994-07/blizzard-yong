...
HRESULT DxCubeMap::InitDeviceObjects ( LPDIRECT3DQ pD3D, LPDIRECT3DDEVICEQ pd3dDevice, D3DSURFACE_DESC &d3dsdBackBuffer )
{
	...
	if ( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxCubeMap::InitDeviceObjects() -- CreateVertexBuffer() -- Failed"));
		return hr;
	}
	...
	if ( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxCubeMap::InitDeviceObjects() -- Lock() -- Failed"));
		return hr;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxCubeMap::InitDeviceObjects() -- CreateIndexBuffer() -- Failed"));
		return hr;
	}
    ...
    m_textureResSkyBox = TextureManager::GetInstance().LoadTexture(
        m_szSkyBoxTex,
        false,
    ...
    m_textureResLight = TextureManager::GetInstance().LoadTexture(
        m_szLightTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResTest = TextureManager::GetInstance().LoadTexture(
        "LobbyCube.dds",
        false,
 ...
 //   TextureManager::GetInstance().BackUpCache( m_textureResSkyBox );
	//TextureManager::GetInstance().BackUpCache( m_textureResLight );
 //   TextureManager::GetInstance().BackUpCache( m_textureResTest );
...
HRESULT DxCubeMap::DeleteDeviceObjects ()
{
	...
	m_textureResSkyBox.Clear();
    m_textureResLight.Clear();
    m_textureResTest.Clear();
...
void	DxCubeMap::RenderLight ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX& matView )
{
	...
	DxBillboardLookAt ( matLocal, &vPos, &vFromPt, &vUp );
	matLocal._41 = vPos.x;
	matLocal._42 = vPos.y;
	matLocal._43 = vPos.z;
	...
	pd3dDevice->SetTexture ( 0, m_textureResLight.GetTexture() );	
...
void	DxCubeMap::RenderSkybox ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	pd3dDevice->SetTexture ( 0, m_textureResSkyBox.GetTexture() );
