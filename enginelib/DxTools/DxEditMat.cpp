	...
	m_pAffineMatrix(NULL)
{
	...
	m_pCurMesh = new DxFrameMesh(TRUE);
}

DxEditMat::~DxEditMat(void)
{
	SAFE_DELETE( m_pCurMesh );
}

void DxEditMat::SetParam(const std::string& Path, HWND hWnd)
{
	m_hWnd = hWnd;
	m_DefaultPath = Path;
}
...
HRESULT DxEditMat::InitDeviceObjects(LPDIRECT3DDEVICEQ pd3dDevice)
{
	...
	m_pCurMesh->SetFile( m_DefaultPath.c_str(), "3dCur.X" );
	hr = m_pCurMesh->Create(pd3dDevice);
	if (FAILED(hr))
	{
		char szMsg[256] = "";
		StringCchPrintf(szMsg, 256, "%s 를 불러오는데 실패하였습니다.", "3dCur.X" );
...
HRESULT DxEditMat::FrameMove ( float fTime, float fElapsedTime )
{
	...
	m_pCurMesh->FrameMove ( fTime, fElapsedTime );
	...
	{
		m_pCurMesh->IsCollision_NoneConst( vFromPt, vTargetPt, NULL, NULL, EMCC_CULL_NONE );

		//	Note : 선택된 프래임을 트리콘트롤에서 찾아 선택.
		//
		m_pDxFrameSel = m_pCurMesh->GetCollisionDetectedFrame();
	}
	...
	{
		m_pDxFrameSel = NULL;
		m_pCurMesh->SetCollisionDetectedFrame ( NULL );
	}
...
HRESULT DxEditMat::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_pCurMesh->Render ( pd3dDevice, &matWorld, &cv );

	//	Note : 커서중에서 선택된 frame을 그린다.
	//
	m_pCurMesh->DrawSelectedFrame ( pd3dDevice, &cv, &matWorld );
