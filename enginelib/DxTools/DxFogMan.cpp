...
void	FOG_PROPERTY::SaveSet ( sc::SerialFile &SFile )
{
	SFile << VERSION;
...
void FOG_PROPERTY::LoadSet_0100 ( sc::SerialFile &SFile )
{
	BOOL	bTemp;
	float	fTemp;
...
void FOG_PROPERTY::LoadSet_0101 ( sc::SerialFile &SFile )
{
	BOOL	bTemp;
	float	fTemp;
...
void FOG_PROPERTY::LoadSet_0102 ( sc::SerialFile &SFile )
{
	BOOL	bTemp;
	float	fTemp;
...
void FOG_PROPERTY::LoadSet_0103 ( sc::SerialFile &SFile )
{
	BOOL	bTemp;
	float	fTemp;
...
void FOG_PROPERTY::LoadSet_0104 ( sc::SerialFile &SFile )
{
	BOOL	bTemp;
	float	fTemp;
...
void FOG_PROPERTY::LoadSet_0105 ( sc::SerialFile &SFile )
{
	float	fTemp;
...
void FOG_PROPERTY::LoadSet_0106 ( sc::SerialFile &SFile )
{
	SFile >> m_bSkyFogEnable;
	SFile >> m_bFogStaticRange;
	SFile >> m_fFogStart;
...
void FOG_PROPERTY::LoadSet_0107 ( sc::SerialFile &SFile )
{
	SFile >> m_bSkyFogEnable;
	SFile >> m_bFogStaticRange;
	SFile >> m_fFogStart;
...
void FOG_PROPERTY::LoadSet_0108 ( sc::SerialFile &SFile )
{
	SFile >> m_bSkyFogEnable;
	SFile >> m_bFogStaticRange;
	SFile >> m_fFogStart;
...
void FOG_PROPERTY::LoadSet_0109 ( sc::SerialFile &SFile )
{
	SFile >> m_bSkyFogEnable;
	SFile >> m_cFogColor;
	SFile >> m_cFogColor_Day;
...
void	FOG_PROPERTY::LoadSet ( sc::SerialFile &SFile )
{
	DWORD	dwVer;
	DWORD	dwBuffSize;
	...
	switch ( dwVer ) 
	{
	case 0x0100:	LoadSet_0100(SFile);	break;
	case 0x0101:	LoadSet_0101(SFile);	break;
	case 0x0102:	LoadSet_0102(SFile);	break;
	case 0x0103:	LoadSet_0103(SFile);	break;
	case 0x0104:	LoadSet_0104(SFile);	break;
	case 0x0105:	LoadSet_0105(SFile);	break;
	case 0x0106:	LoadSet_0106(SFile);	break;
	case 0x0107:	LoadSet_0107(SFile);	break;
	case 0x0108:	LoadSet_0108(SFile);	break;
	case VERSION:	LoadSet_0109(SFile);	break;
	default:
		{
...
DxFogMan::DxFogMan() :
	...
	m_bFogEnable(FALSE),
    m_fFogStart_RealTime(720.0f),
    m_fFogEnd_RealTime(790.0f)
	, m_bPMarketON_OnlyUI(FALSE)
{
}
...
{
	// 강제 셋팅
	if( m_bFogStaticRange && m_emFogRange == EMFR_HIGH )
	{
		return m_fFogEnd;
	}
	...
	{
	case EMFR_LOW:		
		return m_fRangeLow_Far;

	case EMFR_MIDDLE:	
		return m_fRangeMiddle_Far;

	case EMFR_HIGH:		
		return m_fRangeHigh_Far;
...
void DxFogMan::FrameMove()
{
	// m_bPMarketON_OnlyUI 일 경우는 m_emFogRange 를 EMFR_OFF 로 해야 한다.
	// ResizeWindow 일 경우 m_emFogRange 값을 재 셋팅 해버려서 FrameMove 에서 다시 셋팅 할 필요가 있다.
	if ( m_bPMarketON_OnlyUI )
	{
		m_emFogRange = EMFR_OFF;
	}
}

void	DxFogMan::RenderFogSB ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bFORCE_FOG_OFF )
{
	...
	if ( m_bFogStaticRange && m_emFogRange == EMFR_HIGH )					// 강제 셋팅일 경우이다.
	{
		// 시야가 가장 멀때만 작동된다. & 강제 시야 셋팅.
    ...
    if ( bFORCE_FOG_OFF )
    {
        fFogStart = 80000.f;   // 실제 게임에서 적용되는 수치
        fFogEnd = 100000.f;    // 실제 게임에서 적용되는 수치
    }

	DxViewPort::GetInstance().SetFarPlane( fFogEnd );				// View Cliping -> DxViewPort FrameMove Setting
    DxViewPort::GetInstance().SetProjection();              	    // 최대 시아를 제한한다.
    ...
    // 실제 게임에서 적용되는 수치
    m_fFogStart_RealTime = fFogStart;
    m_fFogEnd_RealTime = fFogEnd;

	// 렌더링 되는 값으로 작업이 되어야 한다.
	NSMaterialManager::SetFogParam( fFogStart, fFogEnd );
}

// 게임에서 최대 멀리 보이는 거리
float DxFogMan::GetGameFogHighEnd( const FOG_PROPERTY& sProperty )
{
	if ( sProperty.m_bFogStaticRange )
	{
		// 강제 셋팅일 경우이다.
		return sProperty.m_fFogEnd;
	}
	else if ( sProperty.m_bNewRange )
	{
		// 새로운 Octree 의 경우
		return 2200.f;
	}
...
// PMarket 에서 사용하는 함수
void DxFogMan::BackUpFogRangeForPMarket()
{
	m_bPMarketON_OnlyUI = TRUE;	// ResizeWindow 때문에 필요함.
}

void DxFogMan::RollBackFogRangeForPMarket()
{
	m_bPMarketON_OnlyUI = FALSE;
}