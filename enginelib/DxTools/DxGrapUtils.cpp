...
namespace DxGrapUtils
{
	...
	{
		//if( RENDERPARAM::emSERVICE_TYPE == EMSERVICE_CHINA )
		//{
	...
	HRESULT CaptureScreen ( LPDIRECT3DDEVICEQ pDev )
	{
		...
		for ( int i=0; i<999; i++ )
		{
			...
			else if ( RENDERPARAM::emSERVICE_TYPE == RENDERPARAM::EMSERVICE_FEYA )
			{
		...
		*/
		...
		// Note : SDK가 버전업이 되면서 바로 JPG로 저장이 가능하다. ( 10/05/17 ) 
		//		   D3DXSaveSurfaceToFile(...,D3DXIFF_JPG, ...);
		hr = ScreenGrab( pDev, StrPathName.GetString() );
...
*/
	...
	{ 
		...
		hr = pDev->GetBackBuffer( 0, 0, D3DBACKBUFFER_TYPE_MONO, &surf );

		if (FAILED(hr))
		{ 
            std::string ErrorMsg(
                sc::string::format(
                    "DxGrapUtils::ScreenGrab()-GetBackBuffer() %1%", DXGetErrorString9(hr)));
		...
		*/
		...
		{ 
            std::string ErrorMsg(
                sc::string::format(
                    "DxGrapUtils::ScreenGrab()-SaveFile() %1%, %2%", fileName, DXGetErrorString9(hr)));
			CDebugSet::ToLogFile(ErrorMsg);
			surf->Release();
			return hr; 
		} 
