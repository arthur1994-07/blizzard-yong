...
HRESULT DxEffectRain::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXVECTOR3 vDirec, float fPower )
{
	...
	{
		m_pInforRain[i].vPos.x = vLookatPt.x + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius) + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius);
		m_pInforRain[i].vPos.y = vLookatPt.y + ( rand()%(m_iHeight*2) );
		m_pInforRain[i].vPos.z = vLookatPt.z + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius) + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius);

		m_pInforRain[i].vDirect.x = sc::Random::RANDOM_POS()-0.5f;
		m_pInforRain[i].vDirect.y = -sc::Random::RANDOM_POS();
		m_pInforRain[i].vDirect.z = sc::Random::RANDOM_POS()-0.5f;
	}
	...
	if ( !m_pVB )
	{
        CDebugSet::ToLogFile(std::string("DxEffectRain::InitDeviceObjects() -- CreateVertexBuffer() -- Failed"));
		return S_OK;
	}
	...
	if ( !m_pIB )	
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::InitDeviceObjects() -- CreateIndexBuffer() -- Failed"));
		return S_OK;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::InitDeviceObjects() -- m_pVB->Lock() -- Failed"));
		return E_FAIL;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::InitDeviceObjects() -- m_pIB->Lock() -- Failed"));
		return E_FAIL;
	}
...
HRESULT DxEffectRain::ResetMesh ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !m_pVB )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::ResetMesh() -- CreateVertexBuffer() -- Failed"));	
		return S_OK;
	}
	...
	if ( !m_pIB )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::ResetMesh() -- CreateIndexBuffer() -- Failed"));	
		return S_OK;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::ResetMesh() -- m_pVB->Lock() -- Failed"));
		return E_FAIL;
	}
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::ResetMesh() -- m_pIB->Lock() -- Failed"));
		return E_FAIL;
	}
...
HRESULT DxEffectRain::FrameMove ( float fElapsedTime, D3DXVECTOR3 vDirec, float fPower, BOOL bUse )
{
	...
	if( FAILED(hr) )
	{
		CDebugSet::ToLogFile(std::string("DxEffectRain::FrameMove() -- m_pVB->Lock() -- Failed"));
		return E_FAIL;
	}
	...
	for( int i=0; i<((int)m_dwFaces); i++ )
	{
		...
		if( m_pInforRain[i].vPos.y <= vLookatPt.y + m_fLimit )
		{
			...
			m_pInforRain[i].vPos.x = vLookatPt.x + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius) + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius);//( rand()%m_iRadius ) - m_iRadius/2 - (m_iHeight * (1.0f - m_vDirection.x/(fPower*m_fSpeed)) );
			m_pInforRain[i].vPos.y = vLookatPt.y + ( rand()%m_iHeight ) + m_iHeight; //(m_iHeight * m_vWind.y);		
			m_pInforRain[i].vPos.z = vLookatPt.z + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius) + ((sc::Random::RANDOM_POS()-0.5f)*m_iRadius);//( rand()%m_iRadius ) - m_iRadius/2 - (m_iHeight * (1.0f - m_vDirection.z/(fPower*m_fSpeed)) );
...
void	DxEffectRain::SaveSet ( sc::SerialFile &SFile )
{
...
void	DxEffectRain::LoadSet ( sc::SerialFile &SFile )
{
	DWORD	dwBuffSize;
	DWORD	dwVer;
	...
	else if ( dwVer == 0x103 )	//	데이터 포맷 변경 진행
	{
		RAIN_PROPERTY_103	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(RAIN_PROPERTY_103) );
		
		m_Property.m_dwFaces = sProp.m_dwFaces;
		m_Property.m_vColor = sProp.m_vColor;
		m_Property.m_iHeight = sProp.m_iHeight;
		m_Property.m_fLength = sProp.m_fLength;
		m_Property.m_fSpeed = sProp.m_fSpeed;
		m_Property.m_iRadius = sProp.m_iRadius;
		m_Property.m_fLimit = sProp.m_fLimit;
	}
	else if ( dwVer == 0x102 )
	{
...
void	DxEffectCloud::SaveSet ( sc::SerialFile &SFile )
{
...
void	DxEffectCloud::LoadSet ( sc::SerialFile &SFile )
{
	DWORD	dwBuffSize;
	DWORD	dwVer;
