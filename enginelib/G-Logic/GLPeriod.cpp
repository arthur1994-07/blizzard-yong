...
HRESULT GLPeriod::UpdateWeather ( float fElapsedTime )
{
	...
	if ( m_dwWeather & FGW_RAIN )
	{
		...
		if ( m_fThunderTimer <= 0.0f )
		{
            ...
            m_fThunderTimer = 30.0f + 20.0f * sc::Random::RANDOM_POS();
		}
	}
	...
	if (m_fWeatherTimer < 0.0f)
	{
		...
		//  현재 쓰여야 하는 날씨로 셋팅한다.
        if (sc::Random::RANDOM_POS() * 100.0f <= dwHigh)
		{
			DWORD dwSum		= m_dwRainRate+m_dwSnowRate+m_dwSporeRate+m_dwLeavesRate;
			DWORD dwResult	= DWORD(sc::Random::RANDOM_POS() * dwSum);
            ...
            else
            {
				...
				if (m_vecOneMapWeather[i].dwWeather & FGW_RAIN)
				{
					...
					if (m_vecOneMapWeather[i].fThunderTimer <= 0.0f)
					{
						...
						m_vecOneMapWeather[i].fThunderTimer = 30.0f + 20.0f * sc::Random::RANDOM_POS();
					}
				}
...
HRESULT GLPeriod::FrameMove ( float fTime, float fElapsedTime, const BOOL bNightAndDay )
{
...
//	CDebugSet::ToView ( 23, "day : %02d", m_dwMonth_Today );
	...
	{
		//	밤->낮 전환.
		if ( IsNoon() || bNightAndDay == FALSE )		m_bNight = !m_bNight;
	}
	else if ( bNightAndDay == TRUE )
	{
		//	밤->낮 전환.
...
void GLPeriod::LoadFile(const char* szFile, bool bToolMode)
{
	std::string strFullPath(std::string(GLOGIC::GetServerPath()) + szFile);

	CIniLoader gFILE(",", bToolMode);
