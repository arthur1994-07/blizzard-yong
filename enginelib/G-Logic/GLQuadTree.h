...
struct SQUADNODE
{
	...
	BOOL IsWithInThisNode(int nX, int nZ) const
	{
...
struct BOUDRECT
{
	...
	BOOL IsWithIn(int nX, int nZ) const
	{
		return (nMinX<=nX && nX<nMaxX && nMinZ<=nZ && nZ<nMaxZ);
	}
};
...
public:
	...
	int GetMaxZ () const { return m_nMaxZ; }
	int GetMinX () const { return m_nMinX; }
	int GetMinZ () const { return m_nMinZ; }

	int GetSizeX () const { return m_nMaxX-m_nMinX; }
	int GetSizeZ () const { return m_nMaxZ-m_nMinZ; }

	int	GetDivSizeX () const { return m_nDivSizeX; }
	int	GetDivSizeZ () const { return m_nDivSizeZ; }

	int GetAxisX () const { return m_nAxisX; }
	int GetAxisZ () const { return m_nAxisZ; }

	DWORD GetCellNum () const { return m_dwCellCount; }
...
public:
	...
	SQUADNODE<TYPE>* GetNode ( int nIndex )
	{
		...
		{
            sc::writeLogError(std::string("m_dwCellCount < 0 || m_dwCellCount > (DWORD)nIndex"));
			return NULL;
		}
        else
        {
...
template<class TYPE>
void GLQuadTree<TYPE>::DeleteQuadTree()
{
	SAFE_DELETE(m_pRootNode);
	SAFE_DELETE_ARRAY(m_pNodeArray);
}
							  ...
							  int nDivNum, SQUADNODE<TYPE>** ppNode )
{
	...
	*ppNode = new SQUADNODE<TYPE>;
	...
	//
	//mjeon
	//Modified the figure to help understanding.
	//Now it is more clear and showing exact what it is.
	//
	//		
