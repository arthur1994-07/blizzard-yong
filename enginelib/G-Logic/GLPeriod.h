...
public:
	const float GetTIMETOVBR(void) const			{ return m_fTIMETOVBR; }
	const DWORD GetYear(void) const				{ return m_dwThisYear; }
	const DWORD GetYearToday(void) const		{ return m_dwYear_Today; }
	const DWORD GetMonth(void) const			{ return m_dwThisMonth; }
	const DWORD GetMonthToday(void) const	{ return m_dwMonth_Today; }
	const DWORD GetMinute(void) const			{ return DWORD(m_fSecond * (1.0f/60.0f));}
	const DWORD GetHour(void) const				{ return m_dwHour; }
	const float GetSecond(void) const				{ return m_fSecond; }
	const DWORD GetWeather(void)					{ return m_dwWeather; }
...
public:
	...
	// bNightAndDay : 낮밤 구분이 있는가?;
	HRESULT FrameMove(float fTime, float fElapsedTime, const BOOL bNightAndDay = TRUE);

public:
	void LoadFile(const char* szFile, bool bToolMode);
