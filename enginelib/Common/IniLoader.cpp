...
void CIniLoader::ErrorMessageBox(const TCHAR * section, const TCHAR * szText)
{
	if (!m_bUseErrorMsgBox || !section || !szText)
        return;
...
void CIniLoader::ErrorMessageBox(const TCHAR* section, const TCHAR* key, const TCHAR* szText)
{
	if (!m_bUseErrorMsgBox || !section || !szText)
        return;
...
void CIniLoader::ErrorMessageBox(const TCHAR * section, const TCHAR * key, const DWORD dwLine, const TCHAR * szText)
{
	if (!m_bUseErrorMsgBox || !section || !szText)
        return;
...
CIniLoader::MAPKEY* CIniLoader::FindSection( const STRING& section )
{
	...
	ErrorMessageBox( section.c_str(), "A Section which does not exist." );
	return NULL;
}
...
CIniLoader::VECSTRING* CIniLoader::validflag(const STRING& section, const STRING& key, const int nIDX, const int nSIZE)
{
	...
	{
        std::string Msg(
            sc::string::format("It was wrong size of Input Data. Require %1% != %2%", nSIZE, pVecStr->size()));
        ErrorMessageBox(section.c_str(), key.c_str(), Msg.c_str());
        return NULL;
	}
...
CIniLoader::VECSTRING* CIniLoader::validflag( const DWORD dwITER, const STRING& section, const STRING& key, const int nIDX, const int nSIZE )
{
	...
	if (dwPARAMNUM != nSIZE)
	{
        std::string Msg(
            sc::string::format(
                "It was wrong size of Input Data. %1% != %2%", dwPARAMNUM, nSIZE));
		ErrorMessageBox(section.c_str(), key.c_str(), dwITER+1, Msg.c_str());
		return NULL;
	}
		...
		if( !IsNotes( strLine ) ) // Notes : 주석처리
		{
			...
			else
			{
				...
				if( !sectionback.empty() && !pMapKey->empty() )
				{
					...
					if( pr.second != true )   
					{
						ErrorMessageBox( sectionback.c_str(), "The Section already exists." );
					}
