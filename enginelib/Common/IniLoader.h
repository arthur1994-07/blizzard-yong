...
public:
	CIniLoader(const STRING& sep/*= _T(",")*/, bool bToolMode/*=false*/);
	~CIniLoader();
...
private:
    bool m_bToolMode; //! Tool 일때는 에러메시지 박스를 띄우지 않는다.
	...
	void ErrorMessageBox(const TCHAR * section, const TCHAR * szText);
	void ErrorMessageBox(const TCHAR * section, const TCHAR * key, const TCHAR * szText);
	void ErrorMessageBox(const TCHAR * section, const TCHAR * key, const DWORD dwLine, const TCHAR * szText);
