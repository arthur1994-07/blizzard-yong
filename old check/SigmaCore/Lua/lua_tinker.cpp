...
bool lua_tinker::dofile(lua_State *L, const wchar_t *filename)
{	
    std::string strFileName = sc::string::unicodeToAnsi(filename, (int) wcslen(filename));
	return dofile( L, strFileName.c_str() );
}
...
static void call_stack(lua_State* L, int n)
{
	...
	{
		// [shhan][2016.09.21] call_stack 정보를 얻기위해 이곳으로 옮김.
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	{
        sc::writeLogError(text);
		printf("%s\n", text);
		lua_pop(L, 1);

		char textConsole[ 4098 ];
		sprintf( textConsole, "%s\n", text );

		SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ),
			FOREGROUND_RED | FOREGROUND_INTENSITY );

		DWORD dwSize;
		WriteConsoleA( GetStdHandle( STD_OUTPUT_HANDLE ), textConsole, (DWORD)(strlen(textConsole)), &dwSize, 0 );
