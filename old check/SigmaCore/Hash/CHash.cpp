...
namespace HASH
{
	...
	{ // made by Russian;
		unsigned int _hashKey(0x1505);
		// 0001 0101 0000 0101 // 0x1505(5381);

		const char* const _pByteStream((const char* const)_pStream);

		for ( unsigned int _i(_sizeStream); _i--; )
			_hashKey += (_hashKey << 5) + unsigned int(_pByteStream[_i]);
		// hash * 33 + ch;
	...
	CString CHash::GOSTHash()
	{
		...
		if (hashOperation == FILE_HASH)
		{
			...
			fileToHash = _fsopen(hashFile, "rb", _SH_DENYNO);
	...
	CString CHash::SHA1Hash()
	{
		...
		if (hashOperation == STRING_HASH)
		{
			// Hashing a string, open the object, give it the text, and finalise it
			sha1_begin2(&m_sha1);
			sha1_hash2(reinterpret_cast<unsigned char *>(hashString.GetBuffer(hashString.GetLength())), hashString.GetLength(), &m_sha1);
			sha1_end2(tempOut, &m_sha1);
		}
		...
		if (hashOperation == FILE_HASH)
		{
			...
			do
			{
				...
				if(lenRead != 0)
				{
					sha1_hash2(fileBuf, lenRead, &m_sha1);
				}
			} while (lenRead == SIZE_OF_BUFFER);
	...
	CStringW CHashW::DoHash()
	{
		...
		if (hashAlgo == CRC32)
		{
			...
			if (hashOperation == STRING_HASH)
			{
				crc32.StringCrc32( (const wchar_t*) hashString, crc32r );
			}
			if (hashOperation == FILE_HASH)
			{
				crc32.FileCrc32Streams( (const wchar_t*) hashFile, crc32r );
			}

			retHash.Format( L"%08x", crc32r );
#endif
		}
	...
	int CHashW::GetHashAlgorithm()
	{
		return hashAlgo;
	}

	int CHashW::GetHashOperation()
	{
		return hashOperation;
	}

	CStringW CHashW::GetHashString()
	{
		return hashString;
	}

	CStringW CHashW::GetHashFile()
	{
		return hashFile;
	}

	int CHashW::GetHashFormat()
	{
		return hashFormatting;
	}

	int CHashW::GetSHA2Strength()
	{
		return sha2Strength;
	}

	CStringW CHashW::GOSTHash()
	{
#if SUPPORT_GOSTHASH
		// Common variables
		FILE *fileToHash = NULL;
		...
		if (hashOperation == FILE_HASH)
		{
			if (hashFile.IsEmpty()) return L"";
			gosthash_reset(&m_gost);

			fileToHash = _wfopen( hashFile, L"rb" );

			do
			{
				lenRead = (int) fread(fileBuf, 1, SIZE_OF_BUFFER, fileToHash);
				if(lenRead != 0)
				{
					gosthash_update(&m_gost, fileBuf, lenRead);
				}
	...
	CStringW CHashW::MD5Hash()
	{
		...
		if (hashOperation == STRING_HASH)
		{
			// Hashing a string, open the object, give it the text, and finalise it
			MD5Init(&m_md5);
			MD5Update(&m_md5, reinterpret_cast<unsigned char *>(hashString.GetBuffer(hashString.GetLength()*2)), hashString.GetLength()*2);
			MD5Final(&m_md5);
		...
		if (hashOperation == FILE_HASH)
		{
			...
			if ( md5file( (TCHAR*) hashFile.GetBuffer(MAX_PATH), 0, &m_md5) )
			{
				for (int i = 0; i < 16; i++)
				{
					char tmp[3];
	...
	void CHashW::SetHashAlgorithm(int Algo)
	{
		hashAlgo = Algo;
	}

	void CHashW::SetHashFormat(int Style)
	{
		if (Style == 1 || Style == 2 || Style == 3 || Style == 4)
		{
			hashFormatting = Style;
	...
	void CHashW::SetHashOperation(int Style)
	{
		// Check the style is legit
		if (Style == 1 || Style == 2)
		{
	...
	void CHashW::SetHashFile( const wchar_t* File )
	{
		hashFile = File;
	}

	void CHashW::SetSHA2Strength(int Strength)
	{
		// Check the strength is legit
		if (Strength == 256 || Strength == 384 || Strength == 512)
		{
	...
	void CHashW::SetHashString( const wchar_t* Hash )
	{
		hashString.Empty();
		hashString = Hash;
	}
