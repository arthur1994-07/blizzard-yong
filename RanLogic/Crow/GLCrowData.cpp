...
HRESULT SCROWDATA::LoadFile(sc::BaseStream& SFile, BOOL bServer, bool bPastLoad)
{
	...
	if ( GLItemMan::GetInstance().ValidTable() )
	{
		for ( int i=0; i<SCROWACTION::SALENUM; ++i )
		{
			...
			case CROW_LOAD_ERROR1			: sc::writeLogError( sc::string::format("SaleInvenLoadFile %1% �Ķ��Ÿ ���� ����"     , m_sAction.m_strSaleFile[i].c_str() )); break;
			case CROW_LOAD_ERROR2			: sc::writeLogError( sc::string::format("SaleInvenLoadFile() %1% ������ ���� �ε� ����", m_sAction.m_strSaleFile[i].c_str() )); break;
			case CROW_LOAD_ERROR3			: sc::writeLogError( sc::string::format("SaleInvenLoadFile %1% ��ġ �ߺ�"			   , m_sAction.m_strSaleFile[i].c_str() )); break;
			case CROW_LOAD_ERROR4			: sc::writeLogError( sc::string::format("SaleInvenLoadFile() %1% cInventory�� �Ķ��Ÿ�� ������ �߸��Ǿ����ϴ�.", m_sAction.m_strSaleFile[i].c_str() )); break;
	...
	if (bServer)
    {
        // �Ʒ��ʿ��� �ٽ� Load �Ѵ�. �ּ�ó��.2011-09-26 jgkim
...
HRESULT SCROWDATA::SaveFile(sc::SerialFile& SFile)
{
	...
	SFile << static_cast<DWORD> ( FILE_SBASIC );
	{
		...
		CString cstrName;
		cstrName.Format( _T("CN_%03d_%03d"), sNativeID.wMainID, sNativeID.wSubID ); // by ���
