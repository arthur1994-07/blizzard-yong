...
namespace FlyCameraControl{
	...
	public:
		...
		FlyCameraKey(const FlyCameraKey& rhs)
			...
			, sLookToTargetID(rhs.sLookToTargetID)
			, nKey(rhs.nKey) {}
		...
		{
			return (sMoveToTargetID == rhs.sMoveToTargetID && sLookToTargetID == rhs.sLookToTargetID) && nKey == rhs.nKey;
		}
		const bool operator !=(const FlyCameraKey& rhs) const
		{
			return (sMoveToTargetID != rhs.sMoveToTargetID || sLookToTargetID != rhs.sLookToTargetID ) || nKey != rhs.nKey;
	...
	struct SMsg
	{
		...
		DWORD dwMsg;
		union{
			struct	{	D3DXVECTOR4 vec;	};
			DWORD dwData;
		};
		SMsg(){}
		SMsg(EMMSG _type, DWORD _dwMsg):type(_type), dwMsg(_dwMsg) {}
		SMsg(EMMSG _type, DWORD _dwMsg, D3DXVECTOR4 _vec):type(_type), dwMsg(_dwMsg), vec(_vec) {}
		SMsg(EMMSG _type, DWORD _dwMsg, DWORD _dwData):type(_type), dwMsg(_dwMsg), dwData(_dwData) {}
	};
}