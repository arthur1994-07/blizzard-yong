...
int CCfg::Process(TCHAR* szLine)
{
	...
	{	
		...
		else if (_tcscmp(token, "user_db_server") == 0) 
		{
			...
			token = ::_tcstok(NULL, seps );
			if (token)
				::StringCchCopy(m_szUserDBServer, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else
				return 0;
		}
		...
		else if (_tcscmp(token, "user_db_database") == 0) 
		{
			...
			token = ::_tcstok(NULL, seps );
			if (token)
				::StringCchCopy(m_szUserDBDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else
				return 0;
		}
		...
		else if (_tcscmp(token, "game_db_server") == 0) 
		{
			...
			token = ::_tcstok(NULL, seps );
			if (token)
				::StringCchCopy(m_szGameDBServer, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else
				return 0;
		}
		...
		else if (_tcscmp(token, "game_db_database") == 0) 
		{
			...
			token = ::_tcstok(NULL, seps );
			if (token)
				::StringCchCopy(m_szGameDBDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else
				return 0;
		}
		...
		else if (_tcscmp(token, "user_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szUserOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "user_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szUserOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "game_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szGameOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "game_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szGameOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "board_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szBoardOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "board_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szBoardOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "log_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szLogOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "log_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szLogOdbcDatabase, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "shop_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szShopOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "shop_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szShopOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "terra_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szTerraOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "terra_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szTerraOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "gsp_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szGspOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "gsp_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szGspOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "kor_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szKorOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "kor_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szKorOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
		...
		else if (_tcscmp(token, "my_odbc_name") == 0) 
		{
			// database server name
			token = ::_tcstok(NULL, seps );
			if (token)	StringCchCopy(m_szMyOdbcName, sc::db::DB_SVR_NAME_LENGTH+1, token);
			else		return 0;
		}
		...
		else if (_tcscmp(token, "my_odbc_database") == 0) 
		{
			// database 명
			token = ::_tcstok(NULL, seps );
			if (token)		StringCchCopy(m_szMyOdbcDatabase, sc::db::DB_NAME_LENGTH+1, token);
			else			return 0;
		}
...
bool CCfg::GetNewServer()   { return m_bNewServer; }

BYTE CCfg::GetExcessExpProcessType() { return m_ExcessExpProcessType; }
...
bool CCfg::IsPKServer(void)     { return m_bPK; }	//	이건 PK 가능여부
bool CCfg::IsFreePKServer(void)	{ return m_bPKServer; }	//	이건 모든 유저 적대 모드 
