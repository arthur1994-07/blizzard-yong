...
#include "Util/s_CLock.h"
#include "Util/s_CConsoleMessage.h"
...
void CSMsgList::AddMsg(DWORD dwClient, void* pBuffer, int nSize)
{
	...
	if (m_pCurrent == NULL) {
		...
		if(nSize<0 )
		{
			CConsoleMessage::GetInstance()->Write(_T("ERROR:CSMsgManager::AddMsg nSize < 0!! dwClient %d, nSize %d"), dwClient, nSize  );
			return;
		}
