...
#define Real real
	...
	//-----------------------------------------------------------------------
	Math::Math( uint32   trigTableSize )
	{
...
//		mSinTable = mrcNEW_ARRAY(Real,mTrigTableSize);
//		mTanTable = mrcNEW_ARRAY(Real,mTrigTableSize);
		mSinTable = mrcNEW Real[mTrigTableSize];
		mTanTable = mrcNEW Real[mTrigTableSize];
	...
	{
		// Convert range to index values, wrap if required
		int32  idx = int(fValue *= mTrigTableFactor) % mTrigTableSize;
		return mTanTable[idx];
	}
	//-----------------------------------------------------------------------
	int32  Math::ISign (int32  iValue)
	{
	...
	{
		// Winding must be consistent from all edges for point32  to be inside
		Vector2 v1, v2;
		Real dot[3];
		bool zeroDot[3];
...
// 	}
		...
		bool discardInside)
	{
		...
		const Vector2& rayorig = ray.getOrigin() - sphere.getCenter();
	...
	std::pair<bool, Real> Math::intersects(const Ray& ray, const AxisAlignedBox& box)
	{
		...
		bool hit = false;
		Vector2 hitpoint;
		const Vector2& min = box.getMinimum();
		const Vector2& max = box.getMaximum();
		...
		if (rayorig.x <= min.x && raydir.x > 0)
		{
			...
			if (t >= 0)
			{
				...
				if (hitpoint.y >= min.y && hitpoint.y <= max.y && (!hit || t < lowt))
				{
					hit = true;
					lowt = t;
				}
		...
		if (rayorig.y <= min.y && raydir.y > 0)
		{
			...
			if (t >= 0)
			{
				// Substitute t back into ray and check bounds and dist
				hitpoint  = rayorig + raydir * t;
				if (hitpoint.x >= min.x && hitpoint.x <= max.x && (!hit || t < lowt))
				{
					hit = true;
					lowt = t;
				}
...
// 
// 		//
// 		// Check the intersection point32  is inside the triangle.
// 		//
// 		{
// 			Real u1 = b[i0] - a[i0];
// 			Real v1 = b[i1] - a[i1];
...
// 	}
	...
	bool Math::intersects(const Sphere& sphere, const AxisAlignedBox& box)
	{
		...
		const Vector2& center = sphere.getCenter();
		Real radius = sphere.getRadius();
		const Vector2& min = box.getMinimum();
		const Vector2& max = box.getMaximum();
...
// 	}
		...
		Real u1, Real v1, Real u2, Real v2, Real u3, Real v3)
	{
		...
		//should point32  in the same direction as our normal calculated above. 
...
// 	}
	...
	{
		Vector2 max = aabb.getMaximum();
		Vector2 min = aabb.getMinimum();
