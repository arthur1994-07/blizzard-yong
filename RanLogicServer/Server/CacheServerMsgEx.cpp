...
int CacheServer::SendAuthInfoToSessionServer()
{
...
void CacheServer::MsgAuthCertificationResponse(DWORD dwClient, NET_MSG_GENERIC* pMsg)
{   
    if ( pMsg == NULL ) 
		return;
	...
	/*if ( nsm->gsi.ServerType != net::SERVER_CACHE )*/
	if ( nsm->gsi.ServerType != m_pConfig->GetServerType() )
	{
		sc::writeLogInfo(std::string("Auth Msg Target is not match(CACHE SERVER)"));
	}
	else
	{
