...
protected:
	...
	//mjeon.dbman.ODBC와 ADO를 겸해서 사용하기 위해 AdoManager를 명시적으로 하나 생성한다.
...
public:
	...
	virtual int SendMatch(NET_MSG_GENERIC* pBuffer);	//Match로 전송
    virtual int SendMatch(EMNET_MSG MsgType, const msgpack::sbuffer& PackBuffer, bool UseCrc=false);	//Match로 전송

    virtual int SendAgent(NET_MSG_GENERIC* pBuffer);	//Match를 거쳐서 해당 Agent로 전송하도록 구현
    virtual int SendAgent(EMNET_MSG MsgType, const msgpack::sbuffer& PackBuffer, bool UseCrc=false);	//Match를 거쳐서 해당 Agent로 전송하도록 구현
