...
void GLAgentServer::initPacketFunc()
{
     ...
     * modified : sckim, 2016.01.07, ������� ���� �ʴ� �޼��� ������� �Ǵ��Ͽ� ����
     */
	...
	m_MsgFunc[NET_MSG_GM_RANMOBILE_COMMAND_REQUEST].Msgfunc = &GLAgentServer::MsgGmRanMobileCommandRequest; //�������
	m_MsgFunc[NET_MSG_GM_RANMOBILE_COMMAND_RESPONSE].Msgfunc = &GLAgentServer::MsgGmRanMobileCommandResponse; //�������
...
BOOL GLAgentServer::MsgReqFieldSvrCharChkFb(NET_MSG_GENERIC* nmg, DWORD _dwClientID, DWORD _dwGaeaID)
{
	...
	*/
	if (pLAND)
	{
		...
		{
			if(m_pCTFMan->GetState() != CTF_STATE_STOP)	//CTF ������
			{
				sGenMapID	= pChar->m_sStartMapID;
				dwGenGateID = pChar->m_dwStartGate;
			}
...
BOOL GLAgentServer::MsgReqCreateInstantMap ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	//GLCharAG* pChar = GetChar(pNetMsg->dwGaeaID); //mjeon.AF.�� �� �޽����� ���Ե� GaeaID�� ��ȿ�ϳ� ChaNum�� ����ϵ��� �Ѵ�.
	...
	if( (pNetMsg->bCreate && bFindEnterMap) ||  // ���� ��û�� �Դµ� �̹� ������ �δ��� �ִ°�� �޽����� ������ �����Ѵ�.
		(!bFindEnterMap && !pNetMsg->bCreate) ) // ���� ��û�� �Դµ� �̹� ������ �δ��� ���°�� �޽����� ������ �����Ѵ�.
	{
		/*
...
BOOL GLAgentServer::MsgReqGateOut ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	/*
		...
		1.CTF PVP�����δ� �̵��� �� ����. (����, CTF �ý��ۿ� ���ؼ��� Recall�� �� �ִ�.)
		
		2.CTF START ���¿��� Gate�� Ÿ�� ���� �� ����.
	*/

	if(pCurLandMan->GetBaseMapID().dwID != m_pCTFMan->GetPvPMapID() &&	//PvP�� -> PvP�������� ����Ʈ �̵��� ���
...
BOOL GLAgentServer::MsgResurrectMoveMapFB(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
    ...
    NetJoinField.bChangeField = true; // MsgResurrectMoveMapFB�� �ٸ� �ʵ忡���� ��Ȱ�̹Ƿ�, �� ���� �׻� true;
...
BOOL GLAgentServer::MsgGmViewWorkEvent ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if (m_WorkEvent.bWorkEvent[EVENT_EXP])  // ����ġ 
	{
		StringCchPrintf(
	...
	if (m_WorkEvent.bWorkEvent[EVENT_ITEM])  // ������
	{
		StringCchPrintf(
            NetMsgFB.szWorkEventInfo,
            GLMSG::SNET_GM_VIEWWORKEVENT_FB::INFO_SIZE,
	...
	if (m_WorkEvent.bWorkEvent[EVENT_MONEY])  // �� �����
	{
		StringCchPrintf(
            NetMsgFB.szWorkEventInfo,
            GLMSG::SNET_GM_VIEWWORKEVENT_FB::INFO_SIZE,
	...
	if (m_WorkEvent.bWorkEvent[EVENT_GRADE])  // ������
	{
		StringCchPrintf(
            NetMsgFB.szWorkEventInfo,
            GLMSG::SNET_GM_VIEWWORKEVENT_FB::INFO_SIZE,
	...
	if (m_WorkEvent.bWorkEvent[EVENT_EXTYPE])  // EX �̺�Ʈ
	{
	...
	if( m_WorkEvent.bWorkEvent[EVENT_LIMIT] )  // ����Ʈ �̺�Ʈ 
	{
        std::string Msg(
            sc::string::format(
    ...
    if( m_WorkEvent.bWorkEvent[EVENT_IP] ) // IP �̺�Ʈ.
    {
...
BOOL GLAgentServer::MsgGmInstantMapNum ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	NetMsgFB.dwInstantmapCnt = (DWORD)m_vecInstantMapId.size()/*m_sInsertInstantMapId.useMapSubIDSet.size()*/;	//������ �δ��� ����
...
BOOL GLAgentServer::MsgGmClubDeathMatchStartEndCA(NET_MSG_GENERIC* pMsg, DWORD ClientSlot, DWORD GaeaID)
{
	...
	if (pPacket->m_nFlag == GLMSG::SNET_GM_DEATHMATCH_ON_OFF_CA::ONE)
	{
		...
		{
		case GLClubDeathMatch::BATTLE_NOSTART://	�������� ����
			{
				pClubDeathMatch->m_bGMReady = true;
			}
			return TRUE;

		case GLClubDeathMatch::BATTLE_READY://	��Ʋ �����
			{
		...
		case GLClubDeathMatch::BATTLE_START://	��Ʋ ��
			{
...
*/
	...
	if (bMoneyAccept)	//mjeon.�ε��׽þ� ������ ���� ��ŷ������ �ּ� �ٽ� ���� -> �׷��Ƿ� �ý��� ����� ���� ���� ÷�ξ������� ���������� ���� ������ ��쿡�� �������� �ؾ��Ѵ�.
	{
	...
	if (bAttach01Accept)//mjeon.�ε��׽þ� ������ ���� ��ŷ������ �ּ� �ٽ� ���� -> �׷��Ƿ� �ý��� ����� ���� ���� ÷�ξ������� ���������� ���� ������ ��쿡�� �������� �ؾ��Ѵ�.
	{
		// update current post's status
...
//
// CTF �ð��� ����Ǿ��� ���, 2�� �̻��� �����⸦ ������ �п��� �¸��п��� �ȴ�. �� ��, �ش� �п� ������ FieldS�κ��� ȹ��.
//
...
BOOL GLAgentServer::MsgTransferCharDoneFA(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	...
	//Transfer�� ������ FieldServer -> InstanceServer�� ���� ������ �������� FieldServer(Source)�� �˸���.
	if (pmsg->bToInstance)
	{
...
BOOL GLAgentServer::MsgTransferChar(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	...
	{
		//InstaceServer���� DropOut�ϸ鼭 �߻��� DB����� transfer�� ���, ���⿡�� ĳ���Ͱ� �������� ���� �� �ִ�.
		//�׷��Ƿ� ������ 0�� ä�η� ������ DB save ��Ű��.
...
BOOL GLAgentServer::MsgChatInviteCAC(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	...
	if (m_pGroupChatMan->CheckLevel(pCharWho->GetChaLevel()) == FALSE)
	{
		...
		SENDTOCLIENT(pChar->ClientSlot(), &msg);	//�ʴ��� ��ü���� ������ �߼��Ѵ�.
...
{
	// AgentServerMsg.cpp case NET_MSG_CHA_SAVE_FA: ���� ó����
...
{
	// AgentServerMsg.cpp case NET_MSG_CHA_SAVE_END_FA: ���� ó����
...
bool GLAgentServer::IsCharJoining( DWORD dwUserNum )
{
	if ( m_pCharMan )
		return m_pCharMan->ChaJoinUserNumExist( dwUserNum );
	return false;
}

void GLAgentServer::CharJoinAdd( DWORD dwUserNum )
{
	if ( m_pCharMan )
		m_pCharMan->ChaJoinUserNumAdd( dwUserNum );
}

void GLAgentServer::CharJoinDel( DWORD dwUserNum )
{
	if ( m_pCharMan )
		m_pCharMan->ChaJoinUserNumDel( dwUserNum );
}
...
BOOL GLAgentServer::MsgServerCloseField(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	...
	// channel/ID ��� net::INVALID_SLOT �ϼ� �ִٴ� �� �����صΰ� ó���ؾ� �Ѵ�;
