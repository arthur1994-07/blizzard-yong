...
void CErrorReportDlg::UpdateStackEntryFromMap(CStackEntry& rStackEntry)
{
		...
		))
	{
		...
		if (ptrAddress != NULL)
		{
			std::tr1::shared_ptr<CBaseProcessor> pBaseProcessor(GetModuleInfo(rStackEntry.m_strModule));
			if (pBaseProcessor.get() != NULL)
			{
				std::tr1::shared_ptr<CBaseFnInfo> pFnInfo;
				...
				std::tr1::shared_ptr<CBaseFileInfo> pFileInfo;
				std::tr1::shared_ptr<CBaseLineInfo> pLineInfo;
...
std::tr1::shared_ptr<CBaseProcessor> CErrorReportDlg::GetModuleInfo(const CString& strModule)
{
	std::tr1::shared_ptr<CBaseProcessor> pBaseProcessor;
