...
public:
	...
	std::tr1::shared_ptr<CBaseProcessor> GetModuleInfo(const CString& strModule);
	...
	typedef std::map<CString, std::tr1::shared_ptr<CBaseProcessor> > CModuleMap;
	
	std::list<CStackEntry> m_stackList;
	boost::regex m_rxFunctionName;	
	CModuleMap m_mapModules;	
	std::tr1::shared_ptr<CBaseProcessor> m_pPdbProcessor;
