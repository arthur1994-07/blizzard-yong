...
CSoundSourceManagerApp::CSoundSourceManagerApp()
{
    ...
    BUG_TRAP::BugTrapInstall(GLogicData::GetInstance().GetServiceProvider(), std::string(_T("SoundSourceManager")));
	...
	if ( strAppPath.GetLength() )
	{
        ...
        int i = 0;
		for ( i = strAppPath.GetLength() - 1; 0 <= i; i-- )
		{
