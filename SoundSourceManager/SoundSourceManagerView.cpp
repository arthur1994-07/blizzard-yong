...
void CSoundSourceManagerView::OnNMDblclkListSoundsourceTable(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
    ...
    StringCchCopy(FileName, MAX_PATH, TargetFile.GetString());
	
	//	<--	Wav파일 버퍼에 들어 올리기	-->	//    
	if( FAILED( m_pSoundManager->Create( &m_pSound, FileName, 0, GUID_NULL ) ) )
	{
...
//	dlg.m_ofn.lpstrInitialDir = GLOGIC::GetPath ();
	if ( dlg.DoModal() == IDOK )
	{
		...
		FILE* fp = _fsopen ( dlg.GetFileName ().GetString(), "wt", _SH_DENYNO );
		if ( !fp ) return ;
