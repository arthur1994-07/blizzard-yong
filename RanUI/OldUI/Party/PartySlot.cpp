...
CPartySlot::CPartySlot(EngineDeviceMan* pEngineDevice)
...
void CPartySlot::CreateSubControl ()
{
    ...
    {	//	클래스 표시
        CString strClassKeyword[GLCL_NUM_CLASS] = 
        {
...
CBasicTextBox* CPartySlot::CreateStaticControl (const char* szControlKeyword, CD3DFontPar* pFont, D3DCOLOR dwColor, int nAlign )
{
...
{
    if ( nClassType < GLCI_FIGHTER_M && GLCI_NUM_ETC <= nClassType )
    {
        GASSERT ( 0 && "클래스타입의 인덱스범위가 경계를 벗어납니다." );
        return ;
    }

    EMCLASSINDEX emClassIndex = CharClassToClassIndex( CharIndexToClass( (EMCHARINDEX)nClassType ) );
    ...
    m_pClass[emClassIndex]->SetVisibleSingle ( TRUE );
}
