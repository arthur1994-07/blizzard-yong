...
CPrivateMarketSellWindow::CPrivateMarketSellWindow ( CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice )
...
void CPrivateMarketSellWindow::CreateSubControl ()
{
...

	{
		...
		pEditBoxMan->CreateEditBox ( PRIVATE_MARKET_SELL_WINDOW_MONEY_EDIT, "PRIVATE_MARKET_SELL_WINDOW_MONEY_EDIT", 
									"PRIVATE_MARKET_SELL_WINDOW_CARRAT", TRUE, UINT_MAX, pFont, 11 );
...
CBasicTextBox*	CPrivateMarketSellWindow::CreateStaticControl (const char* szControlKeyword, CD3DFontPar* pFont )
{
	...
	case PRIVATE_MARKET_SELL_WINDOW_OK:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				if ( GetItemCost() > LL_LIMITED_A_UNIT_PRICE )
				{	// 최대 금액 초과시.
					m_pInterface->GetGaeaClient()->PrintMsgTextDlg ( 
						NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("PRIVATE_LIMITED_A_UINIT_PRICE") );
				}
				else
				{
...
{	
	...
	return static_cast<LONGLONG>( _atoi64 ( strTemp ) );
}
