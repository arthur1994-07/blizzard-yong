...
CConftModalWindow::CConftModalWindow(CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice)
...
void CConftModalWindow::CreateSubControl()
{
	...
	//	콤보박스
	m_pComboBoxUsableHPOpen = new CBasicComboBox(m_pEngineDevice);
	...
	m_pComboBoxHPStateMyOpen = new CBasicComboBox(m_pEngineDevice);
	...
	m_pComboBoxHPStateHisOpen = new CBasicComboBox(m_pEngineDevice);
	...
	m_pComboBoxUsableHPRollOver = new CBasicComboBoxRollOver(m_pEngineDevice);
	...
	m_pComboBoxHPStateMyRollOver = new CBasicComboBoxRollOver(m_pEngineDevice);
	...
	m_pComboBoxHPStateHisRollOver = new CBasicComboBoxRollOver(m_pEngineDevice);
...
{
	if ( (CallerID == UI::MODAL_CONFLICT_ONE_TAR) ||
		 (CallerID == UI::MODAL_CONFLICT_PARTY_TAR) ||
		 (CallerID == UI::MODAL_CONFLICT_GUILD_TAR ) )
		m_bReqModal = FALSE;
	else
		m_bReqModal = TRUE;
...
CBasicTextBox* CConftModalWindow::CreateStaticControl(const char* szControlKeyword, CD3DFontPar* pFont, D3DCOLOR dwColor, int nAlign )
{
...
void	CConftModalWindow::LoadUsableHP()
{
	...
	{		
		SCONFT_OPTION* pConftOption = m_pInterface->GetConftOption();
...
MyConftModalWindow::MyConftModalWindow( CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice )
    : CConftModalWindow( pInterface, pEngineDevice )
{
}

void MyConftModalWindow::CreateUIWindowAndRegisterOwnership()
{
    CConftModalWindow::Create( CONFT_MODAL_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );		
    CConftModalWindow::CreateBaseModal( "CONFT_MODAL_WINDOW");
    CConftModalWindow::CreateSubControl();
    CConftModalWindow::SetAlignFlag( UI_FLAG_CENTER_X | UI_FLAG_CENTER_Y );
    CConftModalWindow::LoadDefaultCondition();
    CConftModalWindow::m_pInterface->UiRegisterControl( this );
    CConftModalWindow::m_pInterface->UiShowGroupFocus( CONFT_MODAL_WINDOW );
}

void MyConftModalWindow::DoModal( UI::String strText, UIGUID CallerID, UI::String strMyName, UI::String strHisName )
{
    CConftModalWindow::DoModal( UI::ToString( strText ), CallerID, UI::ToString( strMyName ), UI::ToString( strHisName ) );
}

UIGUID MyConftModalWindow::GetCallerID()
{
    return CConftModalWindow::GetCallerID();
}