...
CHeadChatDisplay::CHeadChatDisplay(GLGaeaClient* pGaeaClient, EngineDeviceMan* pEngineDevice)
...
void CHeadChatDisplay::CreateSubControl ()
{
	...
	CBasicTextBox* pChatBox = new CBasicTextBox(m_pEngineDevice);
...
void CHeadChatDisplay::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	D3DXVECTOR3 *pPos = m_pGaeaClient->FindCharHeadPos ( m_strName.GetString() );
	if ( pPos )
	{
