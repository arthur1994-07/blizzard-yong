...
CBasicAudioOption::CBasicAudioOption(EngineDeviceMan* pEngineDevice)
...
void CBasicAudioOption::CreateSubControl ()
{
	...
	{	//	볼륨 프로그래스바
		...
		pDummyVolume = new CUIControl(m_pEngineDevice);
...
CBasicTextBox* CBasicAudioOption::CreateStaticControl (const char* szControlKeyword, CD3DFontPar* pFont )
{
	...
	case HWOPTION_AUDIO_SFX_PROGRESS:
		{
			...
			long lVolume = long (NSSoundDef::CONTROL_DECIBEL * m_pSfxProgressBar->GetPercent ());
			if ( lVolume < 1 )	RANPARAM::nSndSfx = DSBVOLUME_MIN;
			else				RANPARAM::nSndSfx = lVolume - NSSoundDef::CONTROL_DECIBEL;
	...
	case HWOPTION_AUDIO_ENVIR_PROGRESS:
		{
			...
			long lVolume = long(NSSoundDef::CONTROL_DECIBEL * m_pEnvirProgressBar->GetPercent ());
			if ( lVolume < 1 )	RANPARAM::nSndMap = DSBVOLUME_MIN;
			else				RANPARAM::nSndMap = lVolume - NSSoundDef::CONTROL_DECIBEL;
	...
	case HWOPTION_AUDIO_BGM_PROGRESS:
		{
			...
			long lVolume = long(NSSoundDef::CONTROL_DECIBEL * m_pBgmProgressBar->GetPercent ());
			if ( lVolume < 1 )	RANPARAM::nSndMusic = DSBVOLUME_MIN;
			else				RANPARAM::nSndMusic = lVolume - NSSoundDef::CONTROL_DECIBEL;
...
void CBasicAudioOption::LoadCurrentOption ()
{
	...
	if ( RANPARAM::nSndSfx == DSBVOLUME_MIN )
		...
		lVolume = RANPARAM::nSndSfx + NSSoundDef::CONTROL_DECIBEL;
    fPercent = float(lVolume) / float(NSSoundDef::CONTROL_DECIBEL);
	m_pSfxProgressBar->SetPercent ( fPercent );
	...
	if ( RANPARAM::nSndMap == DSBVOLUME_MIN )
		...
		lVolume = RANPARAM::nSndMap + NSSoundDef::CONTROL_DECIBEL;
    fPercent = float(lVolume) / float(NSSoundDef::CONTROL_DECIBEL);
	m_pEnvirProgressBar->SetPercent ( fPercent );
	...
	if ( RANPARAM::nSndMusic == DSBVOLUME_MIN )
		...
		lVolume = RANPARAM::nSndMusic + NSSoundDef::CONTROL_DECIBEL;
    fPercent = float(lVolume) / float(NSSoundDef::CONTROL_DECIBEL);
	m_pBgmProgressBar->SetPercent ( fPercent );
