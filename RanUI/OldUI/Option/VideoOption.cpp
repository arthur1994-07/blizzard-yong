...
void CBasicVideoOption::LoadCurrentOption()
{
	...
	LoadSight();			// 시야 상태
	LoadTexFilter();		// 텍스쳐 필터링
	LoadTexQuality();		// 텍스쳐 Quality
	...
	LoadTargetEffectButton ();// 타겟 이펙트 표시
	LoadClickEffectButton (); // 클릭시 이펙트 표시
	LoadMineEffectButton ();  // 아이템 표시
	LoadUseNormalMapButton(); // 노말맵 사용
}
...
void CBasicVideoOption::SetTextTexFilterCombo( DWORD dwIndex )
{
	...
	{
		GASSERT ( 0 && "텍스쳐 Filter 범위를 넘어섭니다." );
	}
}
...
void CBasicVideoOption::SetTextTexQualityCombo( DWORD dwIndex )
{
	...
	{
		GASSERT ( 0 && "텍스쳐 Quality 범위를 넘어섭니다." );
	}
}
...
void CBasicVideoOption::SetTextPhysXClothCombo( DWORD dwIndex )
{
	...
	{
		GASSERT ( 0 && "PhysX Level 범위를 넘어섭니다." );
	}
}
