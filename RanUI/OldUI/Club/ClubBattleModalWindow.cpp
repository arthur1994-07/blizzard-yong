...
CClubBattleModal::CClubBattleModal( CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice )
...
CBasicTextBox * CClubBattleModal::CreateStaticControl(const char* szControlKeyword, CD3DFontPar* pFont, int nAlign, const UIGUID& cID )
{
...
void CClubBattleModal::CreateSubControl()
{
	...
	{	// Note : 시간,분 설정
		...
		m_pEditBoxHour = new CUIEditBox(m_pEngineDevice);
		...
		m_pEditBoxMin = new CUIEditBox(m_pEngineDevice);
	...
	case CLUB_BATTLE_OK:
		{
			if( CHECK_KEYFOCUSED( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				m_pInterface->ReqClubBattle( m_dwTime, m_bAlliance );				
