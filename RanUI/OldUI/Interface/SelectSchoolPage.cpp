...
CSelectSchoolPage::CSelectSchoolPage(COuterInterface* pInterface, EngineDeviceMan* pEngineDevice) 
...
void CSelectSchoolPage::CreateSubControl ()
{
	...
	m_pSchoolNameStatic = new CBasicTextBox(m_pEngineDevice);
	...
	m_pSchoolDec = new CBasicTextBox(m_pEngineDevice);
	...
	for ( int i = 0 ; i < MAX_SELSCHOOL; ++i ) 
	{
		...
		m_pSelectSchoolImg[i] = new CUIControl(m_pEngineDevice);
		...
		m_pSelectSchoolSetImg[i] = new CUIControl(m_pEngineDevice);
