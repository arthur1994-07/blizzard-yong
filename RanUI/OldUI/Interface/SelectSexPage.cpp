...
CSelectSexPage::CSelectSexPage(EngineDeviceMan* pEngineDevice)
...
void CSelectSexPage::CreateSubControl ()
{
	...
	for ( int i = 0 ; i < MAX_SEX; ++i ) 
	{
		...
		m_pSelectSexImg[i] = new CUIControl(m_pEngineDevice);
		...
		m_pSelectSexSetImg[i] = new CUIControl(m_pEngineDevice);
