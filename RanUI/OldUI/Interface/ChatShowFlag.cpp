...
CUIChatShowFlag::CUIChatShowFlag(EngineDeviceMan* pEngineDevice)
...
void CUIChatShowFlag::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( m_pNormalButton->IsFlip() )	AddMessageEx ( UIMSG_CHAT_NORMAL1 );
	if ( m_pPrivateButton->IsFlip() )	AddMessageEx ( UIMSG_CHAT_PRIVATE1 );
	if ( m_pPartyButton->IsFlip() )		AddMessageEx ( UIMSG_CHAT_PARTY1 );
	if ( m_pClubButton->IsFlip() )		AddMessageEx ( UIMSG_CHAT_CLUB1 );
	if ( m_pAllianceButton->IsFlip() )	AddMessageEx ( UIMSG_CHAT_ALLIANCE1 );
	if ( m_pSystemButton->IsFlip() )	AddMessageEx ( UIMSG_CHAT_SYSTEM1 );
}
