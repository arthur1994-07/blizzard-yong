...
CSelectClassPage::CSelectClassPage(COuterInterface* pInterface, EngineDeviceMan* pEngineDevice)
...
void CSelectClassPage::CreateSubControl ()
{
	...
	CD3DFontPar* pFont18 = m_pEngineDevice->LoadFont ( _DEFAULT_FONT, static_cast<DWORD>(18 * fontSize), _DEFAULT_FONT_FLAG );
	...
	for ( int i = 0 ; i < MAX_CLASS; ++i ) 
	{
		...
		m_pSelectClassImg[i] = new CUIControl(m_pEngineDevice);
	...
	m_pSelectClassSetImg = new CUIControl(m_pEngineDevice);
...
//	m_pSelectClassImg[i]->SetDiffuse( NS_UIDEBUGSET::PINK );
	...
	m_pClassNameStatic = new CBasicTextBox(m_pEngineDevice);
	...
	m_pClassDec = new CBasicTextBox(m_pEngineDevice);
...
void CSelectClassPage::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( !m_bCreateExtreme && rcPos.left != 0 )
	{
		SetImagePosition();
	}
