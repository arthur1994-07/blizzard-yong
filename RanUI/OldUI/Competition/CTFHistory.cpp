...
CCTFHistory::CCTFHistory(GLGaeaClient* pGaeaClient, CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice)
...
CCTFHistory::~CCTFHistory ()
{

}

void CCTFHistory::CreateSubControl ( LPDIRECT3DDEVICEQ* ppd3dDevice )
{
	...
	//! 버튼
	{
		...
		m_pButton_Ranking->CreateSub( this, "COMPETITION_CTF_PAGE_HISTORY_RANKING_BUTTON", UI_FLAG_DEFAULT, CTF_HISTORY_RANKING_BUTTON );
...
void CCTFHistory::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	switch ( ControlID )
	{
	case CTF_HISTORY_RANKING_BUTTON:
		{
...
CBasicTextBox* CCTFHistory::CreateStaticControl(const char* szControlKeyword, CD3DFontPar* pFont, int nAlign )
{
...
void CCTFHistory::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
	Update();
}

void CCTFHistory::Update()
{
	GLCaptureTheFieldClient* pCTFClient = m_pGaeaClient->GetCaptureTheField();
	GLCaptureTheFieldClient::CTFPREVSTATE& sState = pCTFClient->GetPrevState( m_emCTFType );
