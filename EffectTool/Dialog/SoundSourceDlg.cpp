...
BOOL CSoundSourceDlg::OnInitDialog()
{
	...
	LV_COLUMN	lvcolumn;
	int			ColumnCount = 4;	
	char* ColumnName[4] = { "Type", "Buffer Count", "File", "Comment" };
...
void	CSoundSourceDlg::ReloadAllItems ()
{
	...
	while ( !m_pSoundSourceMan->IsEnd () )
	{
		...
		if ( pReloadSource->TYPE == nCategory || nCategory == 7 )
		{
			...
			{
				char* ColumnName[4] = { "Category", "Buffer Count", "File", "Comment" };
...
void CSoundSourceDlg::OnNMDblclkListSoundsource(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	{
		CString strTemp;
		strTemp.Format("The file does not exist. [%s]", m_SoundSource.FileName);
		MessageBox(strTemp.GetString(), "Error", MB_OK);
		return;
	}		
