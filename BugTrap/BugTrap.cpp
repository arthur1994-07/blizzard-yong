...
BOOL ProcessMiniDump( EXCEPTION_POINTERS* pException )
{
	...
	if ( hDll )
	{
		...
		if ( pDump )
		{
			...
			_stprintf_s( szScratch, TEXT( "%d%d%d%d%d%d" ), 
				sysTime.wYear, 
				sysTime.wMonth, 
				sysTime.wDay,
				sysTime.wHour,
...
unsigned __stdcall CreateMiniDump( void* pArguments )
{
	...
	// restart.bat 실행
	{
		...
		TCHAR* pszCmdParam = "\\cmd.exe /k ";
		TCHAR* pszBatchFileName = "\\restart.bat";
		TCHAR ModuleFilePath[MAX_PATH];
		GetModuleFileName( NULL, ModuleFilePath, sizeof ModuleFilePath );
		PathRemoveFileSpec( ModuleFilePath );
		_stprintf_s( szCommandLine, "%s%s%s%s", szCmdPath, pszCmdParam, ModuleFilePath, pszBatchFileName );
...
static LONG GenericFilter(PEXCEPTION_POINTERS pExceptionPointers, CSymEngine::EXCEPTION_TYPE eExceptionType)
{
	...
	{
		// 버그트랩에서 Stack overflow 처리가 되지않는데, 처리되도록 코드 추가함
		MinidumpInfo info;
		info.threadId = ::GetCurrentThreadId();
		info.pException = pExceptionPointers;
