...
bool CBLOCK_PROG::LOAD ( std::string strROOT, std::string strFile )
{
	...
	if (!cFILE.open(m_strLIST_FILE, true))
        return false;

	DWORD dwNUM(0);
	bool bOK = cFILE.getflag ( "BINARY_IMAGE_NUM", 1, 1, dwNUM, true );
	if ( !bOK )											return false;
	...
	{
		SBLOCK_INFO sINFO;
		cFILE.getflag ( i, "INFO", 1, 7, sINFO.bBLOCK, true );
		cFILE.getflag ( i, "INFO", 2, 7, sINFO.strName, true );
		cFILE.getflag ( i, "INFO", 3, 7, sINFO.strWindowText, true );
		cFILE.getflag ( i, "INFO", 4, 7, sINFO.strProductName, true );
		cFILE.getflag ( i, "INFO", 5, 7, sINFO.strFileDescription, true );
		cFILE.getflag ( i, "INFO", 6, 7, sINFO.strLoadDll, true );
		cFILE.getflag ( i, "INFO", 7, 7, sINFO.strFileStream, true );
