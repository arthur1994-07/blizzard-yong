...
void SetWin_Num_LONGLONG( CDialog *pDlg, int nID, const LONGLONG &nValue )
{
	CString	str;
...
//----------------------------------------------------------------------------------
void AddWin_Text_Semicolon( CDialog *pDlg, int nID, const char *szText )
{
    CString	str;
    ...
    pWnd->GetWindowText( str );
    str.AppendFormat( "%s; ", TEXT(szText) );
    pWnd->SetWindowText ( str );
}
...
void SetWin_Combo_Init_IntVal( CDialog *pDlg, int nID, int nNum )
{
	char buffer [50];
	CString	str;
	CComboBox *pCombo = (CComboBox*) ( pDlg->GetDlgItem(nID) );
...
bool SetWin_Combo_Sel( CDialog *pDlg, int nID, const std::string& strText )
{	
	CString str;
...
DWORD GetWin_Combo_Sel ( CDialog *pDlg, int nID, const char *szTexts )
{
	...
	for ( int i=0; i<nSize; ++i )
	{
		...
		if ( str==szTexts )
		{			
			return static_cast<DWORD>(pCombo->GetItemData ( i ));
		}
	}
...
//----------------------------------------------------------------------------------
void SetWIn_Multi_ListBox_Sel( CDialog *pDlg, int nID, CString strText, BOOL bSelect /* = TRUE */ )
{
	CString	str;
	...
	for ( int i=0; i<nSize; ++i )
	{
		...
		if ( str==strText )
		{
			pListBox->SetSel( i, bSelect );
			return;
		}
	}

	pListBox->SetSel( LB_ERR );
}
...
int GetWin_ListBox_Sel( CDialog *pDlg, int nID, CString strText )
{
	CString	str;
