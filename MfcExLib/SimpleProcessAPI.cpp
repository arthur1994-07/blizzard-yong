...
BOOL CSimpleProcessAPI::WindowsNTBuildProcessList(CMapStringToString &ProcessPIDNameMap)
{
    ...
    pObj = reinterpret_cast<PPERF_OBJECT_TYPE>(reinterpret_cast<UINT_PTR>(pPerf) + pPerf->HeaderLength);

    // 
    // loop thru the performance counter definition records looking 
    // for the process id counter and then save its offset 
    // 
    pCounterDef = reinterpret_cast<PPERF_COUNTER_DEFINITION>(reinterpret_cast<UINT_PTR>(pObj) + pObj->HeaderLength); 
    ...
    pInst = reinterpret_cast<PPERF_INSTANCE_DEFINITION>(reinterpret_cast<UINT_PTR>(pObj) + pObj->DefinitionLength);
    ...
    for (i=0; i<dwNumTasks; ++i)
	{
        ...
        //
        p = reinterpret_cast<LPSTR>(reinterpret_cast<UINT_PTR>(pInst) + pInst->NameOffset);
        ...
        // get the process id
        pCounter = reinterpret_cast<PPERF_COUNTER_BLOCK>(reinterpret_cast<UINT_PTR>(pInst) + pInst->ByteLength);
        DWORD nProcessId = *(reinterpret_cast<LPDWORD>(reinterpret_cast<UINT_PTR>(pCounter) + dwProcessIdCounter));
		TRACE1("%u\n", nProcessId);
        ...
        // next process
        pInst = reinterpret_cast<PPERF_INSTANCE_DEFINITION>(reinterpret_cast<UINT_PTR>(pCounter) + pCounter->ByteLength);
    }
