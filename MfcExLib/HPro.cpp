...
bool CHPro::CreateProcessUPX(const char * szPathName, CString &strTmpFile, SUBPATH* pPath)
{
	if (!szPathName)
        return false;
	...
	//	cmd 명령.
	CString strCMDLine;
	strCMDLine.Format ("\"%s\\extz.exe\" -d \"%s\" -o \"%s\"", pPath->AppRoot().c_str(), szPathName, strTmpFile.GetString() );
